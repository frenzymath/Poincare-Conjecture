...
cross-references ("by \cref{prop:dc-ch4-2-5}", "from (3)", "Chapter 6") so the
...
nowadays $K(p,\sigma)$ is called the \emph{sectional curvature} of $M$ at $p$ with
...
\begin{proposition}[linearity properties of $R$]
  ...
  \dcref{ch4:2.2}
  \lean{Riemannian.AffineConnection.curvature_add_left,
    Riemannian.AffineConnection.curvature_add_middle,
    Riemannian.AffineConnection.curvature_add_right,
    Riemannian.AffineConnection.curvature_smul_left,
    Riemannian.AffineConnection.curvature_smul_middle,
    Riemannian.AffineConnection.curvature_smul_right}
  \leanok
  \uses{def:dc-ch4-2-1}
...
\begin{proof}
  \leanok
  We verify (ii) only, leaving (i) as an exercise. The first part of (ii)
...
\begin{proof}
  \leanok
  From the symmetry of the Riemannian connection,
...
\begin{proposition}[symmetries of the curvature]
  ...
  \lean{Riemannian.AffineConnection.curvatureForm_antisymm_left,
    Riemannian.AffineConnection.curvatureForm_antisymm_right,
    Riemannian.AffineConnection.curvatureForm_bianchi,
    Riemannian.AffineConnection.curvatureForm_pairSwap}
  \leanok
...
\begin{proof}
  \leanok
  To avoid calculating, we observe that we can pass from the basis $\{x,y\}$
  ...
  \emph{Formalization note.} In the Lean development the plane change of basis is
  taken directly: for an invertible linear change $x'=ax+by$, $y'=cx+dy$ (with
  $ad-bc\neq 0$) of a linearly independent pair, both the numerator $(x',y',x',y')$
  and the denominator $|x'\wedge y'|^2$ scale by $(ad-bc)^2$ — the numerator by the
  antisymmetries of \cref{prop:dc-ch4-2-5}, the denominator because $|x\wedge y|^2$
  is itself the diagonal of the algebraic curvature form $R'$ (see
  \cref{lem:dc-ch4-3-4}) — so the ratio $K$ is unchanged.
\end{proof}
...
\begin{lemma}[constant curvature characterization]
  ...
  \dcref{ch4:3.4}
  \lean{Riemannian.IsAlgCurvatureForm.eq_smul_stdCurvForm_of_const,
    Riemannian.IsAlgCurvatureForm.const_of_eq_smul_stdCurvForm}
  \leanok
...
basis; they are called the \emph{Ricci curvature} in the direction $x$ and the *scalar
...
called \emph{the Ricci tensor}. In a coordinate system $(x_i)$ with
...
\begin{lemma}[curvature on a parametrized surface]
  ...
  \dcref{ch4:4.1}
  \lean{Riemannian.Jacobi.surface_covariant_commutator,
    Riemannian.Jacobi.surface_covariant_commutator_of_eventually}
  \leanok
...
\end{lemma}
\begin{proof}
  The proof is a long calculation. Choose a system of coordinates
...
\begin{definition}[tensor of order $r$]
  ...
  \dcref{ch4:5.1}
  \lean{Riemannian.IsCovariantTensor2, Riemannian.IsCovariantTensor4}
  \leanok
...
\begin{example}[the connection is not a tensor]
  \label{ex:dc-ch4-5-4}
  \dcref{ch4:5.4}
  \lean{Riemannian.AffineConnection.connectionForm,
    Riemannian.AffineConnection.connectionForm_not_isCovariantTensor}
  \leanok
