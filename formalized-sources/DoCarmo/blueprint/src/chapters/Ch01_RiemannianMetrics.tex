...
\begin{example}[Lie groups]
  ...
  algebra $\mathcal{G}=T_eG$. Given an inner product on $\mathcal{G}$, formula
  $$
  \langle u,v\rangle_x=
  ...
  A compact Lie group has a bi-invariant metric.

  If the metric is bi-invariant, then for $U,V,X\in\mathcal{G}$,
  $$
  ...
  $t=0$ yields (3). Conversely, a positive bilinear form on $\mathcal{G}$ satisfying
  (3), extended by (2), gives a bi-invariant metric; this converse is not proved here.
\end{example}
