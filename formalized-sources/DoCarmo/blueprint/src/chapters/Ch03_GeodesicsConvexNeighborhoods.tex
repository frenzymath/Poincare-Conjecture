...
\begin{definition}[geodesic]
  ...
  \dcref{ch3:2.1}
  \lean{Riemannian.Geodesic.IsGeodesicCurve}
  \leanok
  ...
  The public Lean predicate \texttt{IsGeodesicCurve} packages continuity
  together with \texttt{IsGeodesic}, the moving-chart geodesic equation.  The
  equation-only predicate is kept separate internally because its
  junk-extended chart readings do not imply continuity by themselves.  The
  current ODE bridge derives the $C^1$ regularity used for speed and length
  arguments.  No separate $C^\infty$ regularity bootstrap is claimed here.
\end{definition}
...
\begin{definition}[exponential map]
  ...
  The Lean declarations attached to this definition use the intrinsic maximal
  geodesic, so their domain is independent of any single coordinate chart.
  The older declarations \texttt{expMap} and \texttt{expDomain} encode a
  chart-fixed local flow; they may be used as a local representative only after
  proving that the relevant geodesic remains in that chart.
\end{definition}
...
\begin{lemma}[base-uniform competitor bound with escape]
  ...
  has length at least $\sqrt{\langle w,w\rangle_{p,\varphi_p q}}$. This is do Carmo's Proposition 3.6
...
\begin{proof}
  ...
  is a homeomorphism) and its closure is compact, so the first exit time is well defined. $\square$
\end{proof}
...
\begin{lemma}[base-uniform lower bound: short geodesics realize the distance]
  ...
  \leanok
  \uses{lem:dc-ch3-4-2-competitor-lb, lem:dc-ch3-4-2-endpoint-id, lem:dc-ch3-geodesic-pathlength,
    lem:dc-ch3-4-2-speedchart, prop:dc-ch3-3-6}
  ...
  the distance between its endpoints: $\ell\le d(q_1,q_2)$. This is the base-uniform reading of do
  Carmo's Proposition 3.6 (\cref{prop:dc-ch3-3-6}) --- exactly the hypothesis $H_{lb}$ of
  \cref{lem:dc-ch3-4-2-minimizing-interior}, now discharged.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[minimizing geodesics with interior in the ball, modulo the lower bound]
  ...
  \uses{lem:dc-ch3-4-2-join-metric, lem:dc-ch3-4-2-interior-minimizing, prop:dc-ch3-3-6,
    cor:dc-ch3-3-9, def:dc-ch3-4-2-stronglyconvex, lem:dc-ch3-4-2-lowerbound}
...
\begin{lemma}[minimizing geodesics with interior in the ball --- unconditional]
  ...
  This is \cref{lem:dc-ch3-4-2-minimizing-interior} with its hypothesis $H_{lb}$ now
  \emph{discharged}: the base-uniform lower bound is the theorem
  \cref{lem:dc-ch3-4-2-lowerbound}. It is the existence + minimizing + interior content of do Carmo's
  strong convexity (\cref{def:dc-ch3-4-2-stronglyconvex}), unconditionally --- only uniqueness remains
  open for the full proposition.
\end{lemma}
\begin{proof}
  \leanok
  Feed the base-uniform lower bound $H_{lb}$ (\cref{lem:dc-ch3-4-2-lowerbound}) into the reduction
  \cref{lem:dc-ch3-4-2-minimizing-interior}. $\square$
\end{proof}
...
\begin{lemma}[strong convexity of the closed ball, reduced to uniqueness alone]
  ...
  \leanok
  \uses{lem:dc-ch3-4-2-stronglyconvex-reduction, lem:dc-ch3-4-2-lowerbound,
    lem:dc-ch3-4-2-minimizing-interior-uncond}
  Assume only the \emph{local uniqueness} $H_{uniq}$ of minimizing geodesics near $p$ (any two
  ...
  \cref{lem:dc-ch3-4-2-stronglyconvex-reduction} with $H_{lb}$ discharged
  (\cref{lem:dc-ch3-4-2-lowerbound}); $H_{uniq}$ is the sole remaining hypothesis of do Carmo's
  Proposition 4.2 for the closed ball.
\end{lemma}
\begin{proof}
  \leanok
  Feed $H_{lb}$ (\cref{lem:dc-ch3-4-2-lowerbound}) into
  \cref{lem:dc-ch3-4-2-stronglyconvex-reduction}. $\square$
\end{proof}
...
\begin{proof}
  ...
  \cref{lem:dc-ch3-4-2-endpoint-id}, freed of the $1<h$ restriction (only $l<0<h$ is used). $\square$
\end{proof}
...
\begin{lemma}[base-uniform geodesic-uniqueness engine]
  ...
  norm $<\rho_w$, coincide on $[0,1]$. This is the \emph{injectivity half} of $H_{uniq}$: the
  uniqueness of minimizing geodesics near $p$, once each competitor is presented with an open time
  window and its chart-$p$ velocity datum.
\end{lemma}
\begin{proof}
  \leanok
  ...
  clause of the totally-normal $C^1$ diffeomorphism (\cref{thm:dc-ch3-3-7}) --- do Carmo's injectivity
...
\begin{proposition}[convex neighborhoods]
  ...
  \uses{thm:dc-ch3-3-7, lem:dc-ch3-4-1, lem:dc-ch3-4-1-strict, lem:dc-ch3-4-2-rebase,
    lem:dc-ch3-4-2-window, lem:dc-ch3-4-2-speed-dist,
    ...
    lem:dc-ch3-4-2-stronglyconvex-reduction, lem:dc-ch3-4-2-lowerbound,
    lem:dc-ch3-4-2-minimizing-interior-uncond, lem:dc-ch3-4-2-stronglyconvex-uniq,
    lem:dc-ch3-4-2-endpoint-id, lem:dc-ch3-4-2-competitor-lb,
    lem:dc-ch3-4-2-eqon-flow-reading, lem:dc-ch3-4-2-uniqueness-engine,
...
\begin{proof}
  ...
  \emph{The lower bound $H_{lb}$ is now a theorem} (\cref{lem:dc-ch3-4-2-lowerbound}): the
  moving-base reconciliation is carried out directly. Constant speed pins the length rate
  $\ell=\sqrt{\langle w,w\rangle_{p,\varphi_p q_1}}$; the joining geodesic is identified with the
  radial flow line by intrinsic uniqueness on the \emph{asymmetric} overlap window
  $(l,h)\cap(-(\varepsilon/T),\varepsilon/T)$ (\cref{lem:dc-ch3-4-2-endpoint-id}), so its endpoint
  $q_2$ is the radial endpoint of $w$; and the base-uniform competitor bound
  (\cref{lem:dc-ch3-4-2-competitor-lb}) --- the moving-base Gauss radius comparison for confined
  curves together with the first-exit escape estimate, both uniform over the ball of centres $q_1$ ---
  makes every $C^1$ competitor from $q_1$ to $q_2$ at least $\sqrt{\langle w,w\rangle}$ long. This
  \emph{replaces} the sought lower-semicontinuous normal radius: the single $\beta$ is extracted once,
  at $p$, from the totally-normal package, and the Gauss estimate is transported to the moving-base
  flow family (rather than re-run per centre).

  \emph{$H_{lb}$ is now discharged into the assembly.} Feeding the theorem
  \cref{lem:dc-ch3-4-2-lowerbound} into the two reductions yields, \emph{unconditionally}, the
  existence + minimizing + interior clauses (\cref{lem:dc-ch3-4-2-minimizing-interior-uncond}), and
  reduces the full proposition for the closed ball to the single hypothesis $H_{uniq}$
  (\cref{lem:dc-ch3-4-2-stronglyconvex-uniq}). So the residual is now precisely: \textbf{uniqueness
  $H_{uniq}$}. Do Carmo reads this off the injectivity of $\exp_{q_1}$; the base-uniform injectivity
  is \emph{already available} (the \texttt{InjOn} clause of \cref{thm:dc-ch3-3-7}'s totally-normal
  package over $B_{\delta_1}(\varphi_p p)\times B_\delta(0)$).

  \emph{The injectivity half of $H_{uniq}$ is now formalized}
  (\cref{lem:dc-ch3-4-2-uniqueness-engine}): once each competitor is presented \emph{with an open time
  window $\supsetneq[0,1]$ and its chart-$p$ velocity datum $w$ ($\|w\|<\rho_w$)}, uniqueness follows
  outright. Each such geodesic coincides with its flow-reading segment $g_w$ on a window containing
  $[0,1]$ (\cref{lem:dc-ch3-4-2-eqon-flow-reading}, the \emph{EqOn} strengthening of the endpoint
  readback \cref{lem:dc-ch3-4-2-endpoint-id}), so both competitors reach $q_2$ as flow-reading
  endpoints; the unique-velocity clause of \cref{thm:dc-ch3-3-7} then forces the two velocities equal,
  hence the two geodesics equal. This is exactly do Carmo's injectivity argument, base-uniform.

  \emph{The presentation is now formalized, and the proposition is unconditional}
  (\texttt{exists\_stronglyConvex\_closedBall}). An abstract minimizing competitor $\alpha$ --- given
  as a geodesic on the \emph{closed} $[0,1]$ with neither an open window nor a velocity --- is
  equipped with a two-sided window $\supsetneq[0,1]$ by \cref{lem:dc-ch3-4-2-window}: the endpoint
  values $\alpha(0)=q_1,\alpha(1)=q_2$ are \emph{known} points (not Cauchy limits), so the geodesic
  prolongs past each endpoint completeness-freely, and the two one-sided extensions glue at $1/2$. Its
  chart-$p$ velocity $w$ then satisfies $\|w\|<\rho_w$ once $\beta$ is small: the conserved speed
  equals $d(q_1,q_2)\le2\beta$ by \cref{lem:dc-ch3-4-2-speed-dist} (a minimizing geodesic has speed
  equal to the distance it realizes, via the radial isometry of the normal ball), and the chart-Gram
  comparison \cref{lem:dc-ch3-4-2-velbound} turns that into $\|w\|\le\sqrt{c}\,d(q_1,q_2)$. Feeding
  both competitors into the injectivity engine \cref{lem:dc-ch3-4-2-uniqueness-engine} discharges
  $H_{uniq}$, so \cref{lem:dc-ch3-4-2-stronglyconvex-uniq} becomes unconditional.

  The proposition is stated for the \emph{closed} ball because the literal open-ball
  form of \cref{def:dc-ch3-4-2-stronglyconvex} is unsatisfiable at a boundary diagonal
  $q_1=q_2\in\partial B_\beta(p)$, where the (constant) minimizing geodesic's interior $\{q_1\}$ is
  not in the open ball.
\end{proof}
