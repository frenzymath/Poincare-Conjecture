...
cross-references ("by \cref{prop:dc-ch6-2-1}", "\cref{thm:dc-ch6-2-5} of this chapter", "Chapter 4")
...
defined in the $x,y$ plane. $\mathit{II}$ is called the \emph{second fundamental form}
...
Gauss formula (\cref{thm:dc-ch6-2-5}), which generalizes (1) and yields the difference
...
$v^N\in(T_pM)^\perp$, calling $v^T$ the \emph{tangential component} and $v^N$ the
\emph{normal component} of $v$; this splitting is differentiable.
...
The identification of $U$ with $f(U)$ places the whole chapter inside the
ambient manifold; the following definitions record the working setup so
obtained and the tangential and normal parts of the objects above.

\begin{definition}[the identified setup: tangent distribution and orthogonal splitting]
  \label{def:dc-ch6-2-patch}
  \lean{Riemannian.DCImmersedPatch,
    Riemannian.DCImmersedPatch.normalSpace,
    Riemannian.DCImmersedPatch.mem_normalSpace_iff,
    Riemannian.DCImmersedPatch.normalProj,
    Riemannian.DCImmersedPatch.IsTangentField,
    Riemannian.DCImmersedPatch.IsNormalField,
    Riemannian.DCImmersedPatch.isTangentField_bracketField}
  \leanok
...
\begin{lemma}[uniqueness and tensoriality of the tangential projection]
  ...
  \lean{Riemannian.DCImmersedPatch.eq_zero_of_mem_tang_of_mem_normalSpace,
    Riemannian.DCImmersedPatch.tangentProj_eq_at,
    Riemannian.DCImmersedPatch.tangentProj_congr_apply,
    Riemannian.DCImmersedPatch.tangentProj_add,
    Riemannian.DCImmersedPatch.tangentProj_smul,
    Riemannian.DCImmersedPatch.tangentProj_apply_of_mem,
    Riemannian.DCImmersedPatch.tangentProj_apply_of_mem_normalSpace,
    Riemannian.DCImmersedPatch.normalProj_add,
    Riemannian.DCImmersedPatch.normalProj_smul}
  \leanok
...
\begin{proposition}[the induced connection is the Riemannian connection of the induced metric]
  ...
  \lean{Riemannian.DCImmersedPatch.isTangentField_inducedCov,
    Riemannian.DCImmersedPatch.inducedCov_add_left,
    Riemannian.DCImmersedPatch.inducedCov_smul_left,
    Riemannian.DCImmersedPatch.inducedCov_add_right,
    Riemannian.DCImmersedPatch.inducedCov_smul_right,
    Riemannian.DCImmersedPatch.inducedCov_sub_swap,
    Riemannian.DCImmersedPatch.inducedCov_compat}
  \leanok
...
\begin{definition}[the second fundamental form field]
  ...
  \lean{Riemannian.DCImmersedPatch.secondFundForm,
    Riemannian.DCImmersedPatch.cov_eq_inducedCov_add_secondFundForm,
    Riemannian.DCImmersedPatch.secondFundForm_mem}
  \leanok
...
\begin{proposition}[the second fundamental form mapping $B$]
  ...
  \lean{Riemannian.DCImmersedPatch.secondFundForm_add_left,
    Riemannian.DCImmersedPatch.secondFundForm_add_right,
    Riemannian.DCImmersedPatch.secondFundForm_sub_left,
    Riemannian.DCImmersedPatch.secondFundForm_smul_left,
    Riemannian.DCImmersedPatch.secondFundForm_smul_right,
    Riemannian.DCImmersedPatch.secondFundForm_symm,
    Riemannian.DCImmersedPatch.isNormalField_secondFundForm}
  \leanok
...
\begin{lemma}[pointwise locality of $B$ and of the covariant derivative]
  ...
  \lean{Riemannian.AffineConnection.cov_congr_apply_left,
    Riemannian.exists_decomposition_of_apply_eq_zero,
    Riemannian.DCImmersedPatch.exists_tangent_decomposition_of_apply_eq_zero,
    Riemannian.DCImmersedPatch.secondFundForm_congr_apply,
    Riemannian.DCImmersedPatch.exists_isTangentField_eq,
    Riemannian.DCImmersedPatch.exists_isNormalField_eq}
  \leanok
  ...
  (resp.\ normal) field, so the pointwise objects of this chapter are
  well-defined.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{definition}[second fundamental form $\mathit{II}_\eta$]
  ...
  \lean{Riemannian.DCImmersedPatch.secondFundFormAt,
    Riemannian.DCImmersedPatch.secondFundScalarAt,
    Riemannian.DCImmersedPatch.secondFundQuadAt,
    Riemannian.DCImmersedPatch.shapeOperatorAt,
    Riemannian.DCImmersedPatch.inner_shapeOperatorAt,
    Riemannian.DCImmersedPatch.inner_shapeOperatorAt_symm,
    Riemannian.DCImmersedPatch.secondFundScalarAt_symm,
    Riemannian.DCImmersedPatch.secondFundFormAt_symm}
  \leanok
...
\begin{example}[hypersurface; principal curvatures; Gauss spherical mapping]
  \label{ex:dc-ch6-2-4}
  \dcref{ch6:2.4}
  \uses{def:dc-ch6-2-2, prop:dc-ch6-2-3, def:dc-ch6-2-4-principal,
    lem:dc-ch6-2-8-euclidean-opens, ex:dc-ch6-2-8}
  \lean{Riemannian.gaussMap, Riemannian.gaussMap_norm,
    Riemannian.normalSpace_eq_span_of_unit_normal,
    Riemannian.mem_tang_of_inner_unit_normal_eq_zero,
    Riemannian.fderiv_gaussMap_mem_tang,
    Riemannian.fderiv_gaussMap_eq_neg_shapeOperatorAt,
    Riemannian.fderiv_gaussMap_mem_sphereTang}
  \leanok
  ...
  In the identified picture, for a hypersurface patch in an open subset of
  Euclidean space (\cref{lem:dc-ch6-2-8-euclidean-opens}) with a smooth unit
  normal field $N$: differentiating $\langle N,N\rangle\equiv1$ gives
  $\langle N(q),dN_q(v)\rangle=0$, so $dg_q(v)$ is tangent both to the
  unit-sphere leaf through $g(q)$ of the concentric-sphere foliation of
  \cref{ex:dc-ch6-2-8} (the "parallel tangent spaces" identification) and — by
  the codimension-$1$ fact that the normal space is exactly the line
  $\mathbb{R}\,N(q)$ — to the hypersurface itself; the Weingarten identity of
  \cref{prop:dc-ch6-2-3} then yields $dg_q(x)=\overline\nabla_xN
  =(\overline\nabla_xN)^T=-S_\eta(x)$.
\end{example}
...
\begin{remark}[compact hypersurfaces with non-vanishing Gauss-Kronecker curvature]
  ...
  \uses{ex:dc-ch6-2-4}
  \notready
  As an application: if $M^n$, $n\geq 2$, is a connected, compact,
  ...
  hence a diffeomorphism since $S_1^n$ is simply connected). The covering-space
  theory needed here is not yet available in the development; this application
...
\begin{definition}[principal curvatures and directions; Gauss-Kronecker and mean curvature]
  ...
  \uses{def:dc-ch6-2-2}
  \lean{Riemannian.DCImmersedPatch.shapeOperatorHom,
    Riemannian.DCImmersedPatch.shapeOperatorHom_isSymmetric,
    Riemannian.DCImmersedPatch.principalCurvatures,
    Riemannian.DCImmersedPatch.principalDirections,
    Riemannian.DCImmersedPatch.principalDirection,
    Riemannian.DCImmersedPatch.metricInner_principalDirection,
    Riemannian.DCImmersedPatch.shapeOperatorAt_principalDirection,
    Riemannian.DCImmersedPatch.secondFundScalarAt_principalDirection,
    Riemannian.DCImmersedPatch.gaussKroneckerCurvature,
    Riemannian.DCImmersedPatch.meanCurvature,
    Riemannian.DCImmersedPatch.trace_shapeOperatorHom,
...
\begin{theorem}[Gauss]
  ...
  \lean{Riemannian.DCImmersedPatch.gauss_theorem,
    Riemannian.DCImmersedPatch.inducedCurvature_inner_sub_curvature_inner}
  \leanok
...
\begin{proof}
  \leanok
  Let $X,Y$ be local orthogonal extensions of $x,y$, tangent to $M$, with
  ...
  Alternatively, the theorem is the special case $Z=x$, $T=y$ of the Gauss
  equation of \cref{prop:dc-ch6-3-1}, together with the symmetry of $B$
  (\cref{prop:dc-ch6-2-1}); for orthonormal $x,y$ the normalizing factor
  $|x|^2|y|^2-\langle x,y\rangle^2$ of the sectional curvature equals $1$.
\end{proof}
...
\begin{remark}[Gauss formula for a hypersurface]
  ...
  \lean{Riemannian.DCImmersedPatch.secondFundFormAt_eq_secondFundScalarAt_smul,
    Riemannian.DCImmersedPatch.hypersurface_gauss}
  \leanok
...
\begin{lemma}[orthonormal pairs spanning a plane give equal curvature values]
  ...
  \lean{Riemannian.linearIndependent_pair_of_orthonormal,
    Riemannian.IsAlgCurvatureForm.apply_eq_of_orthonormal_of_mem_span,
    Riemannian.IsAlgCurvatureForm.apply_eq_of_orthonormal_of_finrank_eq_two}
  \leanok
...
\begin{definition}[pushforward of fields along a diffeomorphism of Euclidean opens]
  ...
  \lean{Riemannian.opensMapExt, Riemannian.opensFDeriv,
    Riemannian.opensPushforward, Riemannian.fderiv_extendZero_comp,
    Riemannian.opensFDeriv_symm_comp}
  \leanok
...
\begin{lemma}[naturality of derivations and brackets under pushforward]
  ...
  \lean{Riemannian.dir_opensPushforward,
    Riemannian.DCLieBracket_opensPushforward,
    Riemannian.bracketField_opensPushforward,
    Riemannian.metricInner_field_contMDiff_opens}
  \leanok
...
\begin{proof}
  ...
  second-derivative terms cancel by the symmetry of $d^2\varphi_p$ (Schwarz),
  leaving
  ...
  derivatives (\cref{lem:dc-ch0-5-2}, computed in
  \cref{lem:dc-ch6-2-8-euclidean-opens}).
\end{proof}
...
\begin{definition}[isometry of immersed patches]
  ...
  \lean{Riemannian.DCPatchIsometry,
    Riemannian.DCPatchIsometry.isTangentField_opensPushforward,
    Riemannian.DCPatchIsometry.metricInner_opensPushforward}
  \leanok
  ...
  $$
  Nothing is required of $d\varphi$ transverse to the distribution — this is
  do Carmo's isometry (\cref{def:dc-ch1-2-2}) of the immersed manifolds, read
  after the identification of each with its image. The pushforward of a
  tangent field along an isometry is a tangent field, and isometries preserve
  inner products of tangent fields pointwise.
\end{definition}

\begin{lemma}[isometries preserve the induced connection]
  \label{lem:dc-ch6-2-7-connection}
  \uses{def:dc-ch6-2-7-isometry, lem:dc-ch6-2-7-koszul,
    lem:dc-ch6-2-7-pushforward-naturality}
...
\begin{lemma}[isometries preserve the induced curvature]
  ...
  \lean{Riemannian.DCPatchIsometry.inducedCurvature_opensPushforward,
    Riemannian.DCPatchIsometry.inducedCurvature_inner_opensPushforward}
  \leanok
...
\begin{lemma}[the pointwise Gauss form of a patch in a flat ambient]
  ...
  \uses{def:dc-ch6-2-2, prop:dc-ch6-3-1, prop:dc-ch4-2-5,
    lem:dc-ch6-2-8-euclidean-opens}
  \lean{Riemannian.DCImmersedPatch.gaussFormAt,
    Riemannian.DCImmersedPatch.isAlgCurvatureForm_gaussFormAt,
    Riemannian.inducedCurvature_inner_eq_gaussFormAt}
  \leanok
...
\begin{proof}
  ...
  (\cref{lem:dc-ch6-2-8-euclidean-opens}), the field-level second fundamental
  forms being evaluated through the pointwise one
  (\cref{def:dc-ch6-2-2}).
\end{proof}

\begin{remark}[Theorema Egregium]
...
\begin{lemma}[the Euclidean ambient: flat connection, vanishing curvature]
  ...
  \uses{def:dc-ch1-2-1, thm:dc-ch2-3-6, def:dc-ch4-2-1,
    def:dc-ch6-3-constant-curvature}
  \lean{Riemannian.SmoothVectorField.contDiff,
    Riemannian.DCLieBracket_eq_fderiv,
    Riemannian.euclideanCov,
    Riemannian.euclideanConnection,
    Riemannian.euclideanConnection_isLeviCivita,
    Riemannian.euclideanConnection_curvature,
    Riemannian.euclideanConnection_isConstantCurvature_zero}
  \leanok
  ...
  This is the ambient datum for the hypersurface examples of this chapter
  (\cref{ex:dc-ch6-2-4}, \cref{ex:dc-ch6-2-8}, \cref{rem:dc-ch6-2-7}).
\end{lemma}
\begin{proof}
  \leanok
  ...
  R(X,Y)Z=\nabla_Y\nabla_XZ-\nabla_X\nabla_YZ+\nabla_{[X,Y]}Z ,
  $$
  the second-order terms cancel by symmetry of the second derivative
  (Schwarz), and the remaining first-order terms cancel exactly against
...
\begin{lemma}[open subsets of Euclidean space as ambient manifolds]
  ...
  \uses{def:dc-ch1-2-1, thm:dc-ch2-3-6, def:dc-ch4-2-1,
    def:dc-ch6-3-constant-curvature, lem:dc-ch6-2-8-euclidean}
  \lean{Riemannian.opensEuclideanMetric,
    Riemannian.opensEuclideanConnection,
    Riemannian.opensEuclideanConnection_isLeviCivita,
    Riemannian.opensEuclideanConnection_curvature,
    Riemannian.opensEuclideanConnection_isConstantCurvature_zero,
    Riemannian.DCLieBracket_opens_eq_fderiv}
  \leanok
...
\begin{proof}
  ...
  vanishes by symmetry of the second derivative (Schwarz). $\square$
\end{proof}

\begin{example}[curvature of $S^n$]
  \label{ex:dc-ch6-2-8}
  \dcref{ch6:2.8}
  \uses{def:dc-ch6-2-patch, def:dc-ch6-2-B, def:dc-ch6-2-2,
    thm:dc-ch6-2-5, rem:dc-ch6-2-6, lem:dc-ch6-2-8-euclidean-opens}
  \lean{Riemannian.punctured, Riemannian.sphereTang, Riemannian.spherePatch,
    Riemannian.spherePatch_secondFundForm_apply,
    Riemannian.spherePatch_secondFundScalarAt,
    Riemannian.spherePatch_shapeOperatorAt,
    Riemannian.spherePatch_inducedCurvature_inner,
    Riemannian.sphere_sectionalCurvature_one}
  \leanok
  ...
  Concretely, work in the punctured space $F\setminus\{0\}$ of
  \cref{lem:dc-ch6-2-8-euclidean-opens}, foliated by the concentric spheres:
  the sphere through $p$ has tangent space $(\mathbb{R}p)^\perp$ and
  orthogonal projection $v\mapsto v-\frac{\langle p,v\rangle}{\langle
  p,p\rangle}p$. Differentiating the tangency identity $\langle x,
  Y(x)\rangle\equiv0$ of a tangent field along a tangent direction gives the
  second fundamental form $B(X,Y)=-\frac{\langle X,Y\rangle}{\langle
  p,p\rangle}\,p$; at $|p|=1$ with inward unit normal $\eta=-p$ this is
  $H_\eta(x,y)=\langle x,y\rangle$, so $S_\eta=\mathrm{id}$. By Gauss'
  theorem (\cref{thm:dc-ch6-2-5}) against the flat ambient, the sphere of
  radius $r$ has constant sectional curvature $\langle
  p,p\rangle^{-1}=1/r^2$, equal to $1$ on the unit sphere.
...
\begin{definition}[geodesic at a point; totally geodesic immersion]
  ...
  \uses{def:dc-ch6-2-2}
  \lean{Riemannian.DCImmersedPatch.IsGeodesicAt,
    Riemannian.DCImmersedPatch.IsTotallyGeodesic,
    Riemannian.DCImmersedPatch.isGeodesicAt_iff_secondFundFormAt_eq_zero}
  \leanok
  An immersion $f:M\to\overline{M}$ is \emph{geodesic at} $p\in M$ if for every
...
\begin{definition}[geodesic field at a point]
  ...
  \leanok
  A tangent field $X\in\mathcal{X}(U)$ is a \emph{geodesic field at} $p$ when its
  induced covariant self-derivative vanishes there: $(\nabla_X X)(p)=0$. In the
  identified picture these are exactly the extensions of the velocity fields of
  curves of $M$ through $p$ that are geodesic at $p$; in particular the velocity
  extension of any geodesic $\gamma$ of $M$ starting from $p$ is a geodesic field
  at $p$, since $\nabla_{\gamma'}\gamma'\equiv0$ along $\gamma$.
\end{definition}
...
\begin{lemma}[existence of geodesic fields; first-order scalar data]
  ...
  \lean{Riemannian.exists_contMDiff_dirTangent_eq_one,
    Riemannian.AffineConnection.cov_apply_eq_zero_of_apply_eq_zero_left,
    Riemannian.DCImmersedPatch.exists_isGeodesicFieldAt}
  \leanok
...
\begin{proof}
  ...
  $(\nabla_XX)(p)=a+X_0(f)(p)\,W(p)=a+1\cdot(-a)=0$. $\square$
\end{proof}
...
\begin{proof}
  ...
  do Carmo quantifies over the genuine geodesics of $M$ through $p$; every such
  geodesic yields a geodesic field at $p$ by extending its velocity, so the
  criterion above is the pointwise content of his statement. Recovering his
  literal quantifier in addition needs the \emph{existence} of geodesics of $M$
  through $p$ tangent to each $x$ — geodesic ODE theory inside the distribution
  (equivalently on the leaf through $p$), which is deferred together with the
  chart-Christoffel bridge (cf.\ the workspace issues on ODE smooth dependence
  and the chart-confined geodesic theory).
\end{proof}
...
\begin{remark}[geometric interpretation of sectional curvature]
  ...
  \uses{prop:dc-ch6-2-9, thm:dc-ch6-2-5, def:dc-ch6-2-8-geodesic,
    def:dc-ch4-3-2}
  \lean{Riemannian.DCImmersedPatch.secondFundForm_apply_eq_zero_of_isGeodesicAt,
    Riemannian.DCImmersedPatch.inducedCurvature_inner_eq_curvature_inner_of_isGeodesicAt,
    Riemannian.DCImmersedPatch.sectionalCurvature_eq_of_isGeodesicAt,
    Riemannian.DCImmersedPatch.inducedCurvatureAt_inner_eq_of_isGeodesicAt}
  \leanok
  ...
  This is exactly the way Riemann defined sectional curvature in [Ri].

  The displayed equation holds for every submanifold that is geodesic at $p$,
  as $S$ is: when the second fundamental form vanishes at $p$
  (\cref{def:dc-ch6-2-8-geodesic}), the right-hand side
  $\langle B(x,x),B(y,y)\rangle-|B(x,y)|^2$ of Gauss' theorem
  (\cref{thm:dc-ch6-2-5}) vanishes, so the sectional curvatures of the
  submanifold and of the ambient manifold agree on every tangent plane
  $\sigma=\operatorname{span}(x,y)\subseteq T_pS$ at $p$, computed as in
  \cref{thm:dc-ch6-2-5} on an orthonormal pair $x,y$ spanning $\sigma$; for a
  two-dimensional submanifold this is $K_S(p)=K(p,\sigma)$ with $\sigma=T_pS$.
  The construction of the geodesic surface $S=\exp_p(\sigma\cap B)$ itself is
  the subject of \cref{rem:dc-ch6-2-9-exp-surface}.
\end{remark}

\begin{remark}[the geodesic surface $\exp_p(\sigma\cap B)$; E. Cartan's theorem]
  \label{rem:dc-ch6-2-9-exp-surface}
  \source{docarmo:page-0148}
  \uses{prop:dc-ch6-2-9}
  \notready
  Realizing $S=\exp_p(\sigma\cap B)$ as a submanifold geodesic at $p$ requires
  the exponential map as a smooth local diffeomorphism, the restriction
  $\exp_p|_{\sigma\cap B}$ as an immersion, and the geodesic theory of the
  surface itself (every geodesic of $M$ tangent to $\sigma$ at $p$ lies in $S$
  near $p$, and its velocity extends to a geodesic field of $S$ at $p$). This
  exponential-map development belongs to Chapter~3/Chapter~7 and is not yet
  available; the construction remains a documented gap.

  Examples of totally geodesic submanifolds are rare. In the case
  $\overline{M}=\mathbb{R}^n$, the linear subspaces and their translates are
  evidently totally geodesic submanifolds. In the case
  $\overline{M}=S^n\subset\mathbb{R}^{n+1}$, the intersections $\Sigma$ of
  linear subspaces of $\mathbb{R}^{n+1}$ with $S^n$ are totally geodesic
  submanifolds: for every $p\in\Sigma$, the geodesics of $S^n$ that start from
  $p$ and are tangent to $\Sigma$ are geodesics of $\Sigma$. It was proven by
  E.~Cartan that, conversely, if a Riemannian manifold $M$ has, for every $p$
  and every two-dimensional $\sigma\subset T_pM$, a totally geodesic
  submanifold tangent to $\sigma$, then $M$ has constant sectional curvature;
  these examples and the converse likewise carry no proof obligation here.
\end{remark}
...
\begin{definition}[minimal immersion; mean curvature vector]
  ...
  \lean{Riemannian.DCImmersedPatch.IsMinimal,
    Riemannian.DCImmersedPatch.meanCurvatureVector,
    Riemannian.DCImmersedPatch.meanCurvatureVector_mem,
    Riemannian.DCImmersedPatch.inner_meanCurvatureVector,
    Riemannian.DCImmersedPatch.meanCurvatureVector_unique,
    Riemannian.DCImmersedPatch.existsUnique_meanCurvatureVector,
    Riemannian.DCImmersedPatch.isMinimal_iff_meanCurvatureVector_eq_zero,
    Riemannian.DCImmersedPatch.shapeOperatorAt_add_normal,
...
$T\overline{M}$ over $M$ decomposes into the direct sum of the \emph{tangent bundle}
$TM$ and the \emph{normal bundle} $TM^\perp$. We use Latin letters $X,Y,Z$ for fields
...
the tangent bundle, we introduce the \emph{normal curvature} $R^\perp$ of the immersion,
...
\begin{definition}[shape operator and normal connection: the Weingarten decomposition]
  ...
  \lean{Riemannian.DCImmersedPatch.shapeOperator,
    Riemannian.DCImmersedPatch.normalCov,
    Riemannian.DCImmersedPatch.cov_eq_neg_shapeOperator_add_normalCov,
    Riemannian.DCImmersedPatch.shapeOperator_mem,
    Riemannian.DCImmersedPatch.normalCov_mem}
  \leanok
...
\begin{proposition}[connection laws of the normal connection]
  ...
  \lean{Riemannian.DCImmersedPatch.normalCov_add_left,
    Riemannian.DCImmersedPatch.normalCov_smul_left,
    Riemannian.DCImmersedPatch.normalCov_add_right,
    Riemannian.DCImmersedPatch.normalCov_smul_right}
  \leanok
...
\begin{lemma}[Weingarten pairing and self-adjointness]
  ...
  \lean{Riemannian.DCImmersedPatch.inner_shapeOperator_apply,
    Riemannian.DCImmersedPatch.inner_shapeOperator_symm}
  \leanok
...
\begin{definition}[curvatures of the induced and normal connections]
  ...
  \lean{Riemannian.DCImmersedPatch.inducedCurvature,
    Riemannian.DCImmersedPatch.inducedCurvature_mem,
    Riemannian.DCImmersedPatch.normalCurvature,
    Riemannian.DCImmersedPatch.normalCurvature_mem}
  \leanok
...
\begin{lemma}[decomposition of the ambient curvature]
  ...
  \lean{Riemannian.DCImmersedPatch.curvature_decomposition,
    Riemannian.DCImmersedPatch.curvature_normal_decomposition}
  \leanok
...
\begin{proposition}[Gauss equation and Ricci equation]
  ...
  \lean{Riemannian.DCImmersedPatch.gauss_equation,
    Riemannian.DCImmersedPatch.ricci_equation}
  \leanok
...
\begin{remark}[flat normal bundle]
  ...
  \lean{Riemannian.DCImmersedPatch.ricci_equation_of_isConstantCurvature,
    Riemannian.DCImmersedPatch.HasFlatNormalBundle,
    Riemannian.DCImmersedPatch.ShapeOperatorsCommuteAt,
    Riemannian.DCImmersedPatch.hasFlatNormalBundle_iff_shapeOperatorsCommute,
...
\begin{definition}[the covariant derivative of the second fundamental form]
  ...
  \lean{Riemannian.DCImmersedPatch.secondFundTensor,
    Riemannian.DCImmersedPatch.secondFundTensorCovDeriv}
  \leanok
...
\begin{remark}[Codazzi for constant curvature ambient and hypersurfaces]
  ...
  \lean{Riemannian.DCImmersedPatch.codazzi_symm_of_isConstantCurvature,
    Riemannian.DCImmersedPatch.normalCov_eq_zero_of_unit_spanning,
    Riemannian.DCImmersedPatch.codazzi_hypersurface}
  \leanok
