...
\begin{example}[hypersurface; principal curvatures; Gauss spherical mapping]
  ...
  $\eta$. Let $S_1^n=\{x\in\mathbb{R}^{n+1};|x|=1\}$ and define the \emph{Gauss spherical
  ...
  Differentiating $\langle N,N\rangle=1$ gives $\langle N(q),dN_q(v)\rangle=0$. Since the normal space is $\mathbb R N(q)$, \cref{prop:dc-ch6-2-3} gives $dg_q(v)=(\overline\nabla_vN)^T=-S_\eta(v)$.
\end{example}
...
\begin{remark}[geometric interpretation of sectional curvature]
  ...
  two-dimensional, then $S=\exp_p(\sigma\cap B)$ is geodesic at $p$. Its
  second fundamental form therefore vanishes there, and Gauss' formula gives
