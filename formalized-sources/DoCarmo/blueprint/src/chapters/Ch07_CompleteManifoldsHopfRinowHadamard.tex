...
\begin{definition}[distance]
  ...
  \dcref{ch7:2.4}
  \uses{def:dc-ch1-2-9}
  \lean{Riemannian.piecewiseRiemannianEDist,
    Riemannian.piecewiseRiemannianEDist_eq_riemannianEDist,
    Riemannian.RiemannianMetric.IsRiemannianDist}
  \leanok
The \emph{distance} $d(p,q)$ is defined by $d(p,q)=$ infimum of the lengths of all
  ...
  In the formal presentation, \texttt{piecewiseRiemannianEDist} takes the infimum over explicit
  finite partitions $(\gamma,n,\tau)$ whose restrictions to the intervals
  $[\tau_i,\tau_{i+1}]$ are $C^\infty$.  Additivity of length and the triangle inequality show
  that \texttt{Manifold.riemannianEDist} is a lower bound for every such competitor.  Conversely,
  a $C^1$ path is subdivided into sufficiently small chart windows and replaced there by the
  pullbacks of straight chart segments.  Continuity of the chart Gram forms controls the polygon's
  length arbitrarily closely by the original path length.  Therefore the two infima agree.
\end{definition}
...
\emph{Standing convention of the chapter.} The manifold $M$ is connected
(Lean: \texttt{[ConnectedSpace M]}) and $\dim M>0$
(Lean: \texttt{NeZero (Module.finrank} $\mathbb R$ \texttt{E)}).
In b), Lean's \texttt{ProperSpace M} formulation is related to the displayed
statement by \texttt{Metric.isCompact\_iff\_isClosed\_bounded}, together with
\texttt{Metric.isClosed\_closedBall} and \texttt{Metric.isBounded\_closedBall}.
\begin{proof}
