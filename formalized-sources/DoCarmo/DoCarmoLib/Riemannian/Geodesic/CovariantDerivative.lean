...
* `covariantDerivCoord_eq_zero_iff` — the parallelism equation in solved ODE form
  `V' = −Γ(u', V)(u)`, the first-order linear system whose Picard–Lindelöf theory
...
`DV/dt = V' + Γ(u', V)(u)`, where `Γ` is `Geodesic.chartChristoffelContraction g α`.
...
derivative along a curve, `D/dt (f · V) = f' · V + f · DV/dt`, for a scalar
...
iff it solves the first-order **linear** system `V'(t) = −Γ(u'(t), V(t))(u(t))`
...
`E` being finite-dimensional — continuous. Writing `A(t) = Γ(u'(t), ·)(u(t))`, a
vector field `V` along `u` is parallel iff it solves the first-order linear system
`V' = −A(t) V`; this is the coefficient `A(t)`, whose operator norm is the natural
...
`V' = −Γ(u', V)(u)` (`isParallelCoord_iff_hasDerivAt`), obtained from Grönwall's
...
`chartChristoffelContractionRight g α (u' t) (u t)` over `[a, b)`; for a `C¹` curve `u` and
...
i.e. solves the first-order linear system `V' = −Γ(u', V)(u)` on `[a, b]`.

This is the existence half of `prop:dc-ch2-2-6`, complementing the uniqueness half
`isParallelCoord_eqOn_Icc`. The parallelism system is the linear ODE `V' = A(t) V` with
coefficient `A(t) = −chartChristoffelContractionRight g α (u' t) (u t)`, and the solution is
...
ODE `V' = −Γ(u', V)(u)` on `[a, b]` (as `HasDerivWithinAt` on `Icc a b`, exactly the shape
...
coefficient `A(t) = −chartChristoffelContractionRight g α (u' t) (u t)`, and complements the
...
`V' = −Γ(u', V)(u)` being first-order linear — and an injective linear endomorphism of the
...
`V(b) = P_c V₀`, solving the parallelism ODE `V' = −Γ(u', V)(u)` on `[a, b]`. Existence of the
...
`DX_j/dt = ∇_{dc/dt} X_j` — obtained from the chain rule `d/dt Y(u(t)) = DY(u(t))·u'(t)`, which
turns the plain `V'` term of the covariant derivative into the directional-derivative term of
...
`∑_{i,j}(∑_k (u')^k ∂_k G_{ij})\, V^i W^j
  = ⟨Γ(u',V), W⟩_y + ⟨V, Γ(u',W)⟩_y`. -/
...
(b) the Leibniz rule, and (c) the induced-field rule `D(Y∘u)/dt = (∇_{u'}Y)(u)`. The
...
* (b) the Leibniz rule `D(f • V) = f' • V + f • D V` for scalar `f : ℝ → ℝ`;
* (c) the induced-field rule `D(Y ∘ u) = (∇_{u'} Y)(u) = chartCovariantDeriv`.
...
(`DV/dt = V' + A(t)V`), the metric read along the curve is a curve of bilinear forms
...
coefficient `A : ℝ → E →L[ℝ] E`: `DV/dt = V'(t) + A(t)·V(t)`. Specializes to
`covariantDerivCoord` for the Levi-Civita coefficient `A(t) = Γ(u'(t),·)(u(t))`. -/
...
connection coefficient `A` when `V' = −A V`, i.e. `DV/dt ≡ 0`. -/
...
`G'(V,W) + G(V',W) + G(V,W')`, obtained by applying `HasDerivAt.clm_apply` twice. -/
...
`G'(a,b) = G(A a, b) + G(a, A b)`, do Carmo's coordinate form of `∇g = 0`. Both metric
...
identity `G'(a,b) = G(Aa,b) + G(a,Ab)`: the derivative of `G(V,W)` computed by
`hasDerivAt_bilin_along` matches `G(DV,W) + G(V,DW)` iff the `G'` term equals the two
...
`G' = G(A·,·) + G(·,A·)`; the reverse says a vanishing derivative gives constancy. -/
