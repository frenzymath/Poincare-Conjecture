...
(`DoCarmoLib.Riemannian.Jacobi.ChartCurvatureVector`), the Jacobi equation
...
`HasDerivWithinAt` style as `DoCarmoLib.Riemannian.Jacobi.ParallelFrame`:
...
* `jacobiPairCoeffCoord` — the pair-system coefficient
  ...
  DoCarmoLib's first-order theory `Riemannian.LinearODE.IsSolOn`;
