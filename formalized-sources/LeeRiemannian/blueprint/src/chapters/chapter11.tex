% Source: Lee, "Introduction to Riemannian Manifolds" (2nd ed.), Chapter 11 "Comparison Theory", PDF pages 329-354 of the cited source.
...
\begin{proof}
Let $\gamma:[0,b]\to M$ be a unit-speed geodesic segment, and let $J$ be a
nontrivial normal Jacobi field along $\gamma$ with $J(0)=0$. Its initial
covariant derivative is nonzero by uniqueness for the Jacobi equation. If
$c\leq0$, the Sturm inequality for the Euclidean comparison field gives
\[
|J(t)|\geq t|D_tJ(0)|>0
\]
for every $t\in(0,b]$. If $c=1/R^2>0$, comparison with the constant-curvature
field gives
\[
|J(t)|\geq R\sin(t/R)|D_tJ(0)|>0
\]
whenever $0<t<\pi R$. Thus no such field can vanish again in either of the
stated ranges, so there is no conjugate point there. \qed
\end{proof}
