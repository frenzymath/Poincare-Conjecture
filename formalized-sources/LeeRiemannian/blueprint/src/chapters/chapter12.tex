...
For $n\geq2$, show that the paraboloid
\[
M=\{(x^1,\ldots,x^n,y)\in\mathbb R^{n+1}:y=|x|^2\}
\]
with its induced metric has strictly positive sectional and Ricci curvatures
everywhere but is noncompact.
\end{exercise}
...
$\frac14$-pinched, as computed in Problem~8-13.
...
Let $(M,g)$ be a simply connected Riemannian $n$-manifold of constant
sectional curvature, without assuming completeness. Prove that $M$ admits an
isometric immersion into the corresponding model
$\mathbb R^n$, $S^n(R)$, or $\mathbb H^n(R)$.
\end{exercise}
...
If $n$ is even, prove that every orientation-preserving orthogonal map of
$\mathbb R^{n+1}$ fixes a point of the unit sphere. Deduce that every
even-dimensional spherical space form is isometric either to a round sphere
or to real projective space with a constant multiple of the metric from
Example~2.34.
\end{exercise}
...
Prove that every compact $2$-dimensional Euclidean space form is
diffeomorphic to the torus or Klein bottle, and every noncompact one is
diffeomorphic to $\mathbb R^2$, $S^1\times\mathbb R$, or the open
Möbius band of Example~2.35.
\end{exercise}
...
A \emph{lattice} in $\mathbb R^n$ is an additive subgroup
\[
\Lambda=
\{m_1v_1+\cdots+m_nv_n:m_1,\ldots,m_n\in\mathbb Z\}
\]
for some basis $(v_1,\ldots,v_n)$ of $\mathbb R^n$. Let $T^n$ be the
$n$-torus.
\begin{enumerate}
\item[(a)] Prove that every flat Riemannian metric $g$ on $T^n$ makes
$(T^n,g)$ isometric to $\mathbb R^n/\Lambda$ for some lattice acting by
translations.
\item[(b)] Prove that
$\mathbb R^n/\Lambda_1$ and $\mathbb R^n/\Lambda_2$ are isometric if and
only if $\Lambda_2=A(\Lambda_1)$ for some $A\in O(n)$.
\end{enumerate}
\end{exercise}
...
Let $T^2=S^1\times S^1$. Prove that every flat metric on $T^2$ is isometric
to exactly one quotient $\mathbb R^2/\Lambda$ for which $\Lambda$ is
generated by
\[
(a,0),\qquad(b,c),
\]
with
\[
a>0,\qquad 0\leq b\leq\frac a2,\qquad c>0,\qquad b^2+c^2\geq a^2.
\]
\emph{Hint.} For a lattice $\Lambda\subseteq\mathbb R^2$, choose a nonzero
$v_1\in\Lambda$ of minimum norm and
$v_2\in\Lambda\setminus\langle v_1\rangle$ of minimum norm, choosing its sign
so that the angle with $v_1$ is at most $\pi/2$. Then apply a suitable
...
Let $(M,g)$ be complete, connected, and have nonpositive sectional
curvature. For $p,q\in M$, every path-homotopy class from $p$ to $q$
contains a geodesic segment by Proposition~6.25. Prove that this geodesic is
unique within its path-homotopy class.
\end{exercise}
...
Prove Proposition~12.10 on triangle inequalities in Cartan--Hadamard
manifolds. \emph{Hint.} Compare with suitable Euclidean triangles.
\end{exercise}
...
Prove Corollary~12.23 on the nonexistence of negatively curved metrics on
compact product manifolds.
\end{exercise}
...
Prove Hermann's generalization of Cartan--Hadamard [Her63]. Let $(M,g)$ be
complete and connected with nonpositive sectional curvature, and let
$P\subseteq M$ be connected, properly embedded, and totally geodesic. If for
some $x\in P$ the inclusion induces a surjection
\[
\pi_1(P,x)\longrightarrow\pi_1(M,x),
\]
then the normal exponential map
\[
E:NP\longrightarrow M
\]
is a diffeomorphism.
\emph{Hint.} Adapt the Cartan--Hadamard proof using Problem~10-20.
\end{exercise}

\begin{exercise}
\label{prob:myers-scalar-counterexample}
\dcref{ch12:12-10}

In every dimension greater than $2$, give a counterexample to Myers's theorem
with its positive lower Ricci bound replaced by a positive lower scalar
curvature bound.
\end{exercise}

\begin{exercise}
\label{prob:milnor-fundamental-group}
\dcref{ch12:12-11}
\uses{thm:milnor-polynomial-growth,prob:nonpositive-curvature-focal-points}

Let $(M,g)$ be a complete, connected Riemannian $n$-manifold with
nonnegative Ricci curvature. Prove Theorem~12.29 on polynomial growth of its
fundamental group. Let
$\pi:(\widetilde M,\widetilde g)\to(M,g)$ be the universal Riemannian cover,
with $\widetilde g=\pi^*g$. It suffices to treat a finitely generated subgroup
$\Gamma$ of the deck-transformation group. Choose generators
$S=\{\varphi_1,\ldots,\varphi_N\}$ and $p\in\widetilde M$, and define
\[
d_i=d_{\widetilde g}(p,\varphi_i(p)),
\qquad
D=\max(d_1,\ldots,d_N).
\]
For nonidentity $\psi\in\Gamma$, define its length as the least $m$ for which
$\psi$ is a product of $m$ elements of $S$ and their inverses.
\begin{enumerate}[label=(\alph*)]
\item Prove that some $\varepsilon>0$ makes the balls
$B_\varepsilon(\psi_1(p))$ and $B_\varepsilon(\psi_2(p))$ disjoint whenever
$\psi_1\neq\psi_2$ in $\Gamma$.
\item If $\psi$ has length $m$, prove
\[
B_\varepsilon(\psi(p))\subseteq B_{mD+\varepsilon}(p).
\]
\item Use Bishop--Gromov comparison to prove that the number of elements of
$\Gamma$ of length at most $m$ is at most $C\,m^n$ for a constant $C$.
\end{enumerate}
\end{exercise}

\begin{exercise}
\label{prob:no-positive-negative-ricci}
\dcref{ch12:12-12}

Prove that no compact manifold admits both a metric with positive-definite
Ricci curvature and a metric with nonpositive sectional curvature.
\end{exercise}

\begin{exercise}
\label{prob:complete-lines-sectional-curvature}
\dcref{ch12:12-13}

Let $(M,g)$ be complete. A \emph{line} is the image of a nonconstant
geodesic $\gamma:\mathbb R\to M$ that minimizes between every pair of its
points.
\begin{enumerate}[label=(\alph*)]
\item If $M$ has strictly positive sectional curvature, prove that it
contains no line.

\emph{Hint.} Given $\gamma:\mathbb R\to M$, choose a smooth positive function
$\alpha$ with $\alpha(t)$ below the minimum sectional curvature at
$\gamma(t)$. Let $u$ solve
\[
u''+\alpha u=0,\qquad u(0)=1,\qquad u'(0)=0.
\]
Find $a<0<b$ with $u(a)=u(b)=0$, and evaluate $I(V,V)$ for
$V(t)=u(t)E(t)$ along $\gamma|_{[a,b]}$, where $E$ is a parallel unit normal
field.
\item Construct a complete, nonflat Riemannian manifold with nonnegative
sectional curvature that contains a line.
\end{enumerate}
\end{exercise}
