% Source: Lee, "Introduction to Riemannian Manifolds" (2nd ed.), Chapter 10 "Jacobi Fields", PDF pages 294-328 of the cited source.
...
\uses{ex:warped-products,UNRESOLVED:chA:2-4}
\[
...
\begin{proof}
For $w\in T_pM$, consider the geodesic variation
\[
\Gamma_w(s,t)=\exp_p\bigl(t(v+sw)\bigr)
\]
and let $J_w$ be its variation field along $\gamma$. Differentiating at the two
ends gives
\[
J_w(0)=0,\qquad D_tJ_w(0)=w,\qquad
J_w(1)=d(\exp_p)_v(w).
\]
Uniqueness for the Jacobi equation shows that every Jacobi field $J$ along
$\gamma$ with $J(0)=0$ is $J_w$ for $w=D_tJ(0)$, and such a field is
nontrivial exactly when $w\ne0$. Thus $d(\exp_p)_v$ has a nonzero kernel
vector exactly when there is a nontrivial Jacobi field vanishing at both
$0$ and $1$, which is precisely the asserted equivalence. \qed
\end{proof}
\end{proposition}
