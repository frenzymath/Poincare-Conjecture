...
The estimates of this subsection are computed in the space-time
$M\times I$, $I=[t_0,t_1]$, rather than in the pullback $c^*g(t)$ of a frozen
metric. This is what makes the time derivatives of the connections of $g(t)$
visible: they contribute terms to $\partial k^2/\partial t$ that a computation
in a frozen metric silently drops.

\begin{notation}[Space-time metric for the curve-shrinking flow]
\label{not:curve-shrinking-spacetime}
\label{spacetimehat}
Let $(M,g(t))$, $t\in I=[t_0,t_1]$, be a Ricci flow on a compact manifold. On
$M\times I$ call the directions along $M$ \emph{horizontal} and the
$I$-direction \emph{vertical}, and let $\hat g$ be the metric on $M\times I$
that is the orthogonal sum of the varying family $g(t)$ of horizontal metrics
and the standard metric on $I$. Write $\nabla$ for the Levi-Civita connection
of $\hat g$, and $\nabla^g$ for the varying family of horizontal connections of
the $g(t)$, an operator applied only to horizontal vector fields. All inner
products are with respect to $\hat g$; on horizontal vector fields these agree
with the moving family $g(t)$. Write $\widehat{\Rm}$ for the Riemann curvature
tensor of $\hat g$ and $\Ric_g$ for the Ricci curvature of $g(t)$.

Given a curve-shrinking flow $c\colon S^1\times I\to M$, define
$$\hat c\colon S^1\times I\to M\times I,\qquad \hat c(x,t)=(c(x,t),t),$$
whose image $\Sigma$ is an immersed surface. Let $X=c_*(\partial_x)$, which is
horizontal, and $\widehat H=\hat c_*(\partial_t)$. These are coordinate vector
fields on the domain surface and hence commute. The horizontal component of
$\widehat H$ is the curvature vector $H=\nabla^g_SS$ and its vertical component
is $\partial_t$, so that $\widehat H=H+\partial_t$ and $k=|H|$.
\end{notation}
...
Let $\widehat\Gamma$ be the Christoffel symbols of $\hat g$ and $\Gamma$ those
of the moving family $g(t)$. Index horizontal coordinate directions by
$i,j,k,\dots$ and the direction $\partial_t$ by $0$. Then
\begin{align*}
\widehat \Gamma^k_{ij} & = \Gamma^k_{ij}, \\
\widehat \Gamma^k_{i0} = \widehat \Gamma^k_{0i} & = -g^{k\ell}(\Ric_g)_{i\ell}, \\
\widehat \Gamma^0_{ij} & = (\Ric_g)_{ij},
\end{align*}
and all other Christoffel symbols of $\hat g$ vanish. Consequently, for
horizontal vector fields $A,B$,
\begin{equation}\label{hatEq1}
\nabla_A(B)=\nabla^g_A(B)+\Ric_g(A,B)\partial_t,
\end{equation}
...
\begin{proof}
\uses{not:curve-shrinking-spacetime}
The metric $\hat g$ has $\hat g_{ij}=g_{ij}(t)$, $\hat g_{00}=1$ and
$\hat g_{i0}=0$, so $\partial_0\hat g_{ij}=\partial g_{ij}/\partial t=-2(\Ric_g)_{ij}$
by the Ricci flow equation, while $\partial_i \hat g_{00}=\partial_i\hat g_{j0}=0$.
The Koszul formula
$$\widehat\Gamma^c_{ab}=\tfrac12\hat g^{cd}\left(\partial_a\hat g_{bd}+\partial_b\hat g_{ad}-\partial_d\hat g_{ab}\right)$$
gives the three displayed identities on substituting these derivatives, and
every remaining symbol has a vanishing numerator. Equation~\eqref{hatEq1} is the
statement that the horizontal part of $\nabla_AB$ is $\nabla^g_AB$ (from
$\widehat\Gamma^k_{ij}=\Gamma^k_{ij}$) and its vertical part is
$\widehat\Gamma^0_{ij}A^iB^j\partial_t=\Ric_g(A,B)\partial_t$.
Equation~\eqref{hatEq2} follows from
$\widehat\Gamma^k_{i0}=-g^{k\ell}(\Ric_g)_{i\ell}$, which also shows
$\nabla_A(\partial_t)$ has no vertical component.
\end{proof}
...
$c=c(x,t)$ is a solution to the curve-shrinking flow, with the space-time
notation of Notation~\ref{not:curve-shrinking-spacetime}. We define
$S=|X|^{-1}X$, the unit vector in the $x$-direction measured in the evolving
metric. Then,
$$\widehat H(|X|^2)(x,t)=-2\Ric_{g(t)}(X(x,t),X(x,t))-2k^2|X(x,t)|^2,$$
and
$$[\widehat H,S](x,t)=\left(k^2+\Ric_{g(t)}(S(x,t),S(x,t))\right)S(x,t).$$
\end{lemma}


\begin{proof}
\uses{lem:spacetime-christoffel}
Since $\widehat H$ and $X$ commute,
$$\widehat H(|X|^2)=2\langle\nabla_{\widehat H}X,X\rangle=2\langle\nabla_X\widehat H,X\rangle
=2\langle\nabla_X\partial_t,X\rangle+2\langle\nabla_XH,X\rangle,$$
using $\widehat H=H+\partial_t$. By Equation~\eqref{hatEq2},
$$2\langle\nabla_X\partial_t,X\rangle=-2\Ric_g(X,X).$$
For the second term, $X$ and $H$ are orthogonal, so with $X=|X|S$,
\begin{align*}
2\langle\nabla_XH,X\rangle & = -2\langle H,\nabla_XX\rangle \\
 & = -2|X|^2\langle H,\nabla_SS\rangle-2\langle H,\nabla_X(|X|)S\rangle \\
 & = -2k^2|X|^2,
\end{align*}
the last step because $\langle H,S\rangle=0$ and, by
Equation~\eqref{hatEq1}, $\langle H,\nabla_SS\rangle=\langle H,H+\Ric_g(S,S)\partial_t\rangle=k^2$.
This proves the first formula.

For the second, since $X$ and $\widehat H$ commute,
$$[\widehat H,S]=[\widehat H,|X|^{-1}X]=\widehat H\left((|X|^2)^{-1/2}\right)X
=\frac{-1}{2\left(|X|^2\right)^{3/2}}\widehat H(|X|^2)X.$$
Substituting the first formula gives
$$[\widehat H,S](x,t)=\left(k^2+\Ric_{g(t)}(S(x,t),S(x,t))\right)S(x,t).$$
\end{proof}
...
\uses{not:curve-shrinking-spacetime,lem:spacetime-christoffel}
\source{morgan-tian-correction}
Let $s$ denote an arc-length parameter on $\hat c(x,t)$, and denote derivatives
with respect to $s$ by $'$. Then
\begin{align*}
\frac{\partial }{\partial t}k^2 = {} & (k^2)''-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle+2k^4 \\
 & -2\Ric_g(H,H)+4k^2\Ric_g(S,S)+2\widehat{\Rm}(\widehat H,S,H,S) \\
 & +2\Ric_g(S,S)\Ric_g(H,H)-2(\nabla^g_S(\Ric_g))(S,H),
\end{align*}
where the superscript $\perp$ means the component orthogonal to $X$.
\end{lemma}

\begin{proof}
\uses{lem:curve-shrinking-first-variation,lem:spacetime-christoffel}
First, $\frac{\partial}{\partial t}k^2=\widehat H(\langle H,H\rangle)$. By
Equation~\eqref{hatEq1} we have $\nabla_SS=H+\Ric_g(S,S)\partial_t$, so
\begin{align*}
\frac{\partial}{\partial t}k^2 = {} & 2\langle\nabla_{\widehat H}\nabla_SS,H\rangle-2\langle\nabla_{\widehat H}(\Ric_g(S,S)\partial_t),H\rangle \\
 = {} & 2\langle\nabla_S\nabla_{\widehat H}S,H\rangle+2\langle\nabla_{[\widehat H,S]}S,H\rangle+2\widehat{\Rm}(\widehat H,S,H,S) \\
 & -2\langle\nabla_{\widehat H}(\Ric_g(S,S)\partial_t),H\rangle \\
 = {} & 2\langle\nabla_S\nabla_S\widehat H,H\rangle+2\langle\nabla_S[\widehat H,S],H\rangle+2\langle\nabla_{[\widehat H,S]}S,H\rangle \\
 & +2\widehat{\Rm}(\widehat H,S,H,S)-2\langle\nabla_{\widehat H}(\Ric_g(S,S)\partial_t),H\rangle,
\end{align*}
using that $\widehat H$ and $S$ commute with $X$ and the definition of
$\widehat{\Rm}$.

Consider $\langle\nabla_{\widehat H}(\Ric_g(S,S)\partial_t),H\rangle$. Since
$\langle\partial_t,H\rangle=0$, this equals
$\Ric_g(S,S)\langle\nabla_{\widehat H}\partial_t,H\rangle$; since
$\nabla_{\partial_t}\partial_t=0$, Equation~\eqref{hatEq2} gives
$$\langle\nabla_{\widehat H}(\Ric_g(S,S)\partial_t),H\rangle=\Ric_g(S,S)\langle\nabla_H\partial_t,H\rangle=-\Ric_g(S,S)\Ric_g(H,H).$$

Break the first term into
\begin{equation}\label{hatEq3}
2\langle\nabla_S\nabla_S\widehat H,H\rangle=2\langle\nabla_S\nabla_S\partial_t,H\rangle+2\langle\nabla_S\nabla_SH,H\rangle.
\end{equation}
By Equation~\eqref{hatEq2}, $\nabla_S(\partial_t)$ is a horizontal vector field
along the surface whose inner product with any horizontal $Z$ is $-\Ric_g(S,Z)$.
Hence $\nabla^g_S(\nabla_S(\partial_t))$ is horizontal with
$$\langle\nabla^g_S(\nabla_S(\partial_t)),Z\rangle=-(\nabla^g_S(\Ric_g))(S,Z)-\Ric_g(\nabla^g_SS,Z)=-(\nabla^g_S(\Ric_g))(S,Z)-\Ric_g(H,Z),$$
and, both fields being horizontal,
$\langle\nabla_S(\nabla_S(\partial_t)),Z\rangle=\langle\nabla^g_S(\nabla_S(\partial_t)),Z\rangle$.
Taking $Z=H$,
$$2\langle\nabla_S\nabla_S\partial_t,H\rangle=-2(\nabla^g_S(\Ric_g))(S,H)-2\Ric_g(H,H).$$

The second term of Equation~\eqref{hatEq3} is
$S(S(|H|^2))-2\langle\nabla_SH,\nabla_SH\rangle$. Since $S$ is a unit vector,
$\nabla_SH=(\nabla_SH)^\perp+\langle\nabla_SH,S\rangle S$, so
$$-2\langle\nabla_SH,\nabla_SH\rangle=-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle-2\left(\langle\nabla_SH,S\rangle\right)^2.$$
As $S$ and $H$ are orthogonal, Equation~\eqref{hatEq1} gives
$$\langle\nabla_SH,S\rangle=-\langle H,\nabla_SS\rangle=-\langle H,H+\Ric_g(S,S)\partial_t\rangle=-\langle H,H\rangle=-k^2,$$
whence
$$-2\langle\nabla_SH,\nabla_SH\rangle=-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle-2k^4.$$

Finally, substituting $[\widehat H,S]=(k^2+\Ric_g(S,S))S$ from
Lemma~\ref{lem:curve-shrinking-first-variation},
$$2\langle\nabla_{(k^2+\Ric_g(S,S))S}S,H\rangle=2\left(k^2+\Ric_g(S,S)\right)\langle H,H\rangle=2\left(k^2+\Ric_g(S,S)\right)k^2,$$
and, $S$ and $H$ being perpendicular,
$$2\langle\nabla_S\left((k^2+\Ric_g(S,S))S\right),H\rangle=2\left(k^2+\Ric_g(S,S)\right)k^2.$$
Since $S(S(|H|^2))=(k^2)''$, assembling these establishes the formula.
\end{proof}

\begin{remark}[The book's Lemma 19.7 is not correct]
\label{rem:curve-shrinking-evolution-erratum}
\source{morgan-tian-correction}
The computation of $\partial k^2/\partial t$ carried out in the pullback
$c^*g(t)$ of a frozen metric omits the terms arising from the time derivative of
the connections of $g(t)$. It yields the formula of
Lemma~\ref{lem:curve-shrinking-curvature-evolution} without its last two terms,
and with $\Rm(H,S,H,S)$ in place of $2\widehat{\Rm}(\widehat H,S,H,S)$ — that is,
the curvature of $g$ rather than of $\hat g$, evaluated on $H$ rather than
$\widehat H$. The two extra terms are what force the linear-in-$k$ term in
Corollary~\ref{cor:curve-shrinking-curvature-inequality-bound} below, and hence
the dependence on the initial length in
Corollary~\ref{cor:curve-shrinking-length-curvature-exponential-bound}.
\end{remark}
...
\source{morgan-tian-correction}
There is a constant $\widehat C<\infty$ depending only on the ambient Ricci flow
$(M,g(t))$, $t_0\le t\le t_1$ — in fact only on an upper bound for its curvature
and the first covariant derivatives thereof — such that
$$\frac{\partial }{\partial t}k^2\le
(k^2)''-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle+2k^4+\widehat C(k^2+k),$$
where the primes refer to the derivative with respect to arc length along the
curve.
\end{corollary}

\begin{proof}
\uses{lem:curve-shrinking-curvature-evolution}
Bound the last five terms of Lemma~\ref{lem:curve-shrinking-curvature-evolution}
in turn. Since $|H|=k$ and $|S|=1$, and writing $\Lambda$ for a bound on the
curvature of the ambient flow and its first covariant derivatives:
$|2\Ric_g(H,H)|\le C\Lambda k^2$; $|4k^2\Ric_g(S,S)|\le C\Lambda k^2$;
$|2\widehat{\Rm}(\widehat H,S,H,S)|\le C\Lambda(k^2+k)$, because
$\widehat H=H+\partial_t$ contributes both a term with two factors of $H$ and a
term with one; $|2\Ric_g(S,S)\Ric_g(H,H)|\le C\Lambda^2k^2$; and
$|2(\nabla^g_S(\Ric_g))(S,H)|\le C\Lambda k$. Each is therefore at most a
constant depending only on the ambient flow times $k^2+k$; taking $\widehat C$
to be the sum of these constants gives the claim.
\end{proof}

Because $k$ is not smooth where it vanishes, the differential inequality for the
curvature itself is stated for the regularization
$h=h_\epsilon=\sqrt{k^2+\epsilon^2}$, $\epsilon>0$.
...
\source{morgan-tian-correction}
 There is a constant $C_1<\infty$ depending only on the ambient Ricci flow
$(M,g(t)),\ t_0\le t\le t_1$, such that for every $\epsilon>0$ the function
$h=h_\epsilon=\sqrt{k^2+\epsilon^2}$ satisfies
$$\frac{\partial}{\partial t} h\le h''+k^3+C_1(h+1).$$
\end{claim}

\begin{proof}
\uses{cor:curve-shrinking-curvature-inequality-bound}
Set $C_1=\widehat C/2$ with $\widehat C$ as in
Corollary~\ref{cor:curve-shrinking-curvature-inequality-bound}. Since
$h^2=k^2+\epsilon^2$ differs from $k^2$ by a constant, every derivative of $h^2$
equals the corresponding derivative of $k^2$, so
Corollary~\ref{cor:curve-shrinking-curvature-inequality-bound} gives
$$\frac{\partial (h^2)}{\partial t}\le (h^2)''-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle+2k^4+\widehat C(k^2+k)
= 2(h')^2+2hh''-2\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle+2k^4+\widehat C(k^2+k).$$
Now $(h^2)'=(k^2)'=2\langle\nabla_SH,H\rangle$, and since $H$ is perpendicular
to $S$ this is $2\langle(\nabla_SH)^\perp,H\rangle$; hence
$h'=\langle(\nabla_SH)^\perp,H\rangle/h$. Since $|H|^2=k^2<h^2$, the
Cauchy--Schwarz inequality gives
$$(h')^2\le\frac{\langle(\nabla_SH)^\perp,H\rangle^2}{h^2}\le\langle(\nabla_SH)^\perp,(\nabla_SH)^\perp\rangle.$$
Substituting, the $(h')^2$ term is absorbed and
$$2h\frac{\partial h}{\partial t}=\frac{\partial (h^2)}{\partial t}\le 2hh''+2k^4+\widehat C(k^2+k).$$
Dividing by $2h$ and using $0\le k<h$ — so that $k^4/h\le k^3$ and
$(k^2+k)/h\le h+1$ — yields
$$\frac{\partial h}{\partial t}\le h''+k^3+C_1(h+1).$$
\end{proof}
...
\source{morgan-tian-correction}
Let $C_2<\infty$ be an upper bound for $|\Ric_g|$ over $M\times I$ and let $C_1$
be the constant of Claim~\ref{claim:curve-shrinking-curvature-inequality}. Then
\begin{equation}\label{Lformula}
\frac{d}{dt}L\le \int (C_2-k^2)ds\end{equation} and
$$\frac{d}{dt}\Theta\le (C_1+C_2)\Theta+C_1L.$$
\end{lemma}

\begin{proof}
\uses{lem:curve-shrinking-first-variation,claim:curve-shrinking-curvature-inequality}
$$\frac{d}{dt}L=\int\frac{\partial}{\partial t}\sqrt{|X|^2}dx.$$
By Lemma~\ref{lem:curve-shrinking-first-variation} we have $$\frac{d}{dt}L=\int
\frac{1}{2|X|}\left(-2\Ric_g(X,X)-2k^2|X|^2\right)dx.$$ Thus,
\begin{equation}\label{dLdteqn}\frac{d}{dt}L=\int (-\Ric_g(S,S)-k^2)|X|dx=\int(-\Ric_g(S,S)-k^2)ds,\end{equation}
and the first inequality follows since $|\Ric_g(S,S)|\le C_2$.

For the second inequality, consider the $\epsilon$-modified total curvature
$$\Theta_\epsilon(t)=\int h_\epsilon(x,t)\,ds,$$
so that
$$\frac{d}{dt}\Theta_\epsilon=\int\frac{\partial}{\partial t}(h_\epsilon|X|)dx
=\int\left(\frac{\partial h_\epsilon}{\partial t}|X|+h_\epsilon\frac{\partial |X|}{\partial t}\right)dx.$$
Using Claim~\ref{claim:curve-shrinking-curvature-inequality} and the first
equation in Lemma~\ref{lem:curve-shrinking-first-variation},
\begin{align*}
\frac{d}{dt}\Theta_\epsilon & \le \int\left(h_\epsilon''+k^3+C_1(h_\epsilon+1)\right)ds
+\int\frac{h_\epsilon}{2|X|}\left(-2\Ric_g(X,X)-2k^2|X|^2\right)dx \\
& = \int\left(h_\epsilon''+k^3+C_1(h_\epsilon+1)\right)ds-\int h_\epsilon\left(\Ric_g(S,S)+k^2\right)ds \\
& \le \int\left(h_\epsilon''+C_1(h_\epsilon+1)-h_\epsilon\Ric_g(S,S)\right)ds,
\end{align*}
the last step because $k^3\le h_\epsilon k^2$, so that $k^3-h_\epsilon k^2\le0$.
Since $\int h_\epsilon''ds=0$ by the fundamental theorem of calculus, and
$|\Ric_g(S,S)|\le C_2$, we conclude
$$\frac{d}{dt}\Theta_\epsilon(t)\le (C_1+C_2)\Theta_\epsilon(t)+C_1L(t).$$
Letting $\epsilon\to0$ gives the second statement.
\end{proof}
...
\source{morgan-tian-correction}
Let $C_1,C_2$ be as in the previous lemma. Let $c(x,t)$ be a curve-shrinking
flow, let $L(t)$ be the total length of $c(t)$ and let $\Theta(t)$ be the total
curvature of $c(t)$. Then for any $t_0\le t'<t''\le t_1$ we have
$$L(t'')\le L(t')e^{C_2(t''-t')}$$
$$\Theta(t'')+L(t'')\le \left(\Theta(t')+L(t')\right)e^{(C_1+C_2)(t''-t')},$$
and in particular $\Theta(t'')\le\left(\Theta(t')+L(t')\right)e^{(C_1+C_2)(t''-t')}$.
\end{corollary}

\begin{proof}
\uses{lem:curve-shrinking-length-curvature-growth}
By Inequality~\eqref{Lformula} and $k^2\ge0$ we have $dL/dt\le C_2L$, and
Gronwall's inequality gives the first bound. Adding the two inequalities of
Lemma~\ref{lem:curve-shrinking-length-curvature-growth},
$$\frac{d}{dt}(\Theta+L)\le (C_1+C_2)\Theta+C_1L+C_2L\le (C_1+C_2)(\Theta+L),$$
and Gronwall's inequality gives the second. The last claim follows since
...
\source{morgan-tian-correction}
Corollary~\ref{cor:curve-shrinking-length-curvature-exponential-bound} does not
bound $\Theta(t'')$ in terms of $\Theta(t')$ alone: the term $C_1L$ in
Lemma~\ref{lem:curve-shrinking-length-curvature-growth}, which descends from the
linear-in-$k$ term of
Corollary~\ref{cor:curve-shrinking-curvature-inequality-bound}, contributes a
constant to the integrand and so brings in the initial length. This is enough for
the applications below, which are made to a compact family of curve-shrinking
flows of $C^2$-curves and hence carry uniform bounds on both total curvature and
...
Fix $\epsilon>0$ and set $h=h_\epsilon=\sqrt{k^2+\epsilon^2}$. Using
Claim~\ref{claim:curve-shrinking-curvature-inequality} and Claim~\ref{claim:ramp-function-evolution} we compute
\begin{eqnarray*} \frac{\partial}{\partial t}\left(\frac{h}{u}\right)
& = &\frac{1}{u}\frac{\partial
h}{\partial t}-\frac{h}{u^2}\frac{\partial u}{\partial t} \\
& \le & \frac{h''+k^3+C_1(h+1)}{u}-\frac{h}{u^2}\left(u''+(k^2+\Ric(S,S))u\right)
\\
& = & \frac{h''}{u}-\frac{hu''}{u^2}+\frac{k^3-hk^2}{u}+\frac{C_1(h+1)}{u}-\frac{h}{u}\Ric(S,S) \\
& \le & \frac{h''}{u}-\frac{hu''}{u^2}+\frac{C_1(h+1)}{u}-\frac{h}{u}\Ric(S,S),
\end{eqnarray*}
the last step because $k\le h$ gives $k^3-hk^2\le0$.
On the other hand,
$$\left(\frac{h}{u}\right)''=\frac{h''u-u''h}{u^2}-2\left(\frac{u'}{u}\right)
\left(\frac{h'u-u'h}{u^2}\right).$$ Plugging this in, and using the
curvature bound on the ambient manifolds we get
$$\frac{\partial}{\partial
t}\left(\frac{h}{u}\right)\le
\left(\frac{h}{u}\right)''+\left(\frac{2u'}{u}\right)\left(\frac{h}{u}\right)'+C'\left(\frac{h+1}{u}\right),$$
...
principle argument shows that the maximum of $h/u$ at time $t$ grows
...
interval, the value of $h$ — and hence of $k\le h$ — is bounded,
...
\uses{prop:curve-deformation-existence}
\source{morgan-tian-correction}
Let $(M,g(t)),\ t_0\le t\le t_1$, be a Ricci flow, and fix $\Theta_0<\infty$ and
$L_0<\infty$. Then there exist
constants $\delta>0$ and $\widetilde C_i<\infty$ for
$i=0,1,2,\ldots$, depending only on $t_1-t_0$, a bound for the
norm of the curvature of the Ricci flow, and $\Theta_0$ and $L_0$, such that the
following
holds. Let $c(x,t)$ be a curve-shrinking flow that is an immersion
for each $t$ whose total curvature and length at time $t_0$ are at most
$\Theta_0$ and $L_0$ respectively. Suppose that at a time $t'$ for some $0<r<1$ such that
...
\source{morgan-tian-correction}
The dependence of $\delta$ and the $\widetilde C_i$ on $\Theta_0$ and $L_0$ is
necessary, and is inherited from
Remark~\ref{rem:total-curvature-needs-initial-length}: the estimates rest on the
growth bound of
Corollary~\ref{cor:curve-shrinking-length-curvature-exponential-bound}, which
controls total curvature only in terms of both the initial total curvature and
the initial length. This costs nothing in the applications made below. For a map
$\Gamma\colon S^2\to\Lambda M$ as in
Proposition~\ref{prop:curve-deformation-existence} and any $0<\lambda<1$, the
associated family $\widetilde\Gamma(c)^\lambda$, $c\in S^2$, has total curvature
and length bounds at the initial time depending only on $\Gamma$; so
Lemma~\ref{lem:curve-shrinking-shi-estimate} applies with a single pair
$\Theta_0,L_0$, and hence with uniform constants, across all of them.
\end{remark}
