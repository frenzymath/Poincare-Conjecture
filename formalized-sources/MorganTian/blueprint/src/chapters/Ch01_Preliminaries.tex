...
\lean{MorganTianLib.metricExpBallDiffeomorph, MorganTianLib.metricBookInjectivityRadius}
...
\begin{lemma}[The abstract model polar volume]
...
Write $\rho_k(v)=\bigl(\sn_k(|v|)/|v|\bigr)^{n-1}$ for the comparison
density on the finite-dimensional chart $E$, and set
$$\mathcal V_k(r):=\int_{B(0,r)}\rho_k\,d\mu.$$
Then
$$\mathcal V_k(r)=\omega_{n-1}\int_0^r\sn_k^{n-1}(t)\,dt,
\qquad \omega_{n-1}:=\mu_S(S).$$
This is only an abstract Haar chart integral; identifying it with a model-manifold
ball volume via Lemma~\ref{lem:model-polar-isometry} remains open.
\end{lemma}
...
the constant $\mu_S(S)=\omega_{n-1}$, proving the chart-integral identity.
\end{proof}

\begin{lemma}[Bishop-Gromov for an abstract ball integral]
...
Let $k\ge0$ and $R>0$. Let $\rho\ge0$ be a measurable density on a finite-dimensional
Euclidean chart $E$, and put $V_\rho(r)=\int_{B(0,r)}\rho\,d\mu$ and
$V_k(r)=\mathcal V_k(r)$. Suppose that in every unit direction
...
$$V_\rho(r_2)\cdot V_k(r_1)\ \le\
V_\rho(r_1)\cdot V_k(r_2),$$
equivalently $r\mapsto V_\rho(r)/V_k(r)$ is non-increasing on $(0,R)$.
No model-manifold identification is used.
\end{lemma}
...
\leanok
Apply the radial inequality in each direction and integrate over the sphere. Polar
coordinates give $V_\rho$ and $V_k$; the ratio lemma gives their quotient's monotonicity.
\end{proof}
...
\lean{MorganTianLib.metricRescalingLaws}
\leanok
