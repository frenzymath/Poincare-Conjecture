...
\begin{proof}
  \leanok
  Tracing the sectional-curvature inequality over an orthonormal basis whose
  first vector is a prescribed unit vector gives
  $\operatorname{Ric}\ge(n-1)k$. The diameter and compactness clauses of Myers's
  theorem then apply.
  \source{petersen:page-0267}
\end{proof}
...
\begin{proof}
  \leanok
  The summed sine-field second variation of
  \cref{lem:pet-ch6-myers-index} rules out every distance-minimizing geodesic
  of length greater than $\pi/\sqrt k$. Hopf--Rinow supplies a minimizing
  geodesic between any two points, and therefore gives the diameter bound.
  The manifold is then complete and bounded, hence compact by Hopf--Rinow.
  \source{petersen:page-0267,petersen:page-0268}
\end{proof}
...
\begin{proof}
  \leanok
  By \cref{thm:pet-ch6-myers-ricci-geometric}, $\widetilde M$ has the stated
  diameter bound and is compact. Its continuous surjective image $M$ is
  compact. For each $y\in M$, monodromy at a point of $p^{-1}(y)$ embeds
  $\pi_1(M,y)$ into that fiber, because $\widetilde M$ is simply connected.
  The fiber is a closed discrete subspace of the compact space $\widetilde M$,
  hence finite.
\end{proof}
