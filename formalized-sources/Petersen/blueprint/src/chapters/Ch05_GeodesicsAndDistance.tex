...
\begin{remark}[Remark 5.2.1 — distance functions produce geodesics]
  ...
  \lean{PetersenLib.distanceFunctionIntegralCurvesAreGeodesics,
        PetersenLib.distanceFunctionIntegralCurvesAreGeodesicsOn}
  \leanok
  \uses{def:pet-ch5-geodesic,lem:pet-ch5-distance-function-segments,
        thm:pet-ch5-energy-minima-geodesics}
...
\begin{proof}
  ...
  $\varphi(U)\times\mathbb R^n$. Picard--Lindel\"of with parameters gives
  radii $r,T,\varepsilon_0>0$ with $T<\varepsilon_0$ and a local flow $Z(z,t)$,
  defined for $|t|\le\varepsilon_0$ and all initial conditions $z$ in the ball
  of radius $r$ about the \emph{zero section} point $z_0=(\varphi(p),0)$, whose
  restriction $z\mapsto Z(z,\cdot)|_{[0,T]}$ is $C^\infty$ as a map into
  $C([0,T],\mathbb R^n\times\mathbb R^n)$.

  The velocity $v$ need not lie in that ball, and the flow is smooth in the
  initial condition rather than jointly with time. Both points are handled by
  the degree-2 fibre homogeneity of $F$, i.e.\ $F(x,aw)=(aw,-a^2\Gamma(w,w)(x))$:
  the geodesic from $q$ with velocity $w$ run for time $t$ coincides with the
  geodesic from $q$ with velocity $\tfrac tT w$ run for the \emph{fixed} time
  $T$,
  \[
    Z\big((\varphi(q),\tfrac tT w),T\big)_1
      = Z\big((\varphi(q),\lambda w),\tfrac t\lambda\big)_1 ,
  \]
  for any $\lambda>0$ with $\lambda w$ and $\tfrac tT w$ small. Since
  $\varepsilon$ may be chosen freely, fix $\lambda>0$ with
  $\lambda(\lVert v\rVert+1)$ below both $r$ and the homogeneity threshold, and
  set $V_2=B(v,1)$, $V_1=\varphi^{-1}(B(\varphi(p),\nu))$ and
  $\varepsilon=T\lambda$. Then every rescaled initial condition
  $(\varphi(q),\tfrac tT w)$, for $q\in V_1$, $w\in V_2$,
  $|t|<\varepsilon$, lies in the ball of radius $r$ about $z_0$, so
  \[
    c_{q,w}(t) := \varphi^{-1}\Big(Z\big((\varphi(q),\tfrac tT w),T\big)_1\Big)
  \]
  is defined. By the displayed identity its coordinate representation is the
  time-rescaled trajectory $Z((\varphi(q),\lambda w),\cdot/\lambda)_1$, whence
  $c_{q,w}$ solves the geodesic equation, $c_{q,w}(0)=q$ (the spray vanishes on
  the zero section, so $Z((\varphi(q),0),T)=(\varphi(q),0)$) and
  $\dot c_{q,w}(0)=\lambda^{-1}\cdot\lambda w=w$.

  Finally $t$ now enters only through the velocity slot of an evaluation at the
  fixed time $T$: the map $(x,w,t)\mapsto(x,\tfrac tT w)$ is smooth, evaluation
  at $T$ is a continuous linear map on $C([0,T],\mathbb R^n\times\mathbb R^n)$,
  and $z\mapsto Z(z,\cdot)|_{[0,T]}$ is $C^\infty$ on the ball; composing the
  three makes $(q,w,t)\mapsto c_{q,w}(t)$ jointly $C^\infty$ on
  $V_1\times V_2\times(-\varepsilon,\varepsilon)$. No joint-in-time smoothness
...
\begin{proof}
  \leanok
  \uses{lem:pet-ch5-distance-function-length-bound}
  Since $|\nabla r|=1$, Cauchy–Schwarz gives, for every $t\in[a,b]$,
  \[
    (r\circ c)'(t)=dr(\dot c(t))=g(\nabla r|_{c(t)},\dot c(t))\le|\dot c(t)|,
  \]
  with equality if and only if $\dot c(t)$ is a nonnegative multiple of
  $\nabla r|_{c(t)}$, in which case the multiple is forced to be $|\dot c(t)|$ by
  comparing norms, i.e.\ $\dot c(t)=|\dot c(t)|\,\nabla r|_{c(t)}$.

  Integrating the displayed inequality over $[a,b]$ and using the fundamental
  theorem of calculus,
  \[
    r(c(b))-r(c(a))=\int_a^b(r\circ c)'\,dt\le\int_a^b|\dot c|\,dt=L(c)|_a^b,
  \]
  and the hypothesis says the two ends are equal. Both integrands are continuous
  and the pointwise inequality $(r\circ c)'\le|\dot c|$ holds on all of $[a,b]$,
  so equality of the integrals over the nondegenerate interval $[a,b]$ forces the
  integrands to agree at every point: $dr(\dot c(t))=|\dot c(t)|$ for all
  $t\in[a,b]$. Thus the Cauchy–Schwarz equality case holds at every $t$, which is
  the claim.
  \source{petersen:page-0195}
\end{proof}
...
\begin{remark}[a minimiser is a reparametrised integral curve of $\nabla r$]
  ...
  \emph{Formalization status.} The Lean statement hypothesises $\sigma$, exactly
  as stated here; the existence of an integral curve of $\nabla r$ on $[0,L]$ is a
  separate standard fact and is not proved here. The node is not
  \texttt{\textbackslash leanok} for a second reason: the Lean carries an
  undischarged side condition asserting that $\nabla r$ is a $C^1$ section of $TM$
  over $U$. That is mathematically automatic — bump-extend $r$ near any point of
  $U$ to a globally smooth function and use that gradients agree near that point —
  but the on-disk smooth-gradient lemma requires $r$ to be smooth on all of $M$,
  so the hypothesis has not been discharged.
  \source{petersen:page-0195}
\end{remark}
\begin{proof}
  \uses{rem:pet-ch5-distance-function-rigidity}
  By \cref{rem:pet-ch5-distance-function-rigidity},
  $\dot c(s)=|\dot c(s)|\,\nabla r|_{c(s)}=\varphi'(s)\,\nabla r|_{c(s)}$ for every
  $s\in[a,b]$, since $\varphi'=|\dot c|$ by the fundamental theorem of calculus.
  On the other hand $\varphi(a)=0$ and $\varphi(b)=L$, so $\sigma\circ\varphi$ is
  defined on $[a,b]$, and by the chain rule
  \[
    (\sigma\circ\varphi)'(s)=\varphi'(s)\,\dot\sigma(\varphi(s))
      =\varphi'(s)\,\nabla r|_{\sigma(\varphi(s))}.
  \]
  Hence $c$ and $\sigma\circ\varphi$ both solve the non-autonomous equation
  $y'(s)=\varphi'(s)\,\nabla r|_{y(s)}$ on $[a,b]$, and they agree at $s=a$:
  $\sigma(\varphi(a))=\sigma(0)=c(a)$. The field $\nabla r$ is $C^1$ on $U$, hence
  locally Lipschitz, and $\varphi'=|\dot c|$ is continuous, so solutions of this
  equation agreeing at one point of $[a,b]$ agree throughout $[a,b]$ by Grönwall's
  inequality. Therefore $c=\sigma\circ\varphi$ on $[a,b]$.

  Note that $\varphi$ need not be strictly monotone — $c$ may rest on an interval
  and still realise the bound — so the conclusion cannot be obtained by inverting
  an arclength parametrisation; the uniqueness argument above avoids this.
  \source{petersen:page-0195}
\end{proof}
...
\begin{proof}
  \leanok
  \uses{cor:pet-ch5-regular-curve-approx}
  The constant-speed competitors form a subset of the competitors defining
  $|pq|$, so $|pq|\le\inf\{L(\gamma)\mid\gamma\in\Omega_{p,q}\text{ of constant
  speed}\}$. Conversely, let $c\in\Omega_{p,q}$ and $\epsilon>0$. By
  \cref{cor:pet-ch5-regular-curve-approx} there is a constant-speed
  $\bar c\in\Omega_{p,q}$ with $L(\bar c)\le(1+\epsilon)L(c)$, so the restricted
  infimum is at most $(1+\epsilon)L(c)$. Letting $\epsilon\to0$ gives that the
  restricted infimum is at most $L(c)$, and taking the infimum over
  $c\in\Omega_{p,q}$ gives the reverse inequality. Both competitor sets are empty
  simultaneously, so the degenerate case needs no separate hypothesis.
  \source{petersen:page-0198}
\end{proof}
...
\begin{lemma}[Prop. 5.5.1(1) — $D\exp_p$ is nonsingular at the origin]
...
\begin{proof}
  \leanok
  \uses{def:pet-ch5-exponential-map}
  By homogeneity $\exp_p(tv)=c_v(t)$, so the chart reading of $\exp_p$ has strict
  Fréchet derivative $\mathrm{id}$ at $0$; the strict-derivative form of the inverse
  function theorem gives injectivity on a small ball and the filter-pushforward
  identity.
  \source{petersen:page-0205}
\end{proof}
...
\begin{proof}
  \leanok
  \uses{def:pet-ch5-injectivity-radius,lem:pet-ch5-exp-differential-nonsingular}
  By \cref{lem:pet-ch5-exp-differential-nonsingular} there is $\rho>0$ with
  $\exp_p$ defined and injective on the model-norm ball $\{w:\|w\|<\rho\}$. Fibre
  coercivity at $p$ gives $c>0$ with $\|v\|\le\sqrt c\,|v|_g$ for all $v\in T_pM$
  (the coordinate norm is dominated by the Riemannian norm, since at the pole the
  chart-Gram form is the intrinsic inner product $g_p$). Setting
  $\varepsilon=\rho/(\sqrt c+1)>0$, any $v$ with $|v|_g<\varepsilon$ satisfies
  $\|v\|\le\sqrt c\,|v|_g<\sqrt c\,\varepsilon<\rho$, so the whole $g$-metric ball
  $\{v:|v|_g<\varepsilon\}$ lies inside the model-norm ball where $\exp_p$ is defined
  and injective. Hence $\varepsilon$ belongs to the set whose supremum is
  $\operatorname{inj}_p$, and $\varepsilon>0$ gives $\operatorname{inj}_p>0$.
  \source{petersen:page-0206}
\end{proof}
...
\begin{theorem}[Thm. 5.5.4 — short geodesics are segments]
  ...
  \uses{def:pet-ch5-exponential-map,def:pet-ch5-segment,
        lem:pet-ch5-short-geodesics-segments-normal}
...
\begin{proof}
  ...
  This is the equality clause of the do Carmo minimizing-geodesic estimate. On a
  normal ball, read in exponential coordinates, the Gauss Lemma
  (\cref{lem:pet-ch5-gauss-lemma}) gives $|\dot\sigma|\ge dr(\dot\sigma)$ with
  equality iff $\dot\sigma$ is radial; a competitor realizing the radial length
  must therefore have $\dot\sigma$ everywhere proportional to $\partial_r$, i.e.
  $\sigma$ traces the radial ray $\exp_p(s\,v)$ with $s$ the (nondecreasing)
  running fraction of radial length. The escape estimate keeps a length-realizing
  competitor inside the normal ball, so no confinement hypothesis is needed. The
  $C^\infty$ length hypothesis $L(\sigma)=|v|_g$ is converted to the equal-length
  clause of the vendored engine by the length bridge $\operatorname{pathELength}
  \sigma=\int_0^1|\dot\sigma|_g$, and the chart-origin identity identifies the
  chart-Gram radial length with $|v|_g$.
...
\begin{lemma}[the uniqueness clause for piecewise-smooth competitors]
  ...
  \cref{def:pet-ch5-segment,def:pet-ch5-distance} actually range over. For every
...
\begin{proof}
  ...
  Identical to \cref{lem:pet-ch5-radial-geodesic-unique}: the Gauss-radius
  deficits telescope over the partition, so the do Carmo minimizing-equality
  engine applies verbatim in its piecewise form; no corner-shortening step is
  needed. The only new ingredient is the length bridge
  $\operatorname{pathELength}\sigma=\int|\dot\sigma|_g$ for piecewise-$C^\infty$
  $\sigma$, obtained from the smooth case by telescoping the partition against
  additivity of both sides.
  \source{petersen:page-0208}
\end{proof}

\begin{lemma}[Thm. 5.5.4 on a normal ball — all clauses]
  \label{lem:pet-ch5-short-geodesics-segments-normal}
  \lean{PetersenLib.expMap_isSegment_unique}
  \leanok
  \uses{lem:pet-ch5-radial-geodesic-segment,
        lem:pet-ch5-radial-geodesic-unique-piecewise,
        cor:pet-ch5-metric-ball-image,def:pet-ch5-segment}
...
\begin{proof}
  ...
  Take $\varepsilon$ below the radii supplied by
  \cref{cor:pet-ch5-metric-ball-image}, \cref{lem:pet-ch5-radial-geodesic-segment}
  and \cref{lem:pet-ch5-radial-geodesic-unique-piecewise}, rescaled by the
  one-sided coercivity $\|w\|\le\sqrt c\,|w|_g$ of the chart Gram form at the pole,
  so that the intrinsic hypothesis $|v|_g<\varepsilon$ implies the model-norm
  hypothesis the engine wants.  For (iii), a segment $\sigma$ from $p$ to
  $\exp_p(v)$ has $L(\sigma)|_0^1=|pq|=|v|_g$ by
  \cref{lem:pet-ch5-radial-geodesic-segment}, so its proportional-arclength
  constant is $k=|v|_g$ and $L(\sigma)|_0^t=|v|_g\,t$.  Feeding that into the
  length-fraction clause of the minimizing-equality engine, which gives
  $L(\sigma)|_0^t=s(t)\,|v|_g$, forces $s(t)\,|v|_g=t\,|v|_g$; cancelling
  $|v|_g>0$ gives $s=\mathrm{id}$, i.e. $\sigma(t)=\exp_p(tv)$.  When $v=0$ both
  sides are constant at $p$.
  \source{petersen:page-0207,petersen:page-0208}
\end{proof}
...
\begin{proof}
  \leanok
  \uses{lem:pet-ch5-radial-geodesic-segment,def:pet-ch5-metric-balls}
  ($\subseteq$) For $v$ with $|v|_g<\delta$ the radial geodesic realizes the
  distance, $d(p,\exp_p v)=|v|_g<\delta$
  (\cref{lem:pet-ch5-radial-geodesic-segment}), so $\exp_p v\in B(p,\delta)$; fibre
  coercivity $|v|\le\sqrt c\,|v|_g$ keeps $v$ inside the normal ball where that
  identity holds.  ($\supseteq$) If $q\in B(p,\delta)$ then, using the proven
  forward comparison $\operatorname{edist}(p,q)\le d(p,q)<\delta$ and the escape
  estimate for the normal ball, $q=\exp_p w$ for some $w$; the normal-ball distance
  identity $\operatorname{edist}(p,\exp_p w)=|w|_g$ then forces $|w|_g<\delta$, so
  $q\in\exp_p(\{|v|_g<\delta\})$.  The closed-ball identity is the same argument
  with non-strict inequalities, choosing $\delta$ strictly below the escape radius.
  \source{petersen:page-0209}
\end{proof}
...
\begin{lemma}[Lemma 5.5.7, jet form — exponential coordinates are flat to first order]
...
\begin{proof}
  \leanok
  \uses{rem:pet-ch5-radial-isometry,lem:pet-ch5-exp-differential-nonsingular}
  Write $g_x$ for the metric read in exponential coordinates at the point $x$ of the
  normal ball, so that $g_x(a,b)=g_{ij}(x)a^ib^j$; each $g_x$ is symmetric and
  bilinear.  Since the chart reading of $\exp_p$ is $C^2$ on a ball about the origin
  (\cref{lem:pet-ch5-exp-differential-nonsingular}), $x\mapsto g_x(a,b)$ is $C^1$
  there; in particular the derivatives below exist.  Because $D\exp_p|_0=\mathrm{id}$,
  $g_0=g_p$, which is the first claim.

  The radial isometry (\cref{rem:pet-ch5-radial-isometry}) states $g_x(x,w)=g_p(x,w)$
  for every $x$ in the normal ball and every $w$; this is Petersen's
  $\sum_jg_{ij}x^j=\sum_j\delta_{ij}x^j$.  Substituting $x=tv$ and using bilinearity,
  \[
    t\,g_{tv}(v,w)=g_{tv}(tv,w)=g_p(tv,w)=t\,g_p(v,w),
  \]
  so for $t\neq0$ we may cancel $t$ to get $g_{tv}(v,w)=g_p(v,w)$: the metric
  evaluated on the radial direction $v$ is \emph{constant along the whole ray}
  $t\mapsto tv$.  At $t=0$ the same value obtains, so $t\mapsto g_{tv}(v,w)$ is
  constant near $0$.

  Let $D_u(a,b):=\partial_u\big(x\mapsto g_x(a,b)\big)\big|_0$.  It is additive in
  $u$, and additive in $a$ and symmetric in $(a,b)$ because each $g_x$ is.
  Differentiating the constant $t\mapsto g_{tv}(v,w)$ at $t=0$ gives the diagonal
  identity $D_v(v,w)=0$ for all $v,w$.  Expanding $D_{a+b}(a+b,w)=0$ by additivity
  and cancelling the diagonal terms $D_a(a,w)=D_b(b,w)=0$ yields the antisymmetry
  $D_a(b,w)=-D_b(a,w)$.  Chaining this against the symmetry of $D$ in its last two
  arguments around the three cyclic permutations of $(a,b,c)$,
  \[
    D_a(b,c)=-D_b(a,c)=-D_b(c,a)=D_c(b,a)=D_c(a,b)=-D_a(c,b)=-D_a(b,c),
  \]
  so $2D_a(b,c)=0$ and hence $D_a(b,c)=0$, i.e.\ $\partial_kg_{ij}|_p=0$ for all
  $i,j,k$.  (This is Petersen's three-permutation computation carried out without
...
\begin{lemma}[Lemma 5.5.7 — first-order flatness of exponential coordinates]
  ...
  Not \texttt{\textbackslash leanok}: the \emph{jet} is formalized
  (\cref{lem:pet-ch5-normal-coords-jet}), but the $O(r^2)$ \emph{rate} is not.  The
  jet alone yields only $o(r)$; the rate needs a second-order Taylor bound on
  $g_{ij}$, hence $C^2$ of $g_{ij}$, hence $C^3$ of the chart reading of $\exp_p$.
  The strongest packaged regularity on disk is $C^2$ of chart-$\exp$, which gives
  $C^1$ of $g_{ij}$ — exactly enough for the jet and no more.
  \source{petersen:page-0210}
\end{lemma}
\begin{proof}
  \uses{lem:pet-ch5-normal-coords-jet}
  By \cref{lem:pet-ch5-normal-coords-jet}, $g_{ij}|_p=\delta_{ij}$ and
  $\partial_kg_{ij}|_p=0$.  If moreover $g_{ij}$ is $C^2$ near the origin — the
  hypothesis that is not currently available — Taylor's theorem with second-order
  remainder leaves $g_{ij}=\delta_{ij}+O(r^2)$.  The original basis computation,
  recorded for reference: differentiate $g_{ij}v^j=\delta_{ij}v^j$, i.e.\
  $\sum_jg_{ij}x^j=\sum_j\delta_{ij}x^j$
...
\begin{proof}
  ...
  (2) Let $J$ be the maximal existence domain of $(p,v)$ and $c_{p,v}:J\to M$ the
  maximal geodesic. By (1), $F\circ c_{p,v}$ is a geodesic on $J$, and it has
  initial position $F(p)$ and initial velocity
  $\frac{d}{dt}F(c_{p,v}(t))|_{t=0}=DF_p(v)$ by the chain rule. So $F\circ c_{p,v}$
  realises the initial datum $(F(p),DF_p(v))$ on the open connected set $J\ni0$,
  whence $J$ is contained in the maximal existence domain of $(F(p),DF_p(v))$. By
  uniqueness of geodesics with a given initial datum
  (\cref{lem:pet-ch5-global-uniqueness}), $F\circ c_{p,v}=c_{F(p),DF_p(v)}$ on
  $J$. Since $\exp_p(v)=c_{p,v}(1)$, evaluating at $t=1$ (when $1\in J$, i.e.\ when
  $\exp_p(v)$ is defined) gives $F(\exp_p(v))=\exp_{F(p)}(DF_p(v))$.
...
\begin{proof}
  ...
  \]
  and since $\exp_x$ maps onto a neighborhood of $x$ — apply
  \cref{cor:pet-ch5-uniform-injectivity-radius-compact} to the compact set $\{x\}$,
  which gives a ball about $0\in T_xM$ on which $\exp_x$ is defined with open image
  — a neighborhood of $x$ lies in the set where $F$ and $G$ agree.

  To see that this neighborhood lies in $A$, i.e.\ that the differentials also
  agree there, note that $F$ and $G$ agree on a neighborhood of each of its points,
  and two maps agreeing on a neighborhood of a point have the same differential at
  that point. (This step is where Petersen's argument is terse: agreement of the
  differentials cannot be propagated by continuity of $x\mapsto DF_x$ alone, and is
  instead obtained from local agreement of the maps.) Hence $A$ is open, and by
  connectedness $A=M$.
  \source{petersen:page-0214}
\end{proof}
...
\begin{proof}
  ...
  Let $p\in\operatorname{Fix}(S)$ and $v\in V_p$. Each $F\in S$ fixes $p$ and
  satisfies $DF_p(v)=v$, so $F$ preserves the initial datum $(p,v)$; by part~(2) of
  \cref{prop:pet-ch5-local-isometry-properties} applied to the isometry $F$,
  $F(\exp_p(tv))=\exp_{F(p)}(tDF_p(v))=\exp_p(tv)$ for every $t$ for which
  $\exp_p(tv)$ is defined. Hence $\exp_p(tv)\in\operatorname{Fix}(S)$ for all such
  $t$.

  By \cref{lem:pet-ch5-short-geodesics-segments-normal} there is $\varepsilon>0$
  such that for $|v|_g<\varepsilon$ the ray $t\mapsto\exp_p(tv)$ is defined and
  continuous on $[0,1]$. For such $v\in V_p$ the ray is therefore a continuous path
  in $\operatorname{Fix}(S)$ starting at $p$, so its image lies in the connected
  component of $\operatorname{Fix}(S)$ containing $p$. Thus a neighborhood of
  $0\in V_p$ is carried into that component by $\exp_p$, which is precisely the
  assertion that the component is totally geodesic with tangent distribution $V_p$
  at $p$. As $p$ was an arbitrary point of the component, the claim follows.
...
\begin{proof}
  ...
  Let $p\in\operatorname{Fix}(S)$ and let $V=V_p\subset T_pM$ be the subspace fixed
  by every $DF_p:T_pM\to T_pM$, $F\in S$ (each $F\in S$ fixes $p$, so $DF_p$ acts on
  ...
  \operatorname{Fix}(S)$. This much is
  \cref{prop:pet-ch5-fixed-point-component-totally-geodesic} and already gives that
  each component is totally geodesic; the remainder of the argument upgrades this to
  the submanifold assertion.
...
\begin{proof}
  ...
  The formalization (\texttt{PetersenLib.hopfRinowTheorem}) proves the cycle
  $(1)\Rightarrow(2)\Rightarrow(3)\Rightarrow(4)\Rightarrow(1)$ through the
  vendored do Carmo Ch.~7 geodesic-completeness engine, under the hypothesis
  $g.\mathtt{IsRiemannianDist}$ that the metric-space distance on $M$ is the
  Riemannian distance of $g$: $(2)\Rightarrow(3)$ is the single-point properness
  \texttt{properSpace\_of\_geodesicallyComplete\_at} (do Carmo's geodesic-sphere
  growth induction together with Bolzano–Weierstrass on the initial data of the
  minimizing geodesics), $(3)\Rightarrow(4)$ is the metric fact that a proper
  metric space is complete, and $(4)\Rightarrow(1)$ is
  \texttt{isGeodesicallyComplete\_of\_complete} (a maximal geodesic that
  Cauchy-converges at a finite endpoint is extended past it by the uniform local
  geodesic flow). Petersen's own exposition, via the exponential-image
  identities of \cref{cor:pet-ch5-metric-ball-image} and the corner-turning
  clopen argument, is recorded below.
...
\begin{lemma}[geodesics are admissible piecewise-smooth competitors]
  ...
  \lean{PetersenLib.IsGeodesicOn.isPiecewiseSmoothCurve,
    PetersenLib.IsGeodesic.isPiecewiseSmoothCurve}
  \leanok
  \uses{def:pet-ch5-geodesic,def:pet-ch5-piecewise-smooth-curve,
...
\begin{proof}
  ...
  The claim is local, and a partition is permitted, so it suffices to smooth
  $\gamma$ near each $t_0\in[a,b]$ and assemble finitely many pieces. Near $t_0$,
  uniqueness of geodesics (\cref{thm:pet-ch5-local-uniqueness}) identifies
  $\gamma$ with a radial ray $\tau\mapsto\exp_{\gamma(t_0)}(\tau u)$ of the
  exponential map after an affine reparametrization of time, and the ray is
  $C^\infty$ because $\exp$ is. Each $t_0$ therefore has a closed two-sided
  neighbourhood on which $\gamma$ is $C^\infty$; continuous induction on
  $\{x\in[a,b]\mid\gamma|_{[a,x]}\text{ is piecewise smooth}\}$ (its supremum is
  attained and can be pushed) exhausts $[a,b]$ in finitely many steps.
  \source{petersen:page-0204}
\end{proof}
...
\begin{proof}
  ...
  \cref{thm:pet-ch5-hopf-rinow}: a minimizing geodesic from $p$ to $q$ exists by
  completeness, and it realizes the distance.  To see that it is a \emph{segment}
  in the sense of \cref{def:pet-ch5-segment} one needs it to be an admissible
  piecewise-$C^\infty$ competitor, which is
  \cref{lem:pet-ch5-geodesics-are-piecewise-smooth}; its length then equals
  $|pq|$ by the squeeze $d\le L(\gamma)$ (the infimum bound) against
  $L(\gamma)=|pq|$ read off the local exponential identities, so no reverse
  comparison between the Riemannian distance and the ambient metric is needed.
  \source{petersen:page-0229}
\end{proof}

\begin{corollary}[unit-speed form: the notation $\overline{pq}$ is meaningful]
  \label{cor:pet-ch5-segment-notation-unitspeed}
  \lean{PetersenLib.completeManifold_exists_isSegment_unitSpeed,
        PetersenLib.segmentNotation_spec,
        PetersenLib.IsSegment.curveLength_eq_self_of_domain}
  \leanok
...
\begin{proof}
  ...
  By \cref{cor:pet-ch5-complete-segment-exists} take a segment
  $\gamma:[0,1]\to M$ from $p$ to $q$; its proportionality constant $k$ satisfies
  $k=L(\gamma)=|pq|$ on evaluating the proportionality clause at $t=1$.  If $p=q$
  the domain $[0,|pq|]$ degenerates to $[0,0]$ and the constant curve at $p$ is a
  segment, for which the arclength clause is vacuous.  If $p\neq q$ then $|pq|>0$
  and $\sigma(t):=\gamma(t/|pq|)$ is defined on $[0,|pq|]$; it is an admissible
  competitor because piecewise smoothness and length are stable under an affine
  reparametrization with positive slope, and its proportionality constant is
  $k\cdot|pq|^{-1}=1$.  For the converse clause, a segment on $[0,|pq|]$ from $p$
  to $q$ has $k\cdot|pq|=L=|pq|$, so $k=1$ since $|pq|\neq0$.
  \source{petersen:page-0229}
\end{proof}
...
\begin{proposition}[Prop. 5.7.7 — injectivity of $\exp_p$ on the star interior]
  ...
  \lean{PetersenLib.expMap_injectiveOnSegmentDomainStarInterior,
        PetersenLib.segment_eq_expMap_ray_of_mem_segmentDomainStarInterior',
        PetersenLib.segmentDomainStarInterior_subset_segmentDomain}
  \leanok
...
\begin{proof}
  ...
  hence a geodesic on $(0,1)$ by \cref{cor:pet-ch5-segments-are-geodesics}.  Now
  $c$ and $\sigma$ agree on the open set $(t_0,1)$, so they share position and
  velocity at any $a\in(t_0,1)$; since geodesics are uniquely determined by
  initial data and $(0,1)$ is connected, this agreement propagates across $t_0$,
  giving $c=\sigma$ on $(0,1)$ and hence $\tilde\sigma=\sigma|_{[0,t_0]}$.  (This
  route never needs $c$ to be smooth at the corner, which is why the node lands
  without any global smoothness of $\exp$.)
  \source{petersen:page-0233}
\end{proof}
...
\begin{lemma}[Lemma 5.7.8 — nonsingularity of $\exp_p$ on the star interior]
  ...
  \cref{rem:pet-ch5-segment-domain-open-blocker}. Its proof needs the Jacobi-field
  and Hessian-comparison apparatus of this subsection, none of which is on disk.
  \source{petersen:page-0233}
\end{lemma}
\begin{proof}
...
\begin{remark}[what \cref{prop:pet-ch5-segment-domain-open} is really blocked on]
  ...
  unformalized (its Jacobi-field and Hessian-comparison apparatus is the real cost),
  and it is the genuine dependency: this is a real blocker on Lemma 5.7.8, not a
  bookkeeping gap, and it should not be re-triaged as though the openness could be
  had directly.
\end{remark}
