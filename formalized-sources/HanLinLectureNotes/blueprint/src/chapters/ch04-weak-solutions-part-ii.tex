...
\tag{1}
\]
Iterate (1) with $\gamma_i=2\chi^i$ and
...
$\|u^+\|_{L^2(B_R)}$ between $L^p$ and $L^\infty$ in (3) to obtain
\[
F(r)\leq\frac12F(R)
+\frac{C}{(R-r)^{n/p}}\|u^+\|_{L^p(B_1)}
+C\|f\|_{L^q(B_1)},
\]
where $F(t)=\|u^+\|_{L^\infty(B_t)}$. Lemma~4.3 removes the
$F(R)$ term.
\end{proof}
...
\tag{1}
\]
Finitely iterate (1) through $2,2\chi,2\chi^2,\ldots$ until the exponent
reaches $q^*/\chi$, and apply (1) once more to obtain the asserted
$L^{q^*}$ estimate.
\end{proof}
...
Lemma~4.6 makes $v=(\log u)^-$ a subsolution, so Theorem~4.1 and
Lemma~4.8 give
\[
\sup_{B_{1/2}}v
...
\]
Choose $\zeta=1$ on $B_1$ and supported in $B_2$. Then (1) bounds
...
\]
has measure at least $|B_1|/2$. Apply Theorem~4.9 to the corresponding
...
a lower-order coefficient bounded in $L^q$. Theorem~4.1 therefore gives
\[
\inf_{B_\theta}\bar u
...
\]
Thus $w\in\BMO$, and Theorem~3.5 supplies $p_0>0$ such that
\[
...
\tag{3}
\]
Combining (1) and (3) proves the estimate for $p_0$.
...
\tag{4}
\]
Finitely iterating (4) upgrades $p_0$ to every
...
\end{corollary}
\begin{proof}
\sketch The oscillation argument for Corollary~4.17 gives
...
are quantitatively comparable on $[0,M]$, Theorem~4.14 gives
\[
\sup_{B_{R/2}}u
\leq C\left\{
...
\tag{2}
\]
Similarly, $w=(1-e^{-\alpha u})/\alpha$ is a supersolution. Theorem~4.15
...
\]
Combining this with (2) proves the estimate.
\end{proof}
...
Taking $\varphi=(u+M)\eta^2$ in (1) gives
\[
\int|Du|^2\eta^2
...
\end{corollary}
\begin{proof}
\sketch Repeat the oscillation argument for Corollary~4.17, replacing
Theorem~4.16 by Lemma~4.21.
\end{proof}
...
Theorem~3.11 applies. For $B_r(x_0)\subset B_1$, let
$w-u\in H^1_0(B_r(x_0))$ solve the frozen-coefficient homogeneous equation
and put $v=u-w$. The maximum principle and Lemma~3.9 give
\[
...
\tag{2}
\]
Testing the equation for $v$ by $v$ and using (1) yields
\[
\int_{B_r(x_0)}|Dv|^2
\leq C\left\{
...
\tag{3}
\]
Corollary~4.23 first gives $u\in C^{\delta_0}$ for some $\delta_0>0$.
Combining (3) with the energy estimate for $w$ and applying Lemma~3.4 gives,
for every $\delta<1$,
\[
\int_{B_r(x_0)}|Du|^2
\leq Cr^{n-2+2\delta}
...
$\delta$ with $3\delta>2$ and $\alpha+\delta>1$. Then (3) gives
\[
\int_{B_r(x_0)}|Dv|^2\leq Cr^{n+2\alpha'}
\]
for some $\alpha'<\alpha$. Equation (2), Lemma~3.4, and Theorem~3.1 imply
$Du\in C^{\alpha'}_{\mathrm{loc}}$, hence
$Du\in L^\infty_{\mathrm{loc}}$. Theorem~3.11 upgrades the exponent to
