...
\]
The iteration of Lemma~3.4 improves this to
\[
\int_{B_\rho(x_0)}u^2
...
Functions satisfying the hypothesis of Theorem~3.5 have bounded mean
...
\sketch Replace balls by comparable cubes and normalize $M=1$. Apply
Lemma~3.7 to $|u-u_Q|$ at a fixed level $\alpha>1$. Repeating the
decomposition inside each selected cube produces disjoint families
$\{Q_j^{(k)}\}$ such that
\[
\sum_j|Q_j^{(k)}|\leq\alpha^{-k}|Q|,
\qquad
|u-u_Q|\leq k2^n\alpha
\quad\text{a.e. on }Q\setminus\bigcup_jQ_j^{(k)}.
\]
Hence
\[
|\{x\in Q:|u-u_Q|>2^nk\alpha\}|
\leq\alpha^{-k}|Q|.
\]
Choosing $k$ with
$t\in[2^nk\alpha,2^n(k+1)\alpha)$ gives
\[
|\{x\in Q:|u-u_Q|>t\}|
\leq\alpha|Q|\exp\left(-\frac{\log\alpha}{2^n\alpha}t\right).
\]
Rescaling $M$ and integrating this distribution estimate proves the stated
exponential bound.
\end{proof}

\section*{3.2. H\"older Continuity of Solutions}
...
Suppose $w$ is as in Lemma~3.9 and $u\in H^1(B_r(x_0))$. Then, for
$0<\rho\leq r$,
\[
...
\end{corollary}
\begin{proof}
\sketch Write $v=u-w$. Apply Lemma~3.9 to $w=u-v$ and use
$|a+b|^2\leq2|a|^2+2|b|^2$, first for $Du$, then after subtracting the
corresponding averages.
\end{proof}

\begin{proof}[Proof of Theorem 3.8]
\sketch For $B_r(x_0)\subset B_1$, let $w-u\in H^1_0(B_r(x_0))$ solve
the frozen-coefficient homogeneous equation and put $v=u-w$. Testing the
equation for $v$ by $v$ gives
\[
\int_{B_r(x_0)}|Dv|^2\leq C\left\{
...
\right\}.
\]
Corollary~3.10 and H\"older's inequality therefore yield
\[
\begin{aligned}
\int_{B_\rho(x_0)}|Du|^2\leq C\biggl\{&
\left[\left(\frac\rho r\right)^n+\tau^2(r)\right]
\int_{B_r(x_0)}|Du|^2+r^{n-2+2\alpha}\|f\|_{L^q(B_1)}^2\\
&+\left(\int_{B_r(x_0)}|c|^n\right)^{2/n}
\int_{B_r(x_0)}u^2\biggr\}.
\end{aligned}
\tag{1}
\]
If $c=0$, Lemma~3.4 applied to (1) gives
\[
\int_{B_\rho(x_0)}|Du|^2
\leq C\left\{
\left(\frac\rho r\right)^{n-2+2\alpha}
\int_{B_r(x_0)}|Du|^2
+r^{n-2+2\alpha}\|f\|_{L^q(B_1)}^2\right\}.
\]
For general $c$, Lemma~3.3 first supplies a power bound for
$\int_{B_r}u^2$. Substitution in (1), followed alternately by Lemmas~3.4
and~3.3, raises that power by two at each step. After finitely many steps,
\[
\int_{B_\rho(x_0)}|Du|^2
\leq C\left\{
\left[\left(\frac\rho r\right)^n+\tau^2(r)\right]
...
\right\}.
\]
A final application of Lemma~3.4 gives the asserted Morrey estimate, and
Corollary~3.2 gives $u\in C^\alpha$.
\end{proof}

\subsection*{3.3. H\"older Continuity of Gradients}
...
Theorem~3.8. Corollary~3.10 gives, for $0<\rho\leq r$,
\[
\begin{aligned}
\int_{B_\rho(x_0)}|Du-(Du)_{x_0,\rho}|^2
\leq C\biggl\{&
...
\]
For constant $a_{ij}$ and $c=0$, Lemma~3.4 applied directly to (1) gives
...
\]
Substitution in (1) gives $Du\in C^{\alpha-\delta}_{\mathrm{loc}}$, hence a
local $L^\infty$ bound for $Du$. Substituting that bound back into (1) and
applying Lemma~3.4 gives exponent $\alpha$.

In the general case, the same bootstrap, with Lemma~3.3 controlling
$\int_{B_r}u^2$, yields
\[
...
\]
Lemma~3.4 and Theorem~3.1 complete the proof.
\end{proof}
...
harmonically across the origin by Theorem~1.28, contradicting the divergence
of $D_{11}u$.
\end{example}
