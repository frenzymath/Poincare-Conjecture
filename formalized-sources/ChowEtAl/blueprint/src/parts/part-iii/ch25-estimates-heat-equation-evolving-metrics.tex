\chapter{Estimates of the Heat Equation for Evolving Metrics}
...
\end{theorem}
\begin{proof}
Choose \(A\geq0\) so that \(Q+A-\mathcal R/(2p)\geq0\) for all
...
\end{lemma}
\begin{proof}
The subsolution inequality gives
\[
...
\end{proposition}
\begin{proof}
Rescale to \(r=1\).  Bishop--Gromov comparison under
...
\end{lemma}
\begin{proof}
Put \(\beta=(n+2)/n\), \(p_i=\beta^{i-1}\), and
\[
...
\begin{example}[Exercise 25.7: rescaled nested-cylinder iteration]
...
Prove Lemma~\ref{lem:chow-iii-moser-cylinder-iteration} using
...
\end{theorem}
\begin{proof}
Use the proof of Theorem~\ref{thm:chow-iii-li-yau-evolving-metrics} with
\(f(x,\tau)=d_{g(\tau)}(x,p)\).  Outside
...
\end{corollary}
\begin{proof}
Let \(\gamma\) join \(x_1\) to \(x_2\).  Along it, the global differential
estimate and
\[
 (1-\varepsilon)|\nabla\log u|^2
...
\end{corollary}
\begin{proof}
Integrate Theorem~\ref{thm:chow-iii-li-yau-ricci-flow} along the prescribed
...
\end{lemma}
\begin{proof}
The variation of the Laplacian is
\[
...
\end{corollary}
\begin{proof}
Insert the assumed bounds into Lemma
