\chapter{Tools Used in Proof of Pseudolocality}
\label{ch:chow-iii-tools-pseudolocality}
\dcref{ch22}
\source{chow-et-al-ricci-flow-part-iii:page-0204}

This chapter supplies the point-picking, heat-kernel, local entropy, and
logarithmic Sobolev tools used in the proof of pseudolocality.  The notation
\(\mathcal M_\alpha=\{(x,t):|\Rm|(x,t)>\alpha/t\}\) is used throughout.
...
\end{lemma}
\begin{proof}
If the initial point does not have the asserted property, choose a point in
...
\end{claim}
\begin{proof}
Starting at \((x_1,t_1)\), choose a later point only when the asserted bound
...
\end{claim}
\begin{proof}
At points which are not \(\alpha\)-large the bound follows from
...
\end{lemma}
\begin{proof}
The triangle inequality gives the inclusion at time \(\bar t\).  If it first
...
\end{lemma}
\begin{proof}
Local volume comparison and conservation of heat-kernel mass give uniform
...
solution amounts to proving convergence to \(\delta_{x_\infty}\) as
\(\tau\downarrow0\); this is posed as Mini-Problem 22.11.
\end{remark}
...
\end{lemma}
\begin{proof}
The adjoint equation and \(\Box^*v\le0\), together with the cutoff
...
\end{theorem}
\begin{proof}
Rescale so that the isoperimetric constant is \(c_n\).  Approximate by a
...
\end{theorem}
\begin{proof}
The proof of Theorem~\ref{thm:chow-iii-log-sobolev-isoperimetric} applies
unchanged because every positive superlevel set of \(\psi\) remains inside
the ball.
\end{proof}
...
\end{corollary}
\begin{proof}
Backward uniqueness gives \(\operatorname{Isom}(g(t))\subseteq\operatorname{Isom}(g(0))\);
