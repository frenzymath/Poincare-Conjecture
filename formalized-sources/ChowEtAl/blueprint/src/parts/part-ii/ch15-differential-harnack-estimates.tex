\chapter{Differential Harnack Estimates of LYH-type}
\label{ch:chow-ii-differential-harnack-estimates}

Throughout this chapter, \((\mathcal M^n,g(t))\) is a Ricci flow.  Indices are
raised with \(g(t)\), and \(R_{ijkl}\), \(R_{ij}\), and \(R\) denote its
...
\end{definition}
\begin{proof}
The skew symmetry is immediate from the definition.  The cyclic identity is
...
\end{lemma}
\begin{proof}
Differentiate the soliton equation and subtract the result with the first and
...
\end{lemma}
\begin{proof}
The differential in a direction \(A\oplus B\) is
\[
2(M_{ij}-P_{pij}\nabla^pf)W^iB^j
+2(P_{pij}-R_{pijq}\nabla^qf)A^{pi}W^j.
\]
...
\end{theorem}
\begin{proof}
First suppose \(\mathcal M\) is closed and \(\Rm>0\).  For small positive
...
\end{remark}
\begin{proof}
Apply the theorem on \([\varepsilon,T')\), with time translated by
...
\end{corollary}
\begin{proof}
For an orthonormal coframe \(\{\omega^a\}\), sum
...
\end{corollary}
\begin{proof}
Apply Corollary~\ref{cor:chow-ii-trace-harnack-and-tr-monotonicity} after
...
\end{corollary}
\begin{proof}
Nonnegative curvature operator gives \(0\le\Ric\le Rg\).  Insert
...
\end{corollary}
\begin{proof}
Repeat the path integration in the preceding proof using the ancient trace
...
\end{proposition}
\begin{proof}
Put \(H=\Delta\log R+R\).  Direct differentiation using
...
\end{remark}
\begin{proof}
Apply the positive-curvature estimate to \(R+\varepsilon\) in the standard
...
\end{corollary}
\begin{proof}
Set \(V=0\) in the weak trace estimate.  For an ancient flow translate the
...
\end{proposition}
\begin{proof}
Use \(R_{ij}=Rg_{ij}/2\) and write every two-form as
...
\end{proposition}
\begin{proof}
On a short interval beginning at \(t_0\), put
...
\end{lemma}
\begin{proof}
Contracted Bianchi and one commutation give
\[
...
\end{example}
\begin{proof}
Apply the product rule to each occurrence of \(U\) and \(W\).  Subtract and
...
\end{lemma}
\begin{proof}
Choose a distance-like function with bounded first and second derivatives for
...
\end{lemma}
\begin{proof}
With \(h\) from Lemma~\ref{lem:chow-ii-harnack-exhaustion-function}, take
...
\end{lemma}
\begin{proof}
Write \(\widehat Q=Q+t^{-1}\phi|W|^2+\psi|U|^2\) and apply the heat
...
\end{theorem}
\begin{proof}
Differentiate \(Z=M-S(P,P)\).  The inverse identity \(S\Rm=I\) gives
...
\end{remark}
\begin{proof}
Contract the definition of \(K\), use
...
\end{proposition}
\begin{proof}
Repeat the differentiation of \(M-S(P,P)\) in the preceding theorem without
...
\end{lemma}
\begin{proof}
Use the sum-of-squares coefficients
\(R=\sum_NY^N\otimes Y^N\),
\(P=\sum_NY^N\otimes X^N\), and
...
\end{lemma}
\begin{proof}
Apply \(S\) to the first equality and use \(S\Rm=I\) on two-forms.  The
...
\end{remark}
\begin{proof}
Diagonalize \(\Ric\) in an orthonormal frame.  Three-dimensional curvature
...
\end{corollary}
\begin{proof}
Translate an arbitrary ancient time \(\alpha\) to zero and apply
...
\end{lemma}
\begin{proof}
On a neighborhood where the nullity is constant, choose a nonzero compactly
...
\end{lemma}
\begin{proof}
Diagonalize the nonnegative quadratic \(\overline Q\) as
...
\end{theorem}
\begin{proof}
Run the complete bounded-curvature Ricci flow from \(g_0\).  The pinching is
...
\end{theorem}
\begin{proof}
Denote the left side by \(Z\).  Direct differentiation of the mean-curvature
