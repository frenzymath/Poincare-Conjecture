\chapter{Geometric Tools and Point Picking Methods}
\label{ch:chow-iii-geometric-tools-point-picking}

Throughout, \(d_g\), \(B_g(x,r)\), and \(L_g\) denote distance, metric balls,
and length for the metric \(g\).  For a time-dependent metric, let
\(\mathcal Z(t_0)\) be the compact set of unit-speed \(g(t_0)\)-minimizing
geodesics from \(x_0\) to \(x\).
...
\end{lemma}
\begin{proof}
For a fixed \(\eta\in\mathcal Z(t_0)\), comparison with its length at nearby
...
\end{corollary}
\begin{proof}
Substitute \(\partial_tg=-2\Ric\) in
...
\end{lemma}
\begin{proof}
For every \(g(t_0)\)-minimizing geodesic \(\eta\), one has
...
\end{lemma}
\begin{proof}
Vary \(\gamma\) through paths whose endpoint moves by the exponential map at
...
\end{theorem}
\begin{proof}
Choose a minimizing geodesic realizing the appropriate one-sided distance
...
\end{proposition}
\begin{proof}
Use in the index form the piecewise linear test function which rises from
...
\end{proposition}
\begin{proof}
Choose \(y_i\) with \(R(y_i)\to\infty\), and maximize
...
\end{proposition}
\begin{proof}
Choose \(y_i\) so that \(R(y_i)\varepsilon_i^2\to\infty\), maximize
...
\end{theorem}
\begin{proof}
The trace Harnack inequality propagates the curvature bound backward in time,
...
\end{corollary}
\begin{proof}
Theorem~\ref{thm:chow-iii-point-picking-infinite-ascr} provides the required
...
\end{lemma}
\begin{proof}
Starting at \((y_1,t_1)\), replace \((y_i,t_i)\) by an admissible point of
...
\end{proposition}
\begin{proof}
If a violating point existed, it would lie in \(\mathcal N_{B,C}\).  Follow
...
\end{lemma}
\begin{proof}
Lemma~\ref{lem:chow-iii-neck-center-bounds-ball} separates the neck into an
...
\end{lemma}
\begin{proof}
In the proof of Lemma~\ref{lem:chow-iii-busemann-level-diameter-neck}, the
union of the inner component and the neck has compact closure, while the
Busemann function on its complement is strictly greater than its value at a
point of the center sphere.  Hence its global infimum is achieved on that
compact closure.
\end{proof}
...
\end{proposition}
\begin{proof}
The neck separation inequalities give \(b_\gamma(y_1)<b_\gamma(y_2)\).
...
\end{theorem}
\begin{proof}
Scale to \(r_0=1\), reverse time, and base reduced distance and reduced volume
...
\end{lemma}
\begin{proof}
The curvature bound gives \(|\Ric|\le n-1\); integrating
