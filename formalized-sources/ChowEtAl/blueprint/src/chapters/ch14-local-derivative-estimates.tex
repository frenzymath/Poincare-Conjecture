\chapter{Local Derivative Estimates for Curvature}
\label{ch:chow-ii-local-derivative-estimates}

Throughout this chapter, \(g(t)\) denotes a Ricci flow and all tensor norms,
covariant derivatives, and metric balls are taken with respect to the indicated
metric.  We write \(\operatorname{Rm}\) and \(\operatorname{Rc}\) for its
...
\end{lemma}
\begin{proof}
Choose a nonincreasing smooth \(\lambda:[0,\infty)\to[0,1]\), equal to one
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:chow-ii-cutoff-gradient-laplacian-estimates} with the
...
\end{theorem}
\begin{proof}
The curvature evolution inequalities have the form
\[
...
\end{lemma}
\begin{proof}
The curvature evolution and Kato's inequality give, in the barrier sense,
...
\end{corollary}
\begin{proof}
Lemma~\ref{lem:chow-ii-curvature-doubling-time} bounds the curvature by
...
\end{theorem}
\begin{proof}
The standard curvature evolution inequalities imply, for
\[
...
\end{theorem}
\begin{proof}
Fix \((y,t)\) and put \(A=r^{-2}+t^{-1}+K\).  After the parabolic rescaling
...
\end{lemma}
\begin{proof}
For every \(k\), commuting covariant derivatives in the curvature equation
gives
\[
...
\end{theorem}
\begin{proof}
Induct on \(m\), using the initial bounds when the maximum of the localized
...
\end{lemma}
\begin{proof}
Set \(a=(m-\ell)_+\) and \(b=(m-\ell+1)_+\).  Apply the commuted curvature
...
\end{theorem}
\begin{proof}
Initial derivative and injectivity-radius bounds give pointed
...
\end{lemma}
\begin{proof}
Hamilton's trace Harnack inequality makes \(tR(x,t)\) nondecreasing, hence
...
\end{proposition}
\begin{proof}
Fix a background connection and solve the local Ricci--DeTurck equation
\[
...
\end{theorem}
\begin{proof}
Parabolically rescale so that \(\sup|\nabla\chi|^2+K=1\).  The local
...
\end{lemma}
\begin{proof}
Differentiating the Levi--Civita connection and then the curvature gives
\[
\partial_sR_{ijkl}
=\nabla_i\nabla_\ell h_{jk}+\nabla_j\nabla_kh_{i\ell}
-\nabla_i\nabla_kh_{j\ell}-\nabla_j\nabla_\ell h_{ik}
