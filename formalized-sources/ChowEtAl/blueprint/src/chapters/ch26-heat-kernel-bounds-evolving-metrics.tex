\chapter{Bounds for the Heat Kernel for Evolving Metrics}
...
\end{lemma}
\begin{proof}
Expand \(L,L^*\), use the evolution of the volume form, and integrate the
...
\end{lemma}
\begin{proof}
Apply Duhamel's identity to \(H(\cdot,\tau;z,\rho)\) and
...
\end{lemma}
\begin{proof}
Differentiate the integral, use \(LH=0\), and apply Gronwall to
\(I'=\int(\mathcal R-Q)H\).
\end{proof}
...
\end{lemma}
\begin{proof}
The derivative with respect to the intermediate time vanishes by the forward
...
\end{lemma}
\begin{proof}
The outward normal derivative of the positive Dirichlet kernel is
...
\end{lemma}
\begin{proof}
The upper bound follows by exhausting \(\mathcal M\) with Dirichlet kernels.
...
\end{lemma}
\begin{proof}
Use the identity on an exhaustion by Dirichlet domains.  Monotone convergence
...
\end{lemma}
\begin{proof}
Apply the parabolic mean value inequality at radius \(\sqrt{\tau-v}/2\),
...
\end{lemma}
\begin{proof}
After replacing (u) by \(e^{-C_0\tau}u\), multiply its equation by a
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:chow-iii-weighted-l2-heat-estimate} to Dirichlet kernels
...
\end{theorem}
\begin{proof}
Split the semigroup identity at \(\rho=(\tau+v)/2\), distribute half the
...
\end{corollary}
\begin{proof}
Near the diagonal use the rough bound.  Off the diagonal use the two-volume
...
\end{lemma}
\begin{proof}
Integrate the one-volume Gaussian upper bound in polar shells.  Relative
...
\end{lemma}
\begin{proof}
The Li--Yau Harnack inequality compares the diagonal value at time \(\tau\)
...
\end{theorem}
\begin{proof}
Apply the Li--Yau Harnack inequality between \((x,\tau)\) and the diagonal
...
\end{theorem}
\begin{proof}
Differentiate the average in (r).  Two integrations by parts using
...
\end{theorem}
\begin{proof}
The diagonal hypothesis yields a Gaussian off-diagonal bound, hence compact
...
\end{theorem}
\begin{proof}
Apply the space-time divergence theorem to
...
\end{proposition}
\begin{proof}
Start from a smooth (u) with \(|u-d(\cdot,O)|\le1\) and
