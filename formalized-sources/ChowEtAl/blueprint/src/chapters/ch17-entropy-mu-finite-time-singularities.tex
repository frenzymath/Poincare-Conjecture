\chapter{Entropy, \texorpdfstring{$\mu$}{mu}-Invariant, and Finite Time Singularities}
\label{ch:chow-iii-entropy-mu-finite-time-singularities}
\dcref{ch17}
\source{chow-et-al-ricci-flow-part-iii:page-0022}

Throughout this chapter, $(\mathcal M^n,g)$ is closed unless explicitly
stated otherwise.  We write $d\mu=d\mu_g$.
...
\end{lemma}
\begin{proof}
Differentiate $\mathcal K(g,w,\tau)$ along variations tangent to the unit
...
\end{lemma}
\begin{proof}
For $\int w^2d\mu=1$, the inequality
...
\end{corollary}
\begin{proof}
Use $\mu(g,\tau)=\mu(cg,c\tau)$ in the preceding lemma with
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:chow-iii-ch17-logarithmic-sobolev} with $a=2\tau$
...
\end{lemma}
\begin{proof}
For $\int w^2d\mu=1$, Jensen's inequality and the $L^2$ Sobolev inequality
give
\[
...
\end{lemma}
\begin{proof}
Entropy monotonicity and the lower bound at scale $n/8+t$ give
\[
...
\end{lemma}
\begin{proof}
For a blow-up factor $K_i\to\infty$, the rescaled time-zero volume is
...
\end{remark}
\begin{proof}
No local collapsing gives a uniform positive lower bound for the volume of
...
\end{corollary}
\begin{proof}
The contrapositive of the preceding lemma gives positivity at some time.
...
\end{theorem}
\begin{proof}
Let $g_i(t)=Q_i g(t_i+Q_i^{-1}t)$ converge smoothly to $g_\infty(t)$.
...
\end{lemma}
\begin{proof}
At one fixed scale, small-scale negativity and continuity of $\mu$ bound all
...
\end{lemma}
\begin{proof}
Let $\widetilde w$ minimize $\mathcal K(\widetilde g,\cdot,\tau)$ and
...
\end{corollary}
\begin{proof}
The preceding theorem makes the model a compact shrinking gradient Ricci
...
\end{lemma}
\begin{proof}
Run Ricci flow from $g$ to time $\bar\tau$ and use the adjoint heat kernel
...
\end{proposition}
\begin{proof}
Negativity gives $\limsup\mu\le0$.  If the lower limit were negative, choose
...
\end{lemma}
\begin{proof}
If constant minimizers existed along $\tau_i\downarrow0$, the
...
\end{proposition}
\begin{proof}
By scaling take $\tau=1$.  A normalized nonnegative minimizing sequence for
...
\end{lemma}
\begin{proof}
The mean value theorem writes the pointwise difference as
...
\end{lemma}
\begin{proof}
Let $F(r)$ be the average of $w$ over $S(p,r)$ and $G(r)$ the average of its
...
\end{lemma}
\begin{proof}
At a maximum point of $w_\tau$, one has $\Delta w_\tau\le0$.  Substitute
...
\end{lemma}
\begin{proof}
Use the standard variations of scalar curvature, the Laplacian, and squared
...
\end{lemma}
\begin{proof}
Differentiate the modified-scalar formula for $\mathcal F$ and use that
...
\end{theorem}
\begin{proof}
The curvature evolution and the contracted Bianchi identity for $\alpha$
give
\[
 \frac14\Box^*(|\Rm|^2e^{-f})
 =e^{-f}\bigl(-L(\alpha,\nabla f)
