\chapter{Heat Kernel for Static Metrics}
\label{chow-iii-ch23-heat-kernel-static-metrics}

This chapter constructs the heat kernel of a closed Riemannian manifold by a
parametrix and space-time convolution, and records its short-time expansion.
...
\end{theorem}
\begin{proof}
The construction in Theorem~\ref{thm:chow-iii-ch23-parametrix-heat-kernel}
...
\end{lemma}
\begin{proof}
Apply the radial Laplacian formula to $E\sum_k\phi_k(t-u)^k$.  Equating each
...
\end{lemma}
\begin{proof}
Integrate the exact derivative in the proof of Lemma~\ref{lem:chow-iii-ch23-recursive-odes}
and substitute $r=\rho r(x)$.
\end{proof}
...
\end{lemma}
\begin{proof}
Differentiate $(\Box_xH_N)=E(\Delta_x\phi_N)(t-u)^N$.  Each spatial derivative
...
\end{proposition}
\begin{proof}
The support of $\eta$ lies inside the injectivity neighborhood, so $P_N$ is
...
\begin{example}[Exercise 23.13 (Convolution and sum)]
...
\end{example}
\begin{proof}
Expand the convolution, use bilinearity and associativity, and pass to the
...
\end{lemma}
\begin{proof}
Inside the region where $\eta=1$ use Lemma~\ref{lem:chow-iii-ch23-heat-error-derivatives}; on the annulus where derivatives of $\eta$ occur the distance is bounded below, so the Gaussian dominates every negative power of $t-u$.  The exterior is identically zero.
\end{proof}
...
\end{definition}
\begin{proof}
Associativity follows by expanding both sides as a triple integral and applying
...
\end{lemma}
\begin{proof}
Differentiate the defining integral in $t$ and use the upper-endpoint term
...
\end{theorem}
\begin{proof}
The convolution estimates below give locally uniform convergence of $G_N$ and
...
\end{theorem}
\begin{proof}
Take $u_j=\eta(d)\phi_j$ for $j\le N$ and subtract the finite parametrix
...
\end{example}
\begin{proof}
Differentiate the convolution representation of $w_N$ and use the derivative
...
\end{lemma}
\begin{proof}
Use the product formula for $P_N$ and factor the Gaussian with denominator five.
...
\end{lemma}
\begin{proof}
The case $k=1$ is Lemma~\ref{lem:chow-iii-ch23-coefficient-structure}.  For
...
\end{corollary}
\begin{proof}
Sum the majorant in Lemma~\ref{lem:chow-iii-ch23-convolution-estimates}; it is
...
\end{lemma}
\begin{proof}
Differentiate the first factor of each convolution.  The derivative estimate
...
\end{lemma}
\begin{proof}
In a finite atlas, covariant derivatives are linear combinations of coordinate
...
\end{lemma}
\begin{proof}
Split the integral into a small normal ball and its complement.  The Gaussian
...
\end{lemma}
\begin{proof}
The difference between $x$- and $z$-second derivatives of $P_N$ is bounded by
...
\end{lemma}
\begin{proof}
Write the convolution as $\int_0^tJ_N(s,t)\,ds$.  The upper endpoint tends to
...
\end{lemma}
\begin{proof}
Insert the normal-coordinate metric expansion into the determinant expansion of
...
\end{lemma}
\begin{proof}
Use $\Delta r=(n-1)/r+\partial_r\log\alpha$ and
...
\end{lemma}
\begin{proof}
Differentiate the normal-coordinate expansion for $\alpha^{1/2}$ and use
...
\end{lemma}
\begin{proof}
Insert the expansion of $\Delta\phi_0$ into the integral formula for $\phi_1$;
...
\end{example}
\begin{proof}
Scale $x=\sqrt{4t}\,\widetilde x$ and use rotational invariance; diagonalizing
...
\end{lemma}
\begin{proof}
Uniform convergence makes the tails outside large balls uniformly small.  Apply
...
\end{lemma}
\begin{proof}
Apply Lemma~\ref{lem:chow-iii-ch23-fubini-noncompact} to $\partial_tf$ and use
