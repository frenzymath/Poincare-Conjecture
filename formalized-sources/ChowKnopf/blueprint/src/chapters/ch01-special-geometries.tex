...
The notions of local homogeneity, homogeneity, homogeneous model, and model geometry are introduced here, together with the isotropy group of a model geometry. The basic comparison between locally homogeneous metrics and complete homogeneous spaces is summarized in the propositions below.
...
\begin{proof}
\source{chow-knopf-ricci-flow:page-0019}
Given any $x,y \in \M^n$, choose an isometry $\gamma$ with $\gamma(x)=y$. Conjugation by $\gamma$ defines an isomorphism from $I_x(\M^n,g)$ to $I_y(\M^n,g)$, with inverse given by conjugation by $\gamma^{-1}$.
\end{proof}
...
The Ricci flow of a left-invariant metric on a unimodular Lie group is reduced to an ordinary differential system in a Milnor frame. The formulas below record the structure constants, the Levi-Civita connection, and the resulting curvature simplifications.
...
\begin{proof}
\source{chow-knopf-ricci-flow:page-0023}
Using the Milnor-frame formulas for $\ad^*$ and the Levi-Civita connection, the mixed curvature component reduces to a product of orthogonal vectors, hence vanishes.
\end{proof}
...
\begin{proof}
\source{chow-knopf-ricci-flow:page-0025,chow-knopf-ricci-flow:page-0026}
After introducing $D=\epsilon A$ and ordering the variables so that $D\le C \le B$, one obtains differential inequalities forcing finite-time extinction and showing that $B,C,D$ stay comparable as $t$ approaches the singular time. This implies asymptotic roundness.
\end{proof}
...
\begin{proof}
\source{chow-knopf-ricci-flow:page-0027}
The quantities $(AE)$ and $E/(BC)$ satisfy differential inequalities that yield a uniform lower bound for $BC$ on a time interval depending only on the initial data, and this in turn bounds the evolution of $A$ from below. Hence the solution persists on a common interval independent of $\varepsilon$.
\end{proof}
...
\begin{proof}
\source{chow-knopf-ricci-flow:page-0027,chow-knopf-ricci-flow:page-0028}
The evolution equation for $F$ has a negative term of size $Q/\varepsilon$ dominating the positive part uniformly on compact time intervals, so $F(t,\varepsilon)$ is forced to zero as $\varepsilon\searrow 0$.
\end{proof}
