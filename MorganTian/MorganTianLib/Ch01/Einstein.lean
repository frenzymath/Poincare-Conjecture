...
* the **Ricci curvature tensor** `Ric(x,y) = ∑ᵢ R(x,eᵢ,y,eᵢ)` and the
  **scalar curvature** `R = tr_g Ric` (Morgan–Tian `def:ricci-curvature`),
  re-exported from OpenGALib's basis-free `Riemannian.ricciForm` /
...
OpenGALib's `curvatureForm X Y Z W = ⟨R(X,Y)Z, W⟩`; the two sign flips cancel, so
Morgan–Tian's 4-tensor at `(X,Y,Z,W)` *equals* OpenGALib's `curvatureForm X Y Z W`
