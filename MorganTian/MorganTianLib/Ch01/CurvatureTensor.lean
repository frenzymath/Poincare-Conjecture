...
which is exactly *minus* OpenGALib's do-Carmo-convention curvature operator
...
opposed to OpenGALib's `Riemannian.AffineConnection.curvatureForm X Y Z T p
...
transported through this bridge from OpenGALib's Ch. 4 results. The
...
This is the *Morgan–Tian* sign convention: it is the negative of OpenGALib's
...
/-- **Math.** Pointwise, Morgan–Tian's curvature operator is minus OpenGALib's
...
OpenGALib's do-Carmo-convention `Riemannian.AffineConnection.curvatureForm`
...
OpenGALib's `Riemannian.AffineConnection.curvatureForm_bianchi` (cyclic in the
