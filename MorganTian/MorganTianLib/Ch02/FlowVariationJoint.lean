...
/-- **Math.** Postcomposition by an operator-curve difference is the difference of
the postcompositions: `A ↦ postcompCurve A` is additive. -/
lemma postcompCurve_sub {K : Type*} [TopologicalSpace K] [CompactSpace K]
