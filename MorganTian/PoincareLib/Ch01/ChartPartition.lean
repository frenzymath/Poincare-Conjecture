...
is computed by chaining one-chart geodesic-flow steps along the compact
geodesic `γ = γ_v : [0,1] → M`. The image `γ([0,1])` is compact, so it is
covered by finitely many chart sources, and `[0,1]` can be partitioned so that
each piece maps into a single chart. This file provides that partition, which
the flow-derivative gluing (`FlowStep`/`StateTransition`/`FlowGluing`) consumes:
each junction between consecutive pieces re-reads the flow state across a chart
change (`stateTransition_jacobiVarPair`), each piece transports the variational
pair inside its chart (`IsJacobiFieldAlongOn.variational_transport`).
...
partition consumed by the flow-derivative gluing of
