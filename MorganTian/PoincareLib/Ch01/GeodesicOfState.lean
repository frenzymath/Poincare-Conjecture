...
The flow chain of `lem:exponential-differential-jacobi` computes, for every chart
...
`stateTransition`), and OpenGALib's intrinsic uniqueness
...
This is OpenGALib's intrinsic uniqueness
...
Construction: Hopf–Rinow (`OpenGALib`'s `exists_global_geodesic`) produces a
