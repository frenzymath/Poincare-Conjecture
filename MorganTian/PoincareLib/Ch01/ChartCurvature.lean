...
/-!
# Poincaré Ch. 1 — chart-level curvature from Christoffel symbols

This file instantiates the manifold-free curvature-commutation engine of
`PoincareLib.Ch01.CurvatureCommutation` with the actual Christoffel data of a
Riemannian metric `g` in the chart at `α`, from OpenGALib's chart API:
...
* `chartChristoffelBilin g α : E → E →L[ℝ] E →L[ℝ] E` — the chart Christoffel
  ...
  `Riemannian.Geodesic.chartChristoffelContraction`, symmetric in `v, w`,
...
* `covDerivAlong_chartChristoffelBilin_eq` — along a curve (`P = ℝ`, `d = 1`)
  the engine's covariant derivative is OpenGALib's
...
(`PoincareLib.Ch01.JacobiODE`, `lem:second-order-linear-ode`) and actual
...
the curvature-commutation engine (`PoincareLib.covDerivAlong`,
`PoincareLib.christoffelCurvature`). -/
...
/-- **Math.** The bilinear-map packaging agrees with OpenGALib's
...
`Riemannian.chartChristoffel_contDiffOn_interior`). -/
...
derivative is exactly OpenGALib's coordinate covariant derivative
`DV/dt = V̇ + Γ(u̇, V)(u)`. -/
