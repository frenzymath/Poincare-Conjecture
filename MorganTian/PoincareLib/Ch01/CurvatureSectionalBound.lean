...
`PoincareLib.Ch01.CurvatureFrameBridge`:
...
`PoincareLib.Ch01.PointwiseCurvature`; `chartCurvature` is Morgan–Tian's
...
`J, u : E`. Combined with `PoincareLib.jacobi_frame_sturm_comparison`
