...
`PoincareLib.Ch01.ChartCurvature`) with the **manifold-level pointwise
curvature tensor** `curvatureFormAt g ∇ p` (defined from the Levi-Civita
connection on vector fields, `PoincareLib.Ch01.PointwiseCurvature`), and
...
`PoincareLib.jacobi_frame_sturm_comparison`.
...
* `curvatureFormAt_chartFrame` — **the bridge**: for `p` in the chart at `α`
  ...
  `PoincareLib.Ch01.PointwiseCurvature`;
* `chartCurvature_pairing_le_of_sectionalCurvatureAt_le` — the **payoff**:
  ...
  frame reduction `PoincareLib.jacobi_frame_sturm_comparison`
...
is `PoincareLib.chartCurvature_add_left`).
Blueprint: `lem:chart-curvature-coordinates`. -/
...
Slot-1 additivity/homogeneity is `PoincareLib.curvatureFormAt_add_left` /
`curvatureFormAt_smul_left` (`PoincareLib.Ch01.PointwiseCurvature`); the other
