...
and the discharged step `PoincareLib.convStepProperty` of `EndsConvStep.lean`.
...
This is Hopf–Rinow `c ⟹ d` (`OpenGALib`'s `exists_global_geodesic`). -/
...
invariant `ConvAt g γ γs` (positions and chart-at-the-limit velocities converge)
...
theorem convAt_of_convAt_zero (g : RiemannianMetric I M)
    ...
    (h0 : ConvAt (I := I) g γ γs 0) (t : ℝ) :
    ConvAt (I := I) g γ γs t := by
