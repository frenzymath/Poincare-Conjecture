...
* `laplacianAt g nabla f p`, the **Laplacian** `Δf(p) = tr_g Hess(f)_p`,
  ...
  as in `PoincareLib.scalarCurvatureAt`), together with basis-independence
...
`PoincareLib.Ch02.LaplacianExtremum`.
...
`PoincareLib.hessian_smul` records joint `𝒟(M)`-homogeneity; the locality
engine of `PoincareLib.Ch01.PointwiseCurvature` needs the four slotwise
