...
* **the variational flow computes the derivative of the flow**: at every initial
  ...
  `v ↦ (t ↦ τ x₀ (t) v)` (`Riemannian.FlowDependence.flowDeriv_eq_applyCurve_opFlow`);
* **second-order dependence**: the family `x ↦ τ x` is itself strictly differentiable at
  every point of the open flow ball
  (`Riemannian.FlowDependence.exists_hasStrictFDerivAt_opFlow_of_picardResidual_curve`),
...
This is the C²-regularity content of do Carmo, *Riemannian Geometry*, Ch. 3,
...
(`Riemannian.Exponential.exists_contDiffOn_two_extChartAt_expMap_ball`).
-/
