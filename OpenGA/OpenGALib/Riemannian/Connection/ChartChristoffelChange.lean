...
1. the zeroth-order Gram change law `G^β = Aᵀ(G^α∘τ)A` (`chartGramMatrix_change`,
   ...
   the open overlap `chartTransitionSource β α` in `β`-chart coordinates
   (`chartGramOnE_chartTransition`);
...
`chartTransitionSource β α` its derivative is `tangentCoordChange I β α` at the
...
coordinates: the source of the transition `chartTransition β α`. -/
...
theorem chartGramOnE_chartTransition (g : RiemannianMetric I M) (α β : M) {y : E}
    (hy : y ∈ chartTransitionSource (I := I) (M := M) β α)
    (i j : Fin (Module.finrank ℝ E)) :
    chartGramOnE (I := I) g β i j y
      = ∑ a, ∑ b,
          chartGramOnE (I := I) g α a b (chartTransition (I := I) β α y)
            * transitionDeriv (I := I) β α a i y
  ...
  rw [chartGramOnE_def, extChartAt_symm_chartTransition (I := I) hy,
...
Product rule on `chartGramOnE_chartTransition`; the derivative of the moving
transition derivative brings in the second derivative of the transition. -/
theorem partialDeriv_chartGramOnE_chartTransition (g : RiemannianMetric I M)
    (α β : M) {y : E}
    (hy : y ∈ chartTransitionSource (I := I) (M := M) β α)
  ...
  have heq : chartGramOnE (I := I) g β i j =ᶠ[𝓝 y]
      (fun z => ∑ a, ∑ b,
        chartGramOnE (I := I) g α a b (chartTransition (I := I) β α z)
          * transitionDeriv (I := I) β α a i z
          * transitionDeriv (I := I) β α b j z) := by
    filter_upwards [(isOpen_chartTransitionSource (I := I) β α).mem_nhds hy] with z hz
    exact chartGramOnE_chartTransition (I := I) g α β hz i j
  have hτ : HasFDerivAt (chartTransition (I := I) β α)
      (fderiv ℝ (chartTransition (I := I) β α) y) y :=
    (hasFDerivAt_chartTransition (I := I) hy).differentiableAt.hasFDerivAt
  have hG : ∀ a b : Fin (Module.finrank ℝ E),
      HasFDerivAt (chartGramOnE (I := I) g α a b)
        (fderiv ℝ (chartGramOnE (I := I) g α a b) (chartTransition (I := I) β α y))
        (chartTransition (I := I) β α y) := fun a b => by
    have hcd : ContDiffAt ℝ ∞ (chartGramOnE (I := I) g α a b)
      ...
      (chartGramOnE_contDiffOn (I := I) g α a b).contDiffAt
        ((isOpen_extChartAt_target (I := I) α).mem_nhds
          (chartTransition_mem_target (I := I) hy))
    exact (hcd.differentiableAt (by simp)).hasFDerivAt
  have hcomp : ∀ a b : Fin (Module.finrank ℝ E),
      HasFDerivAt
        (fun z => chartGramOnE (I := I) g α a b (chartTransition (I := I) β α z))
        ((fderiv ℝ (chartGramOnE (I := I) g α a b)
            (chartTransition (I := I) β α y)).comp
          (fderiv ℝ (chartTransition (I := I) β α) y)) y :=
        ...
        + transitionDeriv (I := I) β α b j y
          • (chartGramOnE (I := I) g α a b (chartTransition (I := I) β α y)
            ...
            + transitionDeriv (I := I) β α a i y
              • ((fderiv ℝ (chartGramOnE (I := I) g α a b)
                    (chartTransition (I := I) β α y)).comp
                  (fderiv ℝ (chartTransition (I := I) β α) y))))) y := by
  ...
  refine Finset.sum_congr rfl fun a _ => Finset.sum_congr rfl fun b _ => ?_
  have hsum : fderiv ℝ (chartGramOnE (I := I) g α a b) (chartTransition (I := I) β α y)
      (fderiv ℝ (chartTransition (I := I) β α) y ((Module.finBasis ℝ E) k))
      = ∑ c, partialDeriv (E := E) c (chartGramOnE (I := I) g α a b)
            (chartTransition (I := I) β α y)
          * Geodesic.chartCoord (E := E) c
              (fderiv ℝ (chartTransition (I := I) β α) y ((Module.finBasis ℝ E) k)) := by
...
lemma eq_zero_of_forall_sum_mul_transitionDeriv {β α : M} {y : E}
  ...
  have hαsrc : x_y ∈ (extChartAt I α).source := by
    rw [extChartAt_source]; exact extChartAt_symm_mem_chartAt_source_right (I := I) hy
  have hderiv : fderiv ℝ (chartTransition (I := I) β α) y
      = tangentCoordChange I β α x_y := fderiv_chartTransition (I := I) hy
...
theorem sum_gram_mul_christoffel_transition (g : RiemannianMetric I M)
    ...
    ∑ m, chartGramOnE (I := I) g β a m y * chartChristoffel (I := I) g β k i m y
      = ∑ p, transitionDeriv (I := I) β α p a y
          * (∑ q, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
              * ((∑ c, ∑ d,
  ...
  have hfootα : (extChartAt I α).symm (chartTransition (I := I) β α y)
      ∈ (trivializationAt E (TangentSpace I) α).baseSet := by
    rw [trivializationAt_baseSet_eq_chartAt_source,
      extChartAt_symm_chartTransition (I := I) hy]
    exact extChartAt_symm_mem_chartAt_source_right (I := I) hy
  ...
  rw [chartGram_christoffel_contraction (I := I) g β a k i y hfootβ,
    partialDeriv_chartGramOnE_chartTransition (I := I) g α β hy k a i,
    partialDeriv_chartGramOnE_chartTransition (I := I) g α β hy i a k,
    partialDeriv_chartGramOnE_chartTransition (I := I) g α β hy a k i]
  exact key (Module.finrank ℝ E)
    (fun p q => chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y))
...
theorem sum_gram_mul_christoffel_expand (g : RiemannianMetric I M)
  ...
  have hG : ∀ m, chartGramOnE (I := I) g β a m y
      = ∑ p, ∑ q, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
          * transitionDeriv (I := I) β α p a y * transitionDeriv (I := I) β α q m y :=
    fun m => chartGramOnE_chartTransition (I := I) g α β hy a m
  have hLHS : ∑ m, chartGramOnE (I := I) g β a m y * chartChristoffel (I := I) g β k i m y
      = ∑ p, ∑ q, ∑ m, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
          * transitionDeriv (I := I) β α p a y * transitionDeriv (I := I) β α q m y
          * chartChristoffel (I := I) g β k i m y := by
    calc ∑ m, chartGramOnE (I := I) g β a m y * chartChristoffel (I := I) g β k i m y
        = ∑ m, (∑ p, ∑ q, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
              * transitionDeriv (I := I) β α p a y * transitionDeriv (I := I) β α q m y)
            * chartChristoffel (I := I) g β k i m y :=
          Finset.sum_congr rfl fun m _ => by rw [hG m]
      _ = ∑ m, ∑ p, ∑ q, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
              * transitionDeriv (I := I) β α p a y * transitionDeriv (I := I) β α q m y
              * chartChristoffel (I := I) g β k i m y := by
          simp_rw [Finset.sum_mul]
      _ = ∑ p, ∑ q, ∑ m, chartGramOnE (I := I) g α p q (chartTransition (I := I) β α y)
              * transitionDeriv (I := I) β α p a y * transitionDeriv (I := I) β α q m y
              * chartChristoffel (I := I) g β k i m y := by
          rw [Finset.sum_comm]
          refine Finset.sum_congr rfl fun p _ => ?_
...
theorem sum_transitionDeriv_mul_chartChristoffel (g : RiemannianMetric I M)
    ...
    ∑ m, transitionDeriv (I := I) β α q m y
        * chartChristoffel (I := I) g β k i m y
      = (∑ c, ∑ d,
          chartChristoffel (I := I) g α c d q (chartTransition (I := I) β α y)
            * transitionDeriv (I := I) β α c k y
            * transitionDeriv (I := I) β α d i y)
        + transitionSndDeriv (I := I) β α q k i y := by
  classical
  set W : Fin (Module.finrank ℝ E) → ℝ := fun q' =>
      (∑ m, transitionDeriv (I := I) β α q' m y * chartChristoffel (I := I) g β k i m y)
        - ((∑ c, ∑ d, chartChristoffel (I := I) g α c d q' (chartTransition (I := I) β α y)
  ...
  set U : Fin (Module.finrank ℝ E) → ℝ := fun p =>
      ∑ q', chartGramOnE (I := I) g α p q' (chartTransition (I := I) β α y) * W q'
    with hU_def
  ...
  have hτy : (extChartAt I α).symm (chartTransition (I := I) β α y)
      ∈ (trivializationAt E (TangentSpace I) α).baseSet := by
    rw [trivializationAt_baseSet_eq_chartAt_source, extChartAt_symm_chartTransition (I := I) hy]
    exact extChartAt_symm_mem_chartAt_source_right (I := I) hy
  have hW0 : ∀ q', W q' = 0 :=
    eq_zero_of_forall_sum_chartGramOnE_mul (I := I) g (α := α)
      (y' := chartTransition (I := I) β α y) hτy W hU0
  have := hW0 q
...
`τ = chartTransition β α` and `D²τ` its second derivative at the `β`-chart image
of `x`,
...
theorem chartChristoffelContraction_change (g : RiemannianMetric I M)
    ...
    tangentCoordChange I β α x
      ...
      = Geodesic.chartChristoffelContraction (I := I) g α
          ...
          (extChartAt I α x)
        + fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) (extChartAt I β x)
            v w := by
  have hy : extChartAt I β x ∈ chartTransitionSource (I := I) (M := M) β α :=
    extChartAt_mem_chartTransitionSource (I := I) hxβ hxα
  have hxsymm : (extChartAt I β).symm (extChartAt I β x) = x :=
    (extChartAt I β).left_inv (by rw [extChartAt_source]; exact hxβ)
  have hτx : chartTransition (I := I) β α (extChartAt I β x) = extChartAt I α x :=
    chartTransition_extChartAt (I := I) hxβ
  have hAx : fderiv ℝ (chartTransition (I := I) β α) (extChartAt I β x)
  ...
  have hLHS : Geodesic.chartCoord (E := E) p
      ...
      = (∑ k, ∑ i, Geodesic.chartCoord (E := E) k v * Geodesic.chartCoord (E := E) i w *
          (∑ c, ∑ d, chartChristoffel (I := I) g α c d p (chartTransition (I := I) β α y)
              * transitionDeriv (I := I) β α c k y * transitionDeriv (I := I) β α d i y))
        + ∑ k, ∑ i, Geodesic.chartCoord (E := E) k v * Geodesic.chartCoord (E := E) i w *
  ...
  have hR2 : Geodesic.chartCoord (E := E) p
      (((fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) y) v) w)
      = ∑ k, ∑ i, Geodesic.chartCoord (E := E) k v * Geodesic.chartCoord (E := E) i w *
          transitionSndDeriv (I := I) β α p k i y := by
    have hgv : ∀ v', (((fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) y) v') w)
        = ((ContinuousLinearMap.apply ℝ E w).comp
            (fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) y)) v' := fun v' => rfl
    ...
    have hgv2 : Geodesic.chartCoord (E := E) p
        (((ContinuousLinearMap.apply ℝ E w).comp
            (fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) y)) ((Module.finBasis ℝ E) k))
        = Geodesic.chartCoord (E := E) p
            (((fderiv ℝ (fderiv ℝ (chartTransition (I := I) β α)) y) ((Module.finBasis ℝ E) k)) w) :=
      rfl
