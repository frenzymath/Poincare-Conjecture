...
/--
The OpenGALib naming-convention linter.
...
`InnerProductSpace`).
-/
register_option linter.openGA.naming : Bool := {
  defValue := true
...
@[inherit_doc linter.openGA.naming]
def namingLinter : Linter where run := withSetOptionIn fun stx ↦ do
  unless getLinterValue linter.openGA.naming (← getLinterOptions) do return
