...
/-!
# Poincaré Ch. 1, §1.4 — the ball-uniform manifold flow-step link

`exists_geodesic_flow_step_jacobiTransport_manifold_at` (`FlowStepManifoldAt`)
supplies a geodesic-flow step of the exponential differential
(`cor:dc-ch5-2-5`) based at an *arbitrary* interior time
`a`, landing at a prescribed nearby time `b`. It builds a *fresh* geodesic flow
for each base time. To glue steps along the whole compact geodesic
`γ = γ_v : [0,1] → M` via a *finite* partition, one instead needs a **single**
flow that serves *every* base time in a whole time-window at once: only then do
the cover elements form genuine open intervals `(s₀ - ρ, s₀ + ρ)` such that *any*
subinterval `[t_i, t_{i+1}]` admits a flow step — the shape consumed by
`exists_monotone_Icc_subset_open_cover_unitInterval`.

This file supplies that. Anchoring one geodesic flow `Z` at the chart state of
`γ` at a center time `s₀` (via `exists_geodesic_flow_step`, whose Picard time `T`
and radius `r` are *uniform over a ball* of initial conditions), there is a
positive window radius `ρ` such that for *every* pair of times `a ≤ b` in
`(s₀ - ρ, s₀ + ρ)`:

* the time-`(b - a)` endpoint map `flowEnd = (fun z ↦ Z z (b - a))` of the
  *shared* flow is strictly differentiable at the chart state
  `(φ_β(γ a), u̇^β(a))` of `γ` at `a`, with derivative `Dstep`;
* `flowEnd` carries that state to the chart state `(φ_β(γ b), u̇^β(b))` at `b`;
* for every manifold Jacobi field `(J, DJ)` along `γ`,
  `Dstep (jacobiVarPair β γ J DJ a) = jacobiVarPair β γ J DJ b`.

The base `a` is admissible because the chart state `a ↦ (φ_β(γ a), u̇^β(a))` is
continuous at `s₀` (`IsGeodesicOn.continuousAt_deriv_extChartAt` for the velocity
component), so shrinking `ρ` keeps every base state inside the ball
`ball zs₀ r` on which the shared flow's strict derivative is defined. The
identification of the flow's internal base geodesic with the time-shifted curve
`σ ↦ γ (a + σ)` and the variational-pair transport are exactly as in
`exists_geodesic_flow_step_jacobiTransport_manifold_at`, now run at each base `a`
against the *shared* flow rather than a fresh one.
...
/-- **Math.** **Ball-uniform manifold flow-step link.** Let `γ` be a geodesic
confined to a single chart at `β` on the window `(s₀ - δ, s₀ + δ)`. Then there is
a positive window radius `ρ` such that for *every* pair of times `a ≤ b` in
`(s₀ - ρ, s₀ + ρ)`:

* the endpoint map `flowEnd = (fun z ↦ Z z (b - a))` of a *single* geodesic flow
  `Z` (anchored at the chart state of `γ` at `s₀`) is strictly differentiable at
  the chart state `(φ_β(γ a), u̇^β(a))` of `γ` at `a`, with derivative `Dstep`;
* `flowEnd` sends that state to the chart state `(φ_β(γ b), u̇^β(b))` at `b`;
* for every manifold Jacobi field `(J, DJ)` along `γ` on `[a, b]`,
  `Dstep (jacobiVarPair β γ J DJ a) = jacobiVarPair β γ J DJ b`.

Unlike `exists_geodesic_flow_step_jacobiTransport_manifold_at`, which builds a
fresh flow per base time, the flow here is shared across the whole window
`(s₀ - ρ, s₀ + ρ)`; every subinterval of that window is a valid flow step. This
is the cover element consumed by the chart/flow-step partition of `[0,1]`.

Blueprint: `cor:dc-ch5-2-5`. -/
theorem exists_geodesic_flow_step_jacobiTransport_manifold_ball
        ...
        HasStrictFDerivAt flowEnd Dstep
          ...
          (∀ J DJ : ℝ → E, IsJacobiFieldAlongOn (I := I) g γ J DJ a b →
            Dstep (jacobiVarPair (I := I) g β γ J DJ a)
              = jacobiVarPair (I := I) g β γ J DJ b) ∧
          -- **neighbourhood semantics of the endpoint map** (rung 4): for every
          -- chart state `z` near the state of `γ` at `a`, `flowEnd z` is the
          -- chart-`β` state at time `b` of a geodesic realizing `z` at time `a`.
          0 < m ∧ b ∈ Ioo (a - m) (a + m) ∧
  ...
  obtain ⟨r, ε, T, Z, D, hr, hT, hTε, hflow, hgeoflow, hderiv⟩ :=
    exists_geodesic_flow_step (I := I) g β hzstar
  ...
  obtain ⟨hstrict, hD0, hDvar⟩ := hderiv za hza_ball'
  ...
  refine ⟨fun z => Z z τ, D za τ, ball zs r, m, ?_, ?_, ?_, hmpos, ?_, ?_, ?_⟩
  ...
  · -- variational-pair transport for any manifold Jacobi field
    ...
    have hIccwin : Icc a b ⊆ Ioo (s₀ - δ) (s₀ + δ) := by
      intro t ht
      exact ⟨by linarith [ht.1, haδ.1], by linarith [ht.2, hb.2, hρδ4]⟩
    ...
    have hkey := IsJacobiFieldAlongOn.variational_transport (I := I) g
      hgeo' hγc' hsrc' hW hJac hWl hab
    have hWb := hkey b ⟨hab, le_rfl⟩
