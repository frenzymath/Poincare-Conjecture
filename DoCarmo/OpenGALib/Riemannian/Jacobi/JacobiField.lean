...
* the **parallel orthonormal frame** `e₁(t),…,eₙ(t)` along the geodesic
  (`OpenGALib/Riemannian/Jacobi/ParallelFrame.lean`, `lem:dc-ch5-2-1-parallel-frame`);
* the **frame reduction** `J = Σᵢ fᵢ eᵢ ⟺` the scalar system `fⱼ'' + Σᵢ aᵢⱼ fᵢ = 0`
  (`OpenGALib/Riemannian/Jacobi/FrameReduction.lean`, `lem:dc-ch5-2-1-frame-reduction`);
* the **abstract second-order linear ODE** `f'' + A(t) f = 0` — existence for arbitrary
  initial data and uniqueness by initial conditions
  (`OpenGALib/Riemannian/Jacobi/JacobiEquationODE.lean`, `lem:dc-ch5-2-1-ode`).
...
`OpenGALib/Riemannian/Connection/ChartCurvatureMovingPoint.lean`) is the remaining step to
