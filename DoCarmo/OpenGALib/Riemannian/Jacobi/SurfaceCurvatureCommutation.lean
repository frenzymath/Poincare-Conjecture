...
* `chartCurvatureOp_eq_contraction2` — the Jacobi-field curvature operator
  `w ↦ R(γ', w)γ'` is the `(X, Z) = (γ', γ')` specialization.
-/
...
/-! ## The Jacobi curvature operator is the `(γ', ·, γ')` specialization -/

/-- **Math.** do Carmo Ch. 5, `def:dc-ch5-2-1`.  The Jacobi-equation curvature
operator `chartCurvatureOp g α u t : w ↦ R(γ', w)γ'` is the `(X, Y, Z) = (γ', w, γ')`
specialization of the general curvature contraction:
`chartCurvatureOp g α u t w = R(u̇, w, u̇)(u t)`. -/
theorem chartCurvatureOp_eq_contraction2 (g : RiemannianMetric I M) (α : M) (u : ℝ → E)
    (t : ℝ) (w : E) :
    chartCurvatureOp (I := I) g α u t w
      = chartCurvatureContraction2 (I := I) g α (deriv u t) w (deriv u t) (u t) := by
  classical
