...
The second sentence of `cor:dc-ch5-2-5`: *`d(exp_p)_v` is singular if
...
Blueprint: `cor:dc-ch5-2-5`, `def:conjugate-point`.
...
/-- **Math.** do Carmo `def:dc-ch5-3-1` / Morgan–Tian's **conjugate point**: `γ(t₁)`
is conjugate to `γ(0)` along `γ` if there is a nontrivial Jacobi field along
`γ|[0,t₁]` vanishing at both endpoints. -/
