...
do Carmo, *Riemannian Geometry*, Ch. 3, §2.2/2.5/2.7 (smooth dependence on initial
...
* `Riemannian.FlowDependence.intervalPrimitive` — the Volterra primitive
  `β ↦ (t ↦ ∫₀ᵗ β)` as a continuous linear operator of norm `≤ T`;
* `Riemannian.FlowDependence.postcomp` — postcomposition `α ↦ A ∘ α` by a continuous linear
...
* `Riemannian.FlowDependence.superposition` — the Nemytskii operator `α ↦ f ∘ α`, and
...
* `Riemannian.FlowDependence.picardResidual` — the residual of the Picard integral equation,
...
* `Riemannian.FlowDependence.hasStrictFDerivAt_of_picardResidual_curve` — the main theorem:
...
* `Riemannian.FlowDependence.linearRamp` and `sub_intervalPrimitive_postcomp_ramp` — the
...
an arbitrary base solution** (do Carmo, Riemannian Geometry, Ch. 3, §2.2/2.5/2.7 — C¹
...
equilibrium** (do Carmo, Riemannian Geometry, Ch. 3, §2.2/2.5/2.7, C¹ dependence on initial
