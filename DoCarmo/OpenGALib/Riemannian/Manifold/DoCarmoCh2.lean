...
full equality of connections (`AffineConnection.leviCivita_unique`). -/
...
structure AffineConnection (I : ModelWithCorners ℝ E H) (M : Type*)
...
def IsSymmetric (nabla : AffineConnection I M) : Prop :=
  ∀ (X Y : SmoothVectorField I M) (p : M),
...
def IsLeviCivita (g : RiemannianMetric I M) (nabla : AffineConnection I M) : Prop :=
...
theorem koszul_formula (g : RiemannianMetric I M) (nabla : AffineConnection I M)
...
@[ext] theorem ext {n₁ n₂ : AffineConnection I M} (h : n₁.cov = n₂.cov) :
    n₁ = n₂ := by
...
theorem leviCivita_unique' [FiniteDimensional ℝ E] [SigmaCompactSpace M]
    [T2Space M] (g : RiemannianMetric I M) (n₁ n₂ : AffineConnection I M)
    (h₁ : n₁.IsLeviCivita g) (h₂ : n₂.IsLeviCivita g) :
    n₁ = n₂ :=
...
(`AffineConnection.leviCivita_unique'`). The *existence* half constructs `∇` by
...
By `AffineConnection.koszul_formula` any Levi-Civita `∇` satisfies
...
Levi-Civita connection in its differentiated argument (`AffineConnection.add_right`
...
candidate Levi-Civita connection (`AffineConnection.leibniz` for the Koszul-dual
connection). -/
...
connection in its direction argument (`AffineConnection.add_left` for the
...
Levi-Civita connection in its direction argument (`AffineConnection.smul_left` for
...
hypothesis `hdual` consumed by `AffineConnection.isLeviCivita_of_koszulDual`;
...
Together with the uniqueness half `AffineConnection.leviCivita_unique'`, this
...
theorem AffineConnection.isLeviCivita_of_koszulDual [FiniteDimensional ℝ E]
...
together with the verification `AffineConnection.isLeviCivita_of_koszulDual`;
uniqueness is `AffineConnection.leviCivita_unique'`. Thus the entire theorem is
reduced to constructing one smooth Koszul-dual connection. -/
theorem AffineConnection.exists_unique_isLeviCivita_of_koszulDual
    [FiniteDimensional ℝ E] [SigmaCompactSpace M] [T2Space M]
    (g : RiemannianMetric I M)
    (hexists : ∃ nabla : AffineConnection I M,
      ∀ (X Y Z : SmoothVectorField I M) (p : M),
        2 * g.metricInner p ((nabla.cov Y X) p) (Z p) = g.koszulRHS X Y Z p) :
    ∃! nabla : AffineConnection I M, nabla.IsLeviCivita g := by
...
theorem RiemannianMetric.koszulDualSection_contMDiffAt [InnerProductSpace ℝ E]
...
`AffineConnection.leviCivita_unique'`. This closes the last analytic input
