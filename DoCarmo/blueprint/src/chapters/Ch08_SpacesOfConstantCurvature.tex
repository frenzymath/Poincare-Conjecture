...
\begin{lemma}[the norm of a constant-curvature Jacobi field is manifold-independent]
  ...
  \lean{Riemannian.Jacobi.metricInner_jacobiField_eq_of_constantCurvature,
    Riemannian.Jacobi.metricInner_jacobiField_transfer_of_constantCurvature}
  \leanok
  \uses{lem:dc-ch8-2-1-jacobi-const-solution,cor:dc-ch5-2-5,def:dc-ch5-2-1}
  Along a unit-speed geodesic $\gamma:[0,\ell]\to M$ on a manifold of constant sectional
  curvature $K_0$, let $J$ be the Jacobi field with $J(0)=0$ and covariant derivative
  $DJ$, and let $h$ solve $h''+K_0 h=0$, $h(0)=0$, $h'(0)=1$. Writing $Z=DJ(0)$ and
  $a=\langle Z,\gamma'(0)\rangle$,
  $$
  |J(t)|^2 = t^2 a^2 + h(t)^2\big(|Z|^2-a^2\big).
  $$
  The right-hand side involves \emph{no} manifold data beyond $K_0$ (through $h$), the
  time $t$, and the two scalar invariants $a=\langle Z,\gamma'(0)\rangle$ and $|Z|^2$.
  Consequently, if $\tilde M$ also has constant curvature $K_0$, $\tilde\gamma$ is a
  unit-speed geodesic, $\tilde J$ is a Jacobi field with $\tilde J(0)=0$, and the initial
  invariants match ($\langle\tilde{DJ}(0),\tilde\gamma'(0)\rangle=a$,
  ...
  and preserves both invariants, this is precisely the norm-preserving (hence isometry)
  step of \cref{thm:dc-ch8-2-1} in the same-curvature case, and requires \emph{no}
  parallel-transport conjugation $\phi_t$.
\end{lemma}
\begin{proof}
  \leanok
  Decompose $Z=a\,\gamma'(0)+Z_\perp$ with $Z_\perp\perp\gamma'(0)$, so
  $|Z_\perp|^2=|Z|^2-a^2$. The tangential Jacobi field $a\,t\,\gamma'(t)$ (do Carmo
  Remark 2.2's $t\gamma'$) and the normal Jacobi field $h(t)\,w(t)$, with $w$ the parallel
  transport of $Z_\perp$ (\cref{lem:dc-ch8-2-1-jacobi-const-solution}), both vanish at
  $t=0$ and have covariant derivatives $a\,\gamma'(0)$ resp. $Z_\perp$ there; their sum has
  initial data $(0,Z)$, so by uniqueness of Jacobi fields with prescribed initial data it
  equals $J$. Parallel transport is an isometry preserving orthogonality, so
  $\langle\gamma'(t),w(t)\rangle=\langle\gamma'(0),Z_\perp\rangle=0$, $|\gamma'(t)|^2=1$
  (unit speed) and $|w(t)|^2=|Z_\perp|^2=|Z|^2-a^2$; expanding the squared norm of the
  orthogonal combination $|a\,t\,\gamma'(t)+h(t)\,w(t)|^2$ gives
  $t^2a^2+h(t)^2(|Z|^2-a^2)$. As this depends only on $K_0,t,a,|Z|^2$, equating it on the
  two sides yields $|\tilde J(t)|=|J(t)|$; by \cref{cor:dc-ch5-2-5} these are the values of
  $(d\exp_p)$ resp. $(d\exp_{\tilde p})$, so $df$ is norm-preserving.
\end{proof}

\begin{theorem}[E. Cartan]
  \label{thm:dc-ch8-2-1}
  \dcref{ch8:2.1}
  \uses{cor:dc-ch5-2-5,lem:dc-ch8-2-1-jacobi-transfer}
  \notready
...
\begin{corollary}[Corollary 2.2]
  ...
  \dcref{ch8:2.2}
  \uses{thm:dc-ch8-2-1,lem:dc-ch8-2-1-jacobi-transfer-const,lem:dc-ch8-2-1-velocity-frame,
