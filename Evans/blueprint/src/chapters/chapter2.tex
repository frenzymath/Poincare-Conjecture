...
A $C^2$ function $u$ satisfying (1) is called a \textit{harmonic function}.
\end{definition}
...
\dcref{ch2:2.2-thm-8}
\group{Harmonic Functions}
\uses{thm:harmonic-function-derivative-estimates}
...
\group{Green's Functions}
\uses{def:greens-function,def:reflection-half-space}
...
\dcref{ch2:2.3-thm-1}
\group{Heat Equation}
\uses{def:fundamental-solution-heat-equation,lem:integral-fundamental-solution-heat}
...
\begin{proof}
If $u$ and $\bar{u}$ are two solutions of (22), apply \cref{thm:strong-maximum-principle-heat-equation} to $w := \pm(u - \bar{u})$.
\end{proof}
...
A direct calculation (cf. \cref{sec:heat-fundamental-solution-derivation}) shows
...
\dcref{ch2:2.4-thm-1}
\group{Wave Equation}
Assume $g \in C^2(\R)$, $h \in C^1(\R)$, and define $u$ by d'Alembert's formula (8). Then

(i) $u \in C^2(\R \times [0,\infty))$,

(ii) $u_{tt} - u_{xx} = 0$ in $\R \times (0,\infty)$,

and

(iii) $\lim\limits_{(x,t) \to (x^0,0), t>0} u(x,t) = g(x^0)$, \quad $\lim\limits_{(x,t) \to (x^0,0), t>0} u_t(x,t) = h(x^0)$
...
\group{Wave Equation}
\uses{thm:wave-equation-dalembert-solution}
...
(ii) $\left(\frac{1}{r}\frac{d}{dr}\right)^k\left(r^{2k-1}\phi(r)\right) = \sum_{j=0}^{k-1}\beta_j^k r^{j+1} \frac{d^j\phi}{dr^j}(r),$
...
u = g & \text{on } \R^n \times \{t = 0\}.
\end{cases}$$
Here $c \in \R$ and $b \in \R^n$ are constants.
\end{exercise}
