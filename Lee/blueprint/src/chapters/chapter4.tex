...
\[
\tilde{\Gamma}^k_{ij} = (A^{-1})^\ell_p A^p_i A^q_j \Gamma^p_{\ell q} + (A^{-1})^\ell_p A^q_\ell (A^p_j).
\]
\end{proposition}
...
\uses{exer:extension-smooth-function}
\uses{prob:embedded-submanifold-covariant-derivative}
...
\dcref{ch4:4.16}
\uses{prob:tensor-field-formulas}
...
\dcref{ch4:4.26}
\uses{prop:covariant-deriv-dependence-at-point}
\uses{thm:covariant-derivative-along-curve}
