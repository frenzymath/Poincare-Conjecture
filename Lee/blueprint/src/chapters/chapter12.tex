...
\dcref{ch12:12.3}
\uses{thm:monodromy-theorem-local-isometries}
...
\uses{cor:global-isometry-from-local-simply-connected}
\uses{thm:small-geodesic-ball-convex}
\uses{prob:model-manifolds-isometric-actions}
...
\uses{cor:isometric-to-quotient}
\uses{prob:model-manifolds-isometric-actions}
\uses{prop:discrete-subgroups-dimension-zero}
\uses{prob:isometry-group-acts-properly}
\uses{prop:riemannian-quotient-metric}
\uses{cor:complete-iff-covering-complete}
...
\dcref{ch12:12.6}
\uses{thm:characterization-locally-symmetric-spaces}
\uses{cor:global-isometry-from-local-simply-connected}
...
\dcref{ch12:12.8}
\uses{thm:conjugate-point-comparison-I}
\uses{prop:critical-points-conjugate}
\uses{cor:exponential-map-defined-implies-complete}
\uses{thm:completeness-of-covering-map}
...
\dcref{ch12:12.9}
\uses{lem:radial-distance-well-defined}
...
\uses{prob:round-metric-distance-sphere}
...
\dcref{ch12:12.24}
\uses{cor:diameter-ricci-bounded}
\uses{cor:complete-iff-covering-complete}
\uses{prop:fiber-cardinality-fundamental-group}
...
\dcref{ch12:12.29}
\uses{prob:milnor-fundamental-group}
...
\uses{thm:milnor-polynomial-growth}
\uses{prob:nonpositive-curvature-focal-points}
