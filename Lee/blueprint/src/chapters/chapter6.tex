...
\dcref{ch6:6.17}
\uses{prob:geodesic-ball-convexity}
...
Suppose $(\tilde{M}, \tilde{g})$ and $(M, g)$ are connected Riemannian manifolds with $M$ complete, and $\pi: \tilde{M} \to M$ is a local isometry. Then $\tilde{M}$ is complete and $\pi$ is a Riemannian covering map.
\end{theorem}
...
\dcref{ch6:6.28}
\uses{prob:closed-geodesic-free-homotopy}
