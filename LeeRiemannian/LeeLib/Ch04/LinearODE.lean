-- Standalone linear-ODE engine; mirrors DoCarmo's copy of the same infrastructure.
...
This is the analytic engine behind Lee, *Introduction to Riemannian Manifolds*,
Chapter 4, Theorem 4.31 (Existence, Uniqueness, and Smoothness for Linear ODEs),
minus the smooth-dependence-on-initial-condition clause. Over a Banach space `E`,
for a continuous bounded coefficient `A : ℝ → (E →L[ℝ] E)` the linear initial value
problem `V̇(t) = A(t) V(t)` has, on *any* compact interval `[a,b]`, a unique solution
with prescribed left-endpoint value, and the resulting endpoint map `x₀ ↦ V(b)` is a
linear injection.
...
ODE the solution exists on all of `[a,b]`; this file supplies that by chopping `[a,b]`
into finitely many short pieces, solving each locally, and gluing the pieces. Uniqueness
and the linearity/injectivity of the endpoint map come from Grönwall
(`Mathlib.Analysis.ODE.Gronwall`).

Main declarations:

* `IsSolOn A a b V` — `V` solves `V̇ = A(t) V` on the compact interval `[a,b]`.
* `exists_hasDerivWithinAt_Icc` — global existence on an arbitrary compact `[a,b]`,
  built from short-time existence (`exists_hasDerivWithinAt_of_small`) glued across
  subintervals (`exists_hasDerivWithinAt_glue`).
* `IsSolOn.eqOn_of_left` / `IsSolOn.eqOn_of_right` — forward / backward uniqueness
  (Grönwall): two solutions that agree at one endpoint agree on all of `[a,b]`.
* `IsSolOn.add` / `IsSolOn.const_smul` — superposition: the solutions form a vector
  space (a linear ODE has a linear solution set).
* `solOf` — a chosen solution with prescribed left-endpoint value, from global existence.
* `flowMap` — the linear endpoint (flow) map `x₀ ↦ V(b)`.
* `flowMap_injective` — the endpoint map is injective (backward uniqueness); in finite
  dimension this upgrades to a linear isomorphism (parallel transport).
-/
...
content behind the parallel transport operator. -/
...
finite dimension this upgrades to a linear isomorphism (parallel transport). -/
