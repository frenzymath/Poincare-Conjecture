/- Vendored from DoCarmo `OpenGALib/Riemannian/Geodesic/FlowC2Dependence.lean`. Namespace `Riemannian` mapped to
   `PetersenLib`; engineering infrastructure only, not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Geodesic.VariationalEquation
