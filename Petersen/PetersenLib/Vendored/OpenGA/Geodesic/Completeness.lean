/- Vendored from DoCarmo `OpenGALib/Riemannian/Geodesic/Completeness.lean`.
   Namespace `Riemannian` mapped to `PetersenLib`; engineering infrastructure only,
   not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Geodesic.FlowGeodesic
import PetersenLib.Vendored.OpenGA.Geodesic.IntrinsicUniqueness
import PetersenLib.Vendored.OpenGA.Geodesic.HopfRinow.GramBound
