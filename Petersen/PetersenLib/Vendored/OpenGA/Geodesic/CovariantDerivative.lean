/- Vendored from DoCarmo `OpenGALib/Riemannian/Geodesic/CovariantDerivative.lean`. Namespace `Riemannian` mapped to
   `PetersenLib`; engineering infrastructure only, not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Geodesic.Equation
import PetersenLib.Vendored.OpenGA.Geodesic.LinearODE
