/- Vendored from DoCarmo `OpenGALib/Riemannian/Geodesic/EquationTransfer.lean`.
   Namespace `Riemannian` mapped to `PetersenLib`; engineering infrastructure only,
   not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Connection.ChartChristoffelChange
...
Chain rule: with `τ = chartTransitionCC α β` and `A = Dτ(u_α(t₀))`,
...
theorem SolvesGeodesicODEAt.transfer
  ...
  set T : E → E := chartTransitionCC (I := I) (M := M) α β with hT_def
  ...
  have hsrc : ∀ᶠ τ in 𝓝 t₀,
      u τ ∈ chartTransitionCCSource (I := I) (M := M) α β := by
    filter_upwards [hmem] with τ hτ
    exact extChartAt_mem_chartTransitionCCSource (I := I) hτ.1 hτ.2
  -- the β-reading is the transition applied to the α-reading, near `t₀`
  have hw_eq : ∀ᶠ τ in 𝓝 t₀, w τ = T (u τ) := by
    filter_upwards [hmem] with τ hτ
    exact (chartTransitionCC_extChartAt (I := I) (β := α) (α := β) hτ.1).symm
  ...
  have hw_deriv : ∀ᶠ τ in 𝓝 t₀,
    ...
    have hT : HasFDerivAt T (fderiv ℝ T (u τ)) (u τ) := by
      have := hasFDerivAt_chartTransitionCC (I := I) (β := α) (α := β) hτsrc
      rwa [← fderiv_chartTransitionCC (I := I) (β := α) (α := β) hτsrc] at this
  ...
  have ht₀src : u t₀ ∈ chartTransitionCCSource (I := I) (M := M) α β :=
    hsrc.self_of_nhds
  ...
  have hc : HasDerivAt (fun τ => fderiv ℝ T (u τ)) (D2 v) t₀ :=
    (hasFDerivAt_fderiv_chartTransitionCC (I := I) (β := α) (α := β)
  ...
  have hAtcc : A = tangentCoordChange I α β (γ t₀) := by
    rw [hA_def, fderiv_chartTransitionCC (I := I) (β := α) (α := β) ht₀src]
    congr 1
  ...
  have hD2rw : fderiv ℝ (fderiv ℝ (chartTransitionCC (I := I) α β))
