/- Vendored from DoCarmo `OpenGALib/Riemannian/Connection/ChartChristoffelSmooth.lean`. Namespace `Riemannian` mapped to
   `PetersenLib`; engineering infrastructure only, not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Connection.ChartChristoffel
