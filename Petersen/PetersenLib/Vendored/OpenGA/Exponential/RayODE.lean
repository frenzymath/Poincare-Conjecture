/- Vendored from DoCarmo `OpenGALib/Riemannian/Exponential/RayODE.lean`. Namespace `Riemannian` mapped to
   `PetersenLib`; engineering infrastructure only, not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Geodesic.FlowC2Dependence
import PetersenLib.Vendored.OpenGA.Exponential.C2Ball
