/- Vendored from DoCarmo `OpenGALib/Riemannian/Exponential/MinimizingEqualityManifold.lean`.
   Namespace `Riemannian` mapped to `PetersenLib`; engineering infrastructure only,
   not a blueprint node. -/
import PetersenLib.Vendored.OpenGA.Exponential.MinimizingEquality
