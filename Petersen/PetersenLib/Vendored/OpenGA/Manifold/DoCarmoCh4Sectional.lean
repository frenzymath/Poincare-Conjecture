/- Vendored from DoCarmo `OpenGALib/Riemannian/Manifold/DoCarmoCh4Sectional.lean` (identical shared OpenGA infra).
   Namespace `Riemannian` mapped to `PetersenLib`. Engineering infrastructure for the
   Petersen Ch. 4 §4.1 computational simplifications — not itself a blueprint node.
   `IsAlgCurvatureForm` and its base lemmas now come from the Petersen-native
   `PetersenLib.Ch03.AlgebraicCurvatureForm` (single definition, per I-0098/I-0103);
   this file keeps only the inner-product-space layer (`wedgeSq`, `stdCurvForm`,
   `algSectionalCurvature` — renamed from `sectionalCurvature` to avoid clashing with
   the Ch03 manifold-level `PetersenLib.sectionalCurvature`). -/
