/- Vendored from DoCarmo `OpenGALib/Riemannian/Manifold/DoCarmoCh4Ricci.lean` (identical shared OpenGA infra).
   Namespace `Riemannian` mapped to `PetersenLib`; `scalarCurvature` renamed
   `algScalarCurvature` to avoid clashing with `PetersenLib.scalarCurvature`
   (Ch01/TensorConcepts). Engineering infrastructure — not itself a blueprint node. -/
