/- Petersen's own Riemannian infrastructure.
   Originally derived from the DoCarmo project's `DoCarmoLib/Riemannian/Geodesic/HopfRinow/ConstantSpeed.lean`; it is maintained
...
* `PetersenLib.Geodesic.speedSq g γ t = g(γ'(t), γ'(t))` — the intrinsic squared
...
theorem IsGeodesicOn.hasDerivAt_speedSq_zero {g : RiemannianMetric I M}
  ...
  have hct : ContinuousAt γ t := (hcont t ht).continuousAt (hs.mem_nhds ht)
  have hkey := PetersenLib.hasDerivAt_chartMetricInner_geodesic_speed_zero g (hγ t ht)
...
theorem IsGeodesicOn.edist_le (g : RiemannianMetric I M) (hg : g.IsRiemannianDist)
  ...
  have h1 : edist (γ a) (γ b) ≤ Manifold.pathELength I γ a b :=
    PetersenLib.HopfRinow.edist_le_pathELength_of_cmdiff hC1 hab
  refine h1.trans ?_
