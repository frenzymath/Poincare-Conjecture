...
\begin{remark}[conjugate versus focal points for an ellipse]
  ...
  \leanok
  % Lean: formalized as elementary plane geometry (Ch03/EllipseEvolute.lean,
  % conjugateVsFocal_ellipse), which is an honest ALTITUDE DROP: there is no
  % FocalPoint definition in PetersenLib (Ch03/DistanceFunctions.lean explicitly
  % punts on it) and no normal-exponential/caustic API, so the illustration is made
  % precise in the plane and its link to def:pet-ch3-conjugate-focal-point is by
  % prose, not by a formal dependency.  The three parts are:
  % centerOfCurvature_ellipse (the centre of curvature at c(t) — where nearby
  % normals meet, i.e. the conjugate point along the normal — is the evolute point),
  % mem_ellipseNormalLine_axis + axis_inter_eq (the normals at t and -t both meet
  % the major axis at exactly ((a^2-b^2)/a)cos t), and evolute_cusp_lt_focus (the
  % correction below).  The sqrt-free form of the centre of curvature keeps
  % Real.sqrt out of the computation entirely.
  \uses{def:pet-ch3-conjugate-focal-point}
  For the lower half of an ellipse $c(t)=(a\cos t,b\sin t)$, $0<b<a$, conjugate
  points occur along its evolute
  $e(t)=\big(\tfrac{a^2-b^2}{a}\cos^3t,\ \tfrac{b^2-a^2}{b}\sin^3t\big)$: the
  centre of curvature at $c(t)$, which is where the normals infinitesimally near
  $c(t)$ meet, is $e(t)$. For the entire ellipse, the normal lines from the top and
  bottom arcs (the parameters $t$ and $-t$) intersect on the major axis, at the
  single point $\big(\tfrac{a^2-b^2}{a}\cos t,0\big)$; so the focal set is the
  segment swept by these, namely the segment between the two \emph{cusps of the
  evolute} on the major axis, $e(0)=\big(\tfrac{a^2-b^2}{a},0\big)$ and
  $e(\pi)=\big(-\tfrac{a^2-b^2}{a},0\big)$.

  \emph{Correction.} The printed source says instead that this focal set ``is the
  line between the focal points of the ellipse''. That is false: the foci lie at
  $\pm\sqrt{a^2-b^2}$, whereas the segment just described has endpoints
  $\pm\tfrac{a^2-b^2}{a}$, and
  \[
    \frac{a^2-b^2}{a}<\sqrt{a^2-b^2}\qquad\text{for every }0<b<a
  \]
  (equivalently $c^2/a<c$ for $c=\sqrt{a^2-b^2}$, since $c<a$). So the focal set is
  strictly contained in the segment joining the foci and never equals it. The
  geometry and Fig.~3.1 of the source are correct — the segment drawn there is
  visibly shorter than the distance between the foci — but the prose identification
  of its endpoints conflates the Riemannian \emph{focal point} just defined (where
  integral curves of $\nabla r$ meet) with the two conic foci.
  \source{petersen:page-0120}
\end{remark}
...
\begin{remark}[Exercise 3.4.9]
  ...
  \leanok
  % Lean: `X`, `Y` are realized as the lifts `liftFst X (p₁,p₂) = (X p₁, 0)` and
  % `liftSnd Y (p₁,p₂) = (0, Y p₂)` under `T_{(p₁,p₂)}(M×N) = T_{p₁}M × T_{p₂}N`.
  % Both conjuncts are proved: `∇_{liftFst X}(liftSnd Y) = 0` and `sec(X,Y) = 0`.
  % The vanishing is of the MIXED curvatures (the `X`–`Y` planes); the factors keep
  % their own curvature, which is what `cov_liftFst_liftFst` (the splitting of ∇) records.
...
\begin{remark}[Exercise 3.4.21 — fundamental theorem of (hyper-)surface theory]
  ...
  \leanok
  % Lean: parts (1)-(2) formalized (Ch03/HypersurfaceFundamentalTheorem.lean,
  % exercise3_4_21) — the Gauss formula (gaussFormula), whose content is the
  % codimension-one decomposition II_{jk} = -Pi_{jk}*N (secondFund_eq_smul_normal,
  % from a spanning hypothesis + nondegeneracy of the ambient form), the Gauss
  % equation (gaussEquation_hypersurface) and the Codazzi equation
  % (codazziEquation), the latter being pd3_symm (Clairaut) read in the normal
  % direction.  CAVEAT on (2): Petersen asks for an EQUIVALENCE, and only the
  % forward direction is proved — that an actual immersion satisfies both equations
  % (Clairaut ==> Gauss and Codazzi).  The reverse (Gauss-Codazzi ==> integrable) is
  % the same Frobenius statement as part (3) and is not formalized.
  % Parts (3)-(4) are the *existence* (Bonnet) statements —
  % reconstructing an immersion from a Gauss-Codazzi-compatible (g,Pi), and
  % realizing a constant-curvature metric inside a sphere — and need an
  % integral-manifold / Frobenius PDE-existence theorem, absent from Mathlib and
  % PetersenLib (the same blocker rem:pet-ch3-ex-20 records for its part (2)), and
  % are not formalized.  The converse direction of (4) IS formalized, as
  % umbilic_constantCurvature: an immersion whose normal is the position field
  % N = R^{-1}F has Pi = R^{-1}g and hence constant curvature R^{-2}.
...
\begin{remark}[Exercise 3.4.22]
  ...
  \leanok
  % Lean: parts (1)-(2) formalized (Ch03/HypersurfaceFundamentalTheorem.lean,
  % exercise3_4_22), as the eps = -1 case of the same theorems that give
  % exercise3_4_21 at eps = +1: the ambient form enters the whole coordinate
  % immersion layer (Ch03/EuclideanImmersionCurvature.lean) only as a symmetric
  % nondegenerate bilinear form B, so Minkowski space is that layer at
  % B = minkowskiForm (Ch01/Minkowski.lean) and the only difference from the
  % Euclidean case is the scalar B(N,N) = -1, which flips the sign of the Gauss
  % equation and leaves Codazzi untouched.  The characterization of H^n(R) is the
  % eps = -1 case of umbilic_constantCurvature: an immersion with N = +-R^{-1}F has
  % constant curvature -R^{-2}.  As in rem:pet-ch3-ex-21, the *existence* half —
  % that every metric of constant curvature -R^{-2} is locally realized this way —
  % needs Frobenius and is not formalized.
