...
\begin{theorem}[Theorem 6.3.3 — Myers, 1941]
  ...
  \lean{PetersenLib.myersRicciDiameterBound,
    PetersenLib.longGeodesic_notMinimizing_of_ricciBound,
    PetersenLib.IsMyersRicciVariation}
  ...
  \emph{Formalization note (compactness + diameter bound; $\pi_1$-finiteness omitted).}
  \texttt{myersRicciDiameterBound} is the Ricci analogue of
  \cref{cor:pet-ch6-hopf-rinow-myers-diameter}, sharing its diameter-and-compactness passage
  (\texttt{diameterBound\_of\_notMinimizing}) and its packaged not-minimizing bridge
  (\texttt{riemannianDistance\_lt\_of\_negSecondVar\_unitSpeed}), but driving them from the
  \emph{Ricci} core \cref{lem:pet-ch6-myers-index} rather than the sectional one. It takes the
  \emph{technical} existence hypothesis \texttt{hvar} of a Myers frame variation
  (\texttt{IsMyersRicciVariation}: $m$ common-base proper exponential variations with parallel
  unit fields $E_i\perp\dot\sigma$, fields $\sin(\pi t/l)E_i$) \emph{together with} the Ricci
  lower bound in frame-sum form $m\,k\le\sum_i\sec(E_i,\dot\sigma)$ on $[0,l]$. That frame-sum
  bound equals $\operatorname{Ric}(\dot\sigma,\dot\sigma)\ge(n-1)k$ for the orthonormal frame with
  $m=n-1$ (\texttt{ricciCurvature\_eq\_sum\_sectionalCurvature}) and is exactly the honest Ricci
  form already carried by \cref{lem:pet-ch6-myers-index}
  (\texttt{myersRicci\_secondVariation\_neg}); it enters genuinely through
  \texttt{myers\_index\_core}, producing some $V_i$ with $d^2E/ds^2|_0<0$, which then fails to
  minimize. The $\pi_1$-finiteness clause is \emph{not} formalized (covering-space theory absent).
  \source{petersen:page-0267}
\end{theorem}
\begin{proof}
