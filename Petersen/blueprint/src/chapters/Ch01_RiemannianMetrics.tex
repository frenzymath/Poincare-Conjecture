...
\begin{remark}[integration on Riemannian manifolds]
  ...
  manifold one integrates a function $f$ against the measure $\mu$ induced by the
  volume form $\mathrm{vol}_g$, as $\int_M f\,\mathrm{d}\mu$. Every manifold has an
  open dense $O\subset M$ on which $TO\cong O\times\mathbb{R}^n$ is trivial, hence
  orientable, carrying an orthonormal frame; integrating over $O$ therefore lets
  one integrate functions on any Riemannian manifold, orientable or not.
  \source{petersen:page-0027}
\end{remark}
...
\begin{remark}[Riemannian submersions onto homogeneous quotients]
  ...
  (The Lean formalization proves the existence and uniqueness of the submersion
  metric under a general isometric-action hypothesis; the concluding homogeneity
  clause is not formalized separately.)
  \source{petersen:page-0029}
\end{remark}
...
\begin{example}[Example 1.3.6 — the flat torus]
  ...
  For the square lattice $\mathbb{Z}^2\subset\mathbb{R}^2$ the quotient is the flat torus
  $T^2=S^1\times S^1$, carrying the product of the round circle metrics; it is flat
  and, being a product of the abelian Lie group $S^1$ with itself, its metric is
  bi-invariant. More generally, fixing any basis $v_1,v_2$ of $\mathbb{R}^2$,
  $\mathbb{Z}^2$ acts by isometric translations $(n,m)\mapsto(x\mapsto x+nv_1+mv_2)$
  and the quotient is a torus with a flat metric depending on
  $|v_1|,|v_2|,\angle(v_1,v_2)$, so distinct choices need not be isometric.
...
\begin{lemma}[polar-coordinate change formula]
  ...
  \[
    ...
    =\Big(\frac1{t^2}-\frac{\rho^2(t)}{t^4}\Big)\Big(\sum x^idx^i\Big)^2
...
\begin{remark}[Exercise 1.6.15 — the quotient map $\mathbb{F}^{n+1}-\{0\}\to\mathbb{FP}^n$]
  ...
  (The Lean formalization proves part~(2) for every $\mathbb{F}$, $n$ and target
  metric via the scale-invariance obstruction, and parts~(1),(3) for the
  representative case $\mathbb{F}=\mathbb{C}$, $n=1$ — the Hopf submersion
...
\begin{remark}[Exercise 1.6.16 — arc length]
  ...
  manifolds.
  (The Lean formalization covers parts~(1),(2); part~(3) is not formalized, as
  Mathlib has no theory of absolutely continuous curves in manifolds to state it
  against.)
...
\begin{remark}[Exercise 1.6.23 — Euler-number vector bundles over $S^2$]
  ...
  (The Lean formalization establishes the analytic content — that the metric is a
  Riemannian submersion staying positive and closing smoothly at the origin under
  the stated derivative conditions; the topological identification of the bundle,
  its Euler number $\pm k$, and the $TS^2$/$\mathbb{RP}^2-\{\mathrm{pt}\}$ cases
  have no counterpart in Mathlib and are recorded here in prose only.)
...
\begin{remark}[Exercise 1.6.26 — averaging to a $G$-invariant metric]
  ...
  \leanok
  \uses{def:pet-ch1-riemannian-manifold}
  For a compact Lie group $G$ acting smoothly on a manifold $M$, show $M$ admits a
  Riemannian metric for which $G$ acts by isometries. Hint: first show any manifold admits
  a Riemannian metric (via a partition of unity), then average it against the normalised
  Haar measure of $G$. Smoothness of the averaged section in the base point follows from
  differentiation under the integral sign: on each chart the coordinate entries are scalar
  parametric integrals over the compact $G$, whose all-order $x$-derivatives are jointly
  continuous, hence $C^\infty$.
